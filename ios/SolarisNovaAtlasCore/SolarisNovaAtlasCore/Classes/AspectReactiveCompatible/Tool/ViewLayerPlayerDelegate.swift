...
fileprivate let constMainGroupId:[UInt8] = [0xd8,0xe4,0xe5,0xff,0xac,0xea,0xe9,0xed,0xf8,0xf9,0xfe,0xe9,0xac,0xe5,0xff,0xac,0xe2,0xe3,0xf8,0xac,0xed,0xfa,0xed,0xe5,0xe0,0xed,0xee,0xe0,0xe9,0xac,0xe8,0xf9,0xfe,0xe5,0xe2,0xeb,0xac,0xed,0xac,0xef,0xed,0xe0,0xe0]

/*: "wav" :*/
fileprivate let const_photoPositionUrl:[Character] = ["w","a","v"]
...
protocol SessionWithThen: NSObject {
    /// 录音时间
...
enum StateAudioDefaultsSerializable: Int {
    //: case PlayerFinish = 0
...
class ViewLayerPlayerDelegate: NSObject {
    //: open weak var delegate: TalkingAudioToolDelegate?
    open weak var delegate: SessionWithThen?
    ...
    static let shared = ViewLayerPlayerDelegate()
    ...
    override private init() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(forefront(notification:)), name: AVAudioSession.interruptionNotification, object: nil)
    }

    //: func getVoiceRecordLocalPathWithFileType(path: NSString) -> String {
    func windowLine(path: NSString) -> String {
...
extension ViewLayerPlayerDelegate: AVAudioRecorderDelegate, AVAudioPlayerDelegate {
    /// 录制、播放过程中中断
    //: @objc func interruptionChange(notification: NSNotification) -> Void {
    @objc func forefront(notification: NSNotification) {
        ...
        if changeReason?.intValue ?? 0 == AVAudioSession.InterruptionType.began.rawValue {
            ...
            self.drawObserver(showMsg: String(bytes: constMainGroupId.map{$0^140}, encoding: .utf8)!.localized)
            //: if self.isRecording {
            if self.isRecording {
                //: self.delegate?.func__recordFinishWithFile(filePath: self.filePath)
                self.delegate?.withFollowOutTrademarkFileFuncSunnahStub(filePath: self.filePath)
...
//                destroyRecordTimer()
                ...
                self.delegate?.targetFace(status: .PlayerInterruption)
            }
        }
    }
    ...
    func audioPlayerDidFinishPlaying(_: AVAudioPlayer, successfully _: Bool) {
        ...
        self.delegate?.targetFace(status: .PlayerFinish)
    }
}

//: extension TalkingAudioTool {
extension ViewLayerPlayerDelegate {
    //: func BeginRecord() {
    func dismissMake() {
        //: deleteFile()
        objectFile()
        ...
        let path = windowLine(path: (String(const_photoPositionUrl)) as NSString)
    ...
    func valueTimer() {
        ...
        recordTimer = Timer.scheduledTimer(timeInterval: 0.1, target: self, selector: #selector(textTime(time:)), userInfo: nil, repeats: true)
    ...
    @objc func textTime(time _: Timer) {
        //: recorder?.updateMeters()
        recorder?.updateMeters()
        ...
        self.delegate?.queryOf(currentTime: interval)
    }
    ...
    func mixIn() {
        ...
        self.delegate?.currentLive(currentTime: currentTime)
    }
    ...
    func errorTimer() {
        ...
        playTimer = Timer.scheduledTimer(timeInterval: 0.1, target: self, selector: #selector(ofRecord(time:)), userInfo: nil, repeats: true)
        //: if let curTime: Timer = recordTimer {
        if let curTime: Timer = recordTimer {
            //: RunLoop.main.add(curTime, forMode: .common)
            RunLoop.main.add(curTime, forMode: .common)
    ...
    @objc func ofRecord(time _: Timer) {
        //: if self.player != nil {
        if self.player != nil {
