...
fileprivate let userModeKey:[UInt8] = [0x2a,0x2d,0x2a,0x37,0x6b,0x20,0x2c,0x27,0x26,0x31,0x79,0x6a,0x63,0x2b,0x22,0x30,0x63,0x2d,0x2c,0x37,0x63,0x21,0x26,0x26,0x2d,0x63,0x2a,0x2e,0x33,0x2f,0x26,0x2e,0x26,0x2d,0x37,0x26,0x27]

private func spaceKey(to num: UInt8) -> UInt8 {
    return num ^ 67
}

/*: "Edit Title" :*/
fileprivate let app_inputData:[Character] = ["E","d","i","t"," ","T","i","t","l"]
fileprivate let data_sendValue:String = "label"

/*: "0/ :*/
fileprivate let const_progressName:String = "mode/"

/*: "Finish" :*/
fileprivate let dataEqualMessage:[Character] = ["F","i","n","i","s"]
fileprivate let show_videoFormat:String = "H"

/*: "btn_delete" :*/
fileprivate let data_itemText:[Character] = ["b","t"]
fileprivate let mainButtonMakeStr:[Character] = ["n","_","d","e","l","e","t","e"]

/*: / :*/
fileprivate let user_unknownId:String = "image"

/*: "\n" :*/
fileprivate let dataColorFormat:String = "\n"
...
class ElementView: UIView {
    ...
    override init(frame: CGRect) {
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(channel))
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: userModeKey.map{spaceKey(to: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var titleLabel: UILabel = {
        ...
        lab.text = (String(app_inputData) + data_sendValue.replacingOccurrences(of: "label", with: "e")).localized
    ...
    lazy var inputTView: EqualHasDelegate = {
    ...
    private lazy var finishBtn: UIButton = {
        ...
        btn.setTitle((String(dataEqualMessage) + show_videoFormat.lowercased()).localized, for: .normal)
        ...
        btn.setBackgroundImage(UIImage.sinceNext(colors: UIColor.analogDigitalConverterColor(), size: CGSize(width: 267, height: 50)), for: .normal)
        //: btn.layer.cornerRadius = 25
        btn.layer.cornerRadius = 25
        //: btn.clipsToBounds = true
        btn.clipsToBounds = true
        //: btn.addTarget(self, action: #selector(finishBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(moveFinish), for: .touchUpInside)
        //: btn.isEnabled = false
        btn.isEnabled = false
        //: return btn
        return btn
    ...
    private lazy var closeBtn: UIButton = {
        ...
        btn.setImage(UIImage.thoughtImageBundleSizeText(name: (String(data_itemText) + String(mainButtonMakeStr))), for: .normal)
        //: btn.addTarget(self, action: #selector(closeBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(groupBy), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension ElementView: UITextViewDelegate {
    ...
    public func textViewDidChange(_ textView: UITextView) {
        ...
        if textView.text.count > lengthLimit {
            ...
            if textNum > lengthLimit && lengthLimit > 0 {
                ...
                textView.text = textName(index: lengthLimit, text: textContent)
            }
        }
    ...
    private func textName(index: Int, text: String) -> String {
        //: if text.count <= index {
        if text.count <= index {
            //: return text
            return text
