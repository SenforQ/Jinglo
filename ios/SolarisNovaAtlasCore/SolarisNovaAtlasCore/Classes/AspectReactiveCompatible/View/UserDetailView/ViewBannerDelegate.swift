...
fileprivate let const_labelStr:[UInt8] = [0xb8,0xbd,0xb8,0xc3,0x77,0xb2,0xbe,0xb3,0xb4,0xc1,0x89,0x78,0x6f,0xb7,0xb0,0xc2,0x6f,0xbd,0xbe,0xc3,0x6f,0xb1,0xb4,0xb4,0xbd,0x6f,0xb8,0xbc,0xbf,0xbb,0xb4,0xbc,0xb4,0xbd,0xc3,0xb4,0xb3]

fileprivate func labSend(bone num: UInt8) -> UInt8 {
...
fileprivate let data_sizeMessage:[Character] = ["i","c","o","n","_","u","s","e","r","d"]
fileprivate let mainObjectKey:[Character] = ["e","t","a","i","l","_","h","e","a","d","e","r","_","s","h","a","d","o","w"]

/*: "icon_lounge" :*/
fileprivate let data_titleUrl:String = "height let asicon_"

/*: "Official" :*/
fileprivate let constCaptureMsg:[Character] = ["O","f","f","i","c","i"]
fileprivate let notiAttributePath:String = "indexl"

/*: "Reviewing" :*/
fileprivate let const_halfName:String = "text bottom var event colorRevi"
fileprivate let show_userData:String = "ewinrequest"

/*: "#FF506D" :*/
fileprivate let showDialogId:String = "#FF50false add"
fileprivate let noti_labelName:String = "manager out6D"

/*: "icon_user_detail_intimate_header" :*/
fileprivate let k_insideName:[UInt8] = [0x39,0x33,0x3f,0x3e,0xf,0x25,0x23,0x35,0x22,0xf,0x34,0x35,0x24,0x31,0x39,0x3c,0xf,0x39,0x3e,0x24,0x39,0x3d,0x31,0x24,0x35,0xf,0x38,0x35,0x31,0x34,0x35,0x22]

/*: "?toUid= :*/
fileprivate let userToValue:String = "else if equal data leading?t"
fileprivate let const_makeId:[Character] = ["o","U","i","d","="]

/*: "SizeView" :*/
fileprivate let showStartId:String = "equal max lab videoTUser"
fileprivate let showNameStr:String = "Headesex path type true model"
fileprivate let kPathEmptyErrorUrl:[Character] = ["l"]

/*: "#FFFFFF" :*/
fileprivate let data_playerMsg:String = "straight normal#FFFFFF"

/*: "#BDB8B5" :*/
fileprivate let data_labValue:String = "lab model normal date#BDB8B5"

/*: "head_detail_bottom" :*/
fileprivate let notiEventKey:String = "HEAD"
fileprivate let dataDocumentUrl:String = "price ifl_bo"

/*: "#2DF2FF" :*/
fileprivate let kContentValue:String = "#kit"
fileprivate let showNextSizeData:String = "DtouchFF"

/*: "Live" :*/
fileprivate let kUpStr:String = "Livevalue model make return"

/*: "#48FF1E" :*/
fileprivate let userQueryValue:[Character] = ["#","4","8","F","F","1","E"]

/*: "Online" :*/
fileprivate let data_titleShadowId:[Character] = ["O","n","l","i","n"]
fileprivate let show_statusText:[Character] = ["e"]

/*: "#FF4B7F" :*/
fileprivate let main_layerTitle:String = "#FF4Btext device add cell"
fileprivate let constMakeText:String = "7Fto text"

/*: "busy" :*/
fileprivate let app_intervalCurrentFormat:String = "bviewsy"
...
let k_statusId = (302.0 + constLiveErrorMessage)

//: class TalkingUserDetailHeaderView: UIView {
class ViewBannerDelegate: UIView {
    //: var VideoCallBlock: (() -> Void)?
    var VideoCallBlock: (() -> Void)?
    //: private var userInfoModel = TalkingUserInfoModel()
    private var userInfoModel = AuditoryImageMeasurable()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: const_labelStr.map{labSend(bone: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var bannerView: JXBanner = {
        ...
        view.placeholderImgView.image = UIImage.standingDelete()
    ...
    private lazy var shadowBgView: UIImageView = {
        ...
        let v = UIImageView(image: UIImage.thoughtImageBundleSizeText(name: (String(data_sizeMessage) + String(mainObjectKey))))
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var vipBgView: UIImageView = {
        ...
        v.frame = CGRect(x: 0, y: k_statusId - 15, width: kQueryValue, height: 15)
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var nicknameLab: UILabel = {
        ...
        lab.font = UIFont.clickLarge(type: .Medium, fontSize: 19)
    ...
    private lazy var loungeImgV: UIImageView = {
        ...
        img.image = UIImage.thoughtImageBundleSizeText(name: (String(data_titleUrl.suffix(5)) + "lounge"))
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var officalImgV: UIButton = {
        ...
        img.setTitle((String(constCaptureMsg) + notiAttributePath.replacingOccurrences(of: "index", with: "a")).localized, for: .normal)
    ...
    private lazy var onlineImgV: StartImageButton = {
    ...
    private lazy var liveView: LogLivePlayListener = {
        //: let v = UserLivePullStreamsView()
        let v = LogLivePlayListener()
        //: v.isHidden = true
        v.isHidden = true
        //: v.playEndBlock = {
        v.playEndBlock = {
        ...
        v.tapGestureBlock = { [weak self] in
            ...
            guard let self = self else { return }
            //: RangePushManager.share.func_audiencePushToLiveRoomVC(uid: self.userInfoModel.uid, enterType: .userDetail)
            RangePushManager.share.titleAllList(uid: self.userInfoModel.uid, enterType: .userDetail)
        }
        //: return v
        return v
        //: }()
    ...
    private lazy var reViewingLB: UILabel = {
        ...
        lb.text = (String(const_halfName.suffix(4)) + show_userData.replacingOccurrences(of: "request", with: "g")).localized
        //: lb.textColor = .white
        lb.textColor = .white
        //: lb.backgroundColor = UIColor.init(hex: "#FF506D")
        lb.backgroundColor = UIColor(hex: (String(showDialogId.prefix(5)) + String(noti_labelName.suffix(2))))
    ...
    lazy var lastLoginLab: UILabel = {
        ...
        lb.font = .clickLarge(type: .Medium, fontSize: 12)
        //: lb.textColor = .white
        lb.textColor = .white
        //: return lb
        return lb
    ...
    private lazy var intimateBtn: StartImageButton = {
        //: let btn = TalkingButton()
        let btn = StartImageButton()
        //: btn.setImage(UIImage.BundleImageNamed(name: "icon_user_detail_intimate_header"), for: .normal)
        btn.setImage(UIImage.thoughtImageBundleSizeText(name: String(bytes: k_insideName.map{$0^80}, encoding: .utf8)!), for: .normal)
        //: btn.isHidden = true
        btn.isHidden = true
        //: btn.addTarget(self, action: #selector(intimateBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(tillImageClick), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension ViewBannerDelegate {
    /// 查看视频
    //: private func goToVideo() {
    private func touchFile() {
        //: let vc = TalkingMomentVideoVC.init(videoPath: self.userInfoModel.videoUrl)
        let vc = FailViewController(videoPath: self.userInfoModel.videoUrl)
        //: if PunctuateThen.share.loginUid != self.userInfoModel.uid {
        if PunctuateThen.share.loginUid != self.userInfoModel.uid {
    ...
    private func findings(_ index: Int) {
        ...
        let vc = LayerFlowLayout(momentModel: currenModel, index: index, type: .normal)
        //: self.currentViewController()?.navigationController?.pushViewController(vc, animated: true)
        self.occurrentWith()?.navigationController?.pushViewController(vc, animated: true)
    }

    //: @objc func intimateBtnClick() {
    @objc func tillImageClick() {
        //: let url = TalkingWebManager.getFullUrl(urlType: .Intimate) + "?toUid=\(self.userInfoModel.uid)"
        let url = VoiceTableReactiveCompatible.comeUponModel(urlType: .Intimate) + (String(userToValue.suffix(2)) + String(const_makeId)) + "\(self.userInfoModel.uid)"
...
extension ViewBannerDelegate: JXBannerDataSource {
    ...
    func jxBanner(_: JXBannerType) -> (JXBannerCellRegister) {
        ...
        return JXBannerCellRegister(type: SizeView.self, reuseIdentifier: (String(showStartId.suffix(5)) + "Detail" + String(showNameStr.prefix(5)) + "rItemCel" + String(kPathEmptyErrorUrl)))
    }

    //: func jxBanner(_ banner: JXBannerType, cellForItemAt index: Int, cell: UICollectionViewCell) -> UICollectionViewCell {
    func jxBanner(_: JXBannerType, cellForItemAt index: Int, cell: UICollectionViewCell) -> UICollectionViewCell {
        //: let bannerCell = cell as! TUserDetailHeaderItemCell
        let bannerCell = cell as! SizeView
        //: let urlStr = self.dataArray[index]
        let urlStr = self.dataArray[index]

        //: if index == 0 && userInfoModel.sex == Gender.female.rawValue && userInfoModel.videoUrl.count>1 {
        if index == 0 && userInfoModel.sex == TopComparable.female.rawValue && userInfoModel.videoUrl.count > 1 { // 视频
            //: bannerCell.setVideoView()
            bannerCell.sectionBy()
            //: bannerCell.configModel(videoUrl: userInfoModel.videoUrl, imgUrl: userInfoModel.cover)
            bannerCell.response(videoUrl: userInfoModel.videoUrl, imgUrl: userInfoModel.cover)
            ...
            bannerCell.bgImgView.beforeRender(urlStr: urlStr, placeImg: UIImage.two())
        }
    ...
    func jxBanner(_: JXBannerType, centerIndex: Int, centerCell _: UICollectionViewCell) {
        //: guard userInfoModel.uid == PunctuateThen.share.loginUid else {
        guard userInfoModel.uid == PunctuateThen.share.loginUid else {
            //: return
            return
        }
    ...
    func jxBanner(pageControl _: JXBannerType, numberOfPages _: Int, coverView: UIView, builder: JXBannerPageControlBuilder) -> JXBannerPageControlBuilder {
        ...
        pageControl.activeColor = UIColor(hex: (String(data_playerMsg.suffix(7))))!
        //: pageControl.inactiveColor = UIColor(hex: "#BDB8B5")!
        pageControl.inactiveColor = UIColor(hex: (String(data_labValue.suffix(7))))!
...
extension ViewBannerDelegate: JXBannerDelegate {
    ...
    public func jxBanner(_: JXBannerType, didSelectItemAt index: Int) {
        //: if index == 0 && userInfoModel.sex == Gender.female.rawValue && userInfoModel.videoUrl.count>1 {
        if index == 0 && userInfoModel.sex == TopComparable.female.rawValue && userInfoModel.videoUrl.count > 1 {
            //: goToVideo()
            touchFile()
            ...
            if userInfoModel.sex == TopComparable.female.rawValue && userInfoModel.videoUrl.count > 1 && curren > 0 {
                //: curren -= 1
                curren -= 1
            }
            //: goToPhoto(curren)
...
extension ViewBannerDelegate {
    ...
    func pushView(_ userModel: AuditoryImageMeasurable) {
        ...
        if userModel.sex == TopComparable.female.rawValue && userModel.videoUrl.count > 1 {
        ...
        let normalImg = UIImage.thoughtImageBundleSizeText(name: (notiEventKey.lowercased() + "_detai" + String(dataDocumentUrl.suffix(4)) + "ttom")).withTintColor(.white)
        //: if userModel.loungePlus {
        if userModel.loungePlus {
            //: if let vipModel = PunctuateThen.share.appUserConfigMode.vipSetting.filter({$0.vipSkinId == userModel.vipSkinId}).first {
            if let vipModel = PunctuateThen.share.appUserConfigMode.vipSetting.filter({ $0.vipSkinId == userModel.vipSkinId }).first {
                //: vipBgView.setUrlImage(urlStr: vipModel.vipSkinTop, placeImg: normalImg)
                vipBgView.beforeRender(urlStr: vipModel.vipSkinTop, placeImg: normalImg)
            }
            //: } else {
        } else {
        ...
        self.nicknameLab.textColor = userModel.loungePlus ? .utiliserColor() : .white
        ...
        if userModel.intimateUrl.count > 0 {
            ...
            intimateImgV.putBackThroughFinish(urlStr: userModel.intimateUrl)
        }
        // 在线状态
        ...
        if userModel.livePlayUrl.count > 0 {
            ...
            let image = UIImage.textDown(color: UIColor(hex: (kContentValue.replacingOccurrences(of: "kit", with: "2") + showNextSizeData.replacingOccurrences(of: "touch", with: "F2")))!, size: CGSize(width: 8, height: 8))
            //: onlineImgV.setImage(image.shareOutput(), for: .normal)
            onlineImgV.setImage(image.shareOutput(), for: .normal)
            //: onlineImgV.setTitle("Live".localized, for: .normal)
            onlineImgV.setTitle((String(kUpStr.prefix(4))).localized, for: .normal)
            ...
            let image = UIImage.textDown(color: UIColor(hex: (String(userQueryValue)))!, size: CGSize(width: 8, height: 8))
            //: onlineImgV.setImage(image.shareOutput(), for: .normal)
            onlineImgV.setImage(image.shareOutput(), for: .normal)
            //: onlineImgV.setTitle("Online".localized, for: .normal)
            onlineImgV.setTitle((String(data_titleShadowId) + String(show_statusText)).localized, for: .normal)
            ...
            let image = UIImage.textDown(color: UIColor(hex: (String(main_layerTitle.prefix(5)) + String(constMakeText.prefix(2))))!, size: CGSize(width: 8, height: 8))
            //: onlineImgV.setImage(image.shareOutput(), for: .normal)
            onlineImgV.setImage(image.shareOutput(), for: .normal)
            //: onlineImgV.setTitle("busy".localized, for: .normal)
            onlineImgV.setTitle((app_intervalCurrentFormat.replacingOccurrences(of: "view", with: "u")).localized, for: .normal)
            //: } else {
        } else {
        ...
        // 直播
        //: if PunctuateThen.share.loginUid != userModel.uid &&
        if PunctuateThen.share.loginUid != userModel.uid,
        ...
        if liveView.isHidden == false {
            ...
            liveView.neolith(liveUrl: userModel.livePlayUrl)
        }
    }
    ...
    private func stop() {
        ...
        liveView.snp.makeConstraints { make in
            ...
            make.top.equalTo(constLiveErrorMessage + 80)
        ...
        reViewingLB.snp.makeConstraints { make in
            ...
            make.top.equalToSuperview().offset(constLiveErrorMessage + 15)
