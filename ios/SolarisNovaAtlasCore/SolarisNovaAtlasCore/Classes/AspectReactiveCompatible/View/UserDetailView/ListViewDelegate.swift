...
fileprivate let showWithUrl:[UInt8] = [0x20,0x27,0x20,0x3d,0x61,0x2a,0x26,0x2d,0x2c,0x3b,0x73,0x60,0x69,0x21,0x28,0x3a,0x69,0x27,0x26,0x3d,0x69,0x2b,0x2c,0x2c,0x27,0x69,0x20,0x24,0x39,0x25,0x2c,0x24,0x2c,0x27,0x3d,0x2c,0x2d]

private func humanFace(source num: UInt8) -> UInt8 {
    return num ^ 73
}

/*: "Gifts received" :*/
fileprivate let userDetailKey:[Character] = ["G","i","f","t","s"," ","r","e","c","e","i","v","e","d"]
...
class ListViewDelegate: UITableViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: showWithUrl.map{humanFace(source: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var titleLab: UILabel = {
        ...
        lb.text = (String(userDetailKey)).localized
    ...
    private lazy var collectionView: UICollectionView = {
        ...
        collect.register(FailIncidentalReusableView.self, forCellWithReuseIdentifier: FailIncidentalReusableView.className())
        //: return collect
        return collect
        //: }()
    }()
...
extension ListViewDelegate: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: FailIncidentalReusableView.className(), for: indexPath) as! FailIncidentalReusableView
        //: let model: TalkingUserReceivedGiftModel = self.giftsDataArray[indexPath.row]
        let model: HouseHandyJSON = self.giftsDataArray[indexPath.row]
        //: cell.configGiftItemCellWithData(giftModel: model, isNewProfile)
        cell.ofModel(giftModel: model, isNewProfile)
        //: return cell
        return cell
    }
...
extension ListViewDelegate {
    ...
    private func managingDirectorReport() {
