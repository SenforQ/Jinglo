...
fileprivate let data_labelName:[UInt8] = [0xb7,0xbc,0xb7,0xc2,0x76,0xb1,0xbd,0xb2,0xb3,0xc0,0x88,0x77,0x6e,0xb6,0xaf,0xc1,0x6e,0xbc,0xbd,0xc2,0x6e,0xb0,0xb3,0xb3,0xbc,0x6e,0xb7,0xbb,0xbe,0xba,0xb3,0xbb,0xb3,0xbc,0xc2,0xb3,0xb2]

fileprivate func managerEnd(button num: UInt8) -> UInt8 {
...
/*: "#333333" :*/
fileprivate let k_viewFormat:[Character] = ["#","3","3","3","3","3","3"]

/*: "#999999" :*/
fileprivate let noti_progressKey:String = "#9999"
fileprivate let show_playerContent:String = "99"

/*: "isCupid" :*/
fileprivate let data_frameValue:String = "equals"
fileprivate let kToData:[Character] = ["C","u","p","i","d"]

/*: "autoGreetAuth" :*/
fileprivate let app_errorFormat:String = "indexto"
fileprivate let main_viewId:String = "request return toGreet"

/*: "isLeaderboardAnonymous" :*/
fileprivate let kLogText:String = "isLeextension to"
fileprivate let userLastUrl:String = "view video main as viewrdAn"

/*: "isSameGendered" :*/
fileprivate let noti_valueMessage:[Character] = ["i"]
fileprivate let appAddContent:String = "beauty color any insSameG"

/*: "type" :*/
fileprivate let data_extendedMessage:String = "TYPE"

/*: "value" :*/
fileprivate let main_endViewText:[Character] = ["v","a","l","u","e"]
...
class VideoThen: UITableViewCell {
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(cookie), name: UIApplication.didBecomeActiveNotification, object: nil)
    }

    //: required init?(coder aDecoder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: data_labelName.map{managerEnd(button: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var titleBLB: UILabel = {
        ...
        lb.textColor = UIColor(hex: (String(k_viewFormat)))
        //: return lb
        return lb
        //: }()
    }()
    ...
    lazy var switchView: UISwitch = {
        ...
        swit.addTarget(self, action: #selector(chemicalPhenomenonShouldAlteration), for: .valueChanged)
        //: return swit
        return swit
        //: }()
    }()
    ...
    lazy var detailLB: TitleClickLabel = {
        //: let lb = TextAlignLb.init()
        let lb = TitleClickLabel()
        ...
        lb.textColor = UIColor(hex: (noti_progressKey.capitalized + show_playerContent.capitalized))
        //: lb.isHidden = true
        lb.isHidden = true
        //: lb.numberOfLines = 0
        lb.numberOfLines = 0
...
extension VideoThen {
    ...
    func matchAdd(titile: String, row: Int, isLast: Bool) {
...
//        if titile == ListGiftTableConvertible.General.rawValue {
//            CustomPermissionTool.checkPushNotification { isShow in
...
//        }
    ...
    func deleteDetail(detail: String) {
    ...
    func exhibitRequestShow(isShow: Int) {
        //: if isShow == 1 {
        if isShow == 1 {
    ...
    @objc func chemicalPhenomenonShouldAlteration() {
        //: if titleBLB.text == SettingsUniversalType.Turnon_Notif.rawValue.localized {
        if titleBLB.text == SwitchicialBidirectionalCollection.Turnon_Notif.rawValue.localized {
        ...
        } else if titleBLB.text == SwitchicialBidirectionalCollection.Cupid.rawValue.localized {
            //: setConfig(type: "isCupid", value: self.switchView.isOn)
            sizeEnable(type: (data_frameValue.replacingOccurrences(of: "equal", with: "i") + String(kToData)), value: self.switchView.isOn)
            //: } else if titleBLB.text == SettingsUniversalType.Automatic.rawValue.localized {
        } else if titleBLB.text == SwitchicialBidirectionalCollection.Automatic.rawValue.localized {
            //: setConfig(type: "autoGreetAuth", value: self.switchView.isOn)
            sizeEnable(type: (app_errorFormat.replacingOccurrences(of: "index", with: "au") + String(main_viewId.suffix(5)) + "Auth"), value: self.switchView.isOn)
            //: } else if titleBLB.text == SettingsUniversalType.Ranking.rawValue.localized {
        } else if titleBLB.text == SwitchicialBidirectionalCollection.Ranking.rawValue.localized {
            //: setConfig(type: "isLeaderboardAnonymous", value: self.switchView.isOn)
            sizeEnable(type: (String(kLogText.prefix(4)) + "aderboa" + String(userLastUrl.suffix(4)) + "onymous"), value: self.switchView.isOn)
            //: } else if titleBLB.text == SettingsUniversalType.Same_gendered.rawValue.localized {
        } else if titleBLB.text == SwitchicialBidirectionalCollection.Same_gendered.rawValue.localized {
            //: setConfig(type: "isSameGendered", value: self.switchView.isOn)
            sizeEnable(type: (String(noti_valueMessage) + String(appAddContent.suffix(6)) + "endered"), value: self.switchView.isOn)
        }
    }

    //: func setConfig(type: String, value: Bool) {
    func sizeEnable(type: String, value: Bool) {
        //: let va = value ? 1 : -1
        let va = value ? 1 : -1
        //: var dict = Dictionary<String, Any>()
        var dict = [String: Any]()
        //: dict["type"] = type
        dict[(data_extendedMessage.lowercased())] = type
        //: dict["value"] = va
        dict[(String(main_endViewText))] = va

        //: ProgressHUD.show()
        DirectThen.levelHandle()
        //: TalkingMeRequestTool.req_SettingChange(params: dict) { succeed, result, errorModel in
        TampReactiveCompatible.first(params: dict) { succeed, _, _ in
            //: ProgressHUD.dismiss()
            DirectThen.cutWith()
            //: if succeed {
            if succeed {
    ...
    @objc func cookie(notification _: NSNotification) {
//        if titleBLB.text == ListGiftTableConvertible.General.rawValue {
//            CustomPermissionTool.checkPushNotification { isShow in
//                DispatchQueue.main.sync {
//                    self.switchView.isOn = isShow
//                }
