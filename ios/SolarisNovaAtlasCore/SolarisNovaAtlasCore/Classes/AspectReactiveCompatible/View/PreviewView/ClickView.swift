...
fileprivate let app_addValue:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "icon_male_default" :*/
fileprivate let noti_cardVideoMessage:[UInt8] = [0x1d,0x17,0x1b,0x1a,0x2b,0x19,0x15,0x18,0x11,0x2b,0x10,0x11,0x12,0x15,0x1,0x18,0x0]

private func universalSize(s num: UInt8) -> UInt8 {
    return num ^ 116
}

/*: "icon_female_default" :*/
fileprivate let app_makeKey:String = "icto"
fileprivate let showTimeMsg:String = "ale_dequal label"
fileprivate let userTitleMsg:String = "player"

/*: "level_%@" :*/
fileprivate let const_rangeNoseUrl:[Character] = ["l","e","v","e","l","_"]
fileprivate let mainArrayId:String = "%@"
...
class ClickView: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: app_addValue.reversed(), encoding: .utf8)!)
    }
    ...
    lazy var nickLabel = UILabel().then {
        ...
        $0.font = .clickLarge(type: .Medium, fontSize: 17)
        //: $0.textColor = .white
        $0.textColor = .white
    }
    ...
    lazy var ageLabel = UILabel().then {
        ...
        $0.font = .clickLarge(type: .Regular, fontSize: 10)
        //: $0.textColor = .white
        $0.textColor = .white
    }
...
extension ClickView {
    ...
    func click(model: PublisherMeasurable) {
        //: headerPic.kf.setImage(with: URL(string: model.headPic), placeholder: UIImage.placeImgSquare())
        headerPic.kf.setImage(with: URL(string: model.headPic), placeholder: UIImage.brother())
        ...
        let imgStr = model.sex == TopComparable.male.rawValue ? String(bytes: noti_cardVideoMessage.map{universalSize(s: $0)}, encoding: .utf8)! : (app_makeKey.replacingOccurrences(of: "to", with: "on") + "_fem" + String(showTimeMsg.prefix(5)) + "efaul" + userTitleMsg.replacingOccurrences(of: "player", with: "t"))
        //: sexImgView.image = UIImage(named: imgStr)
        sexImgView.image = UIImage(named: imgStr)
        //: let levelStr = String(format: "level_%@", model.level)
        let levelStr = String(format: (String(const_rangeNoseUrl) + mainArrayId.capitalized), model.level)
