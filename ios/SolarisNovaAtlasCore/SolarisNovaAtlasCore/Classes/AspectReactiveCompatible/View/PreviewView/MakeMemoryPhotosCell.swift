...
fileprivate let appViewQuoteFormat:[UInt8] = [0xa3,0xa4,0xa3,0xbe,0xe2,0xa9,0xa5,0xae,0xaf,0xb8,0xf0,0xe3,0xea,0xa2,0xab,0xb9,0xea,0xa4,0xa5,0xbe,0xea,0xa8,0xaf,0xaf,0xa4,0xea,0xa3,0xa7,0xba,0xa6,0xaf,0xa7,0xaf,0xa4,0xbe,0xaf,0xae]

private func styleCollection(action num: UInt8) -> UInt8 {
    return num ^ 202
}

/*: "img_loading_lose" :*/
fileprivate let kGiftText:String = "lab case time nameimg_lo"
fileprivate let show_infoData:String = "G"
fileprivate let appTailContent:[Character] = ["_","l","o","s","e"]
...
class MakeMemoryPhotosCell: UICollectionViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: appViewQuoteFormat.map{styleCollection(action: $0)}, encoding: .utf8)!)
    }
...
extension MakeMemoryPhotosCell {
    ...
    private func session() {
        ...
        doubleTap = UITapGestureRecognizer(target: self, action: #selector(videoAction)).then {
...
extension MakeMemoryPhotosCell {
    ...
    func cur(imgModel: FailModelType) {
        ...
        if imgModel.imagePic != nil {
            ...
            imgView.kf.setImage(with: URL(string: imgModel.url), placeholder: UIImage(named: (String(kGiftText.suffix(6)) + "adin" + show_infoData.lowercased() + String(appTailContent))), options: [.keepCurrentImageWhileLoading]) { _ in
                //: self.recoverSubviews()
                self.viewStart()
            }
        }
    ...
    func cagy(imgUrl: String) {
        ...
        imgView.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage(named: (String(kGiftText.suffix(6)) + "adin" + show_infoData.lowercased() + String(appTailContent))), options: [.keepCurrentImageWhileLoading]) { _ in
            //: self.recoverSubviews()
            self.viewStart()
        }
    }
...
extension MakeMemoryPhotosCell: UIScrollViewDelegate, UIGestureRecognizerDelegate {
...
extension MakeMemoryPhotosCell {
    ...
    func viewStart() {
        ...
        var newW = kQueryValue
        //: var newH = ScreenWidth * tempImg!.size.height / tempImg!.size.width
        var newH = kQueryValue * tempImg!.size.height / tempImg!.size.width

        //: if newH > ScreenHeight {
        if newH > notiKeyTitle {
            //: newH = ScreenHeight
            newH = notiKeyTitle
            //: newW = tempImg!.size.width * ScreenHeight / tempImg!.size.height
            newW = tempImg!.size.width * notiKeyTitle / tempImg!.size.height
        }
        ...
        self.imgContainerView.origin = CGPoint(x: (kQueryValue - newW) / 2.0, y: (notiKeyTitle - newH) / 2)
