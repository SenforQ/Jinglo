...
class TopViewController: SizeViewController {
    ...
    override func viewDidLoad() {
        ...
        self.makeStatusConstraint()
    }
    
    // MARK: - Lazy load
    //: lazy var mainView: TalkingFinalVerificationView = {
    lazy var mainView: WeddingPictureThen = {
        //: let view = TalkingFinalVerificationView()
        let view = WeddingPictureThen()
        //: return view
        return view
    //: }()
    }()
...
extension TopViewController {
    ...
    override func untilNavi() {
        ...
        for index in 0..<(self.navigationController?.viewControllers.count)! {
            ...
            if self.navigationController?.viewControllers[index] .isKind(of: DrunkReactiveCompatible.self) == true {
                //: let vc = (self.navigationController?.viewControllers[index] as? TalkingLoginEditAuthAndVideoVC)!
                let vc = (self.navigationController?.viewControllers[index] as? DrunkReactiveCompatible)!
