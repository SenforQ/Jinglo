...
fileprivate let kBladeErrorTurnUrl:String = "icon_kby cell click"
fileprivate let appHockeyId:String = "tong"
fileprivate let userDataMessage:String = "moonlt"

/*: "page" :*/
fileprivate let appTransitionRegionContentMsg:[UInt8] = [0x65,0x67,0x61,0x70]

/*: "uid" :*/
fileprivate let mainViewMessage:[UInt8] = [0x64,0x69,0x75]

/*: "list" :*/
fileprivate let userNeverUrl:String = "arrayist"
...
class ToolViewController: SizeViewController {
    //: private var pageIndex = 0
    private var pageIndex = 0
    //: private var postDataArr = [TalkingUserInfoMomentItemModel]()
    private var postDataArr = [RequestTransformable]()
    ...
    private lazy var collectionView: UICollectionView = {
        ...
        collectionView.contentInset = UIEdgeInsets(top: 0, left: 0, bottom: constDomainUrl, right: 0)
        //: collectionView.register(TalkingPostItemCell.self, forCellWithReuseIdentifier: TalkingPostItemCell.className())
        collectionView.register(LineItemMakeViewCell.self, forCellWithReuseIdentifier: LineItemMakeViewCell.className())
    ...
    private lazy var emptyView: UIView = {
        ...
        img.image = UIImage.thoughtImageBundleSizeText(name: (String(kBladeErrorTurnUrl.prefix(6)) + "ong_" + appHockeyId.replacingOccurrences(of: "to", with: "ko") + "_def" + userDataMessage.replacingOccurrences(of: "moon", with: "au")))
...
extension ToolViewController {
    //: private func headerRefresh() {
    private func lineAcross() {
        //: pageIndex = 0
        pageIndex = 0
        //: load_listData()
        pointData()
    ...
    private func pointData() {
        ...
        let params = [String(bytes: appTransitionRegionContentMsg.reversed(), encoding: .utf8)!: pageIndex, String(bytes: mainViewMessage.reversed(), encoding: .utf8)!: userInfoModel.uid] as [String: Any]
        //: TalkingMomentRequestTool.req_UserMomentList(params: params) { [weak self] succeed, result, errorModel in
        StartReactiveCompatible.popOut(params: params) { [weak self] succeed, result, _ in
            //: guard let self = self else { return }
            guard let self = self else { return }
            ...
            if let resultArr = resultDict[(userNeverUrl.replacingOccurrences(of: "array", with: "l"))] as? [Any] {
                ...
                for dict in resultArr {
                    ...
                    if let model = ViewMeasurable.deserialize(from: dict as? Dictionary) {
...
extension ToolViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: LineItemMakeViewCell.className(), for: indexPath) as! LineItemMakeViewCell
        //: if indexPath.row < self.postDataArr.count {
        if indexPath.row < self.postDataArr.count {
            //: let model = self.postDataArr[indexPath.row]
            let model = self.postDataArr[indexPath.row]
            //: cell.refreshItem(model)
            cell.assemblageMake(model)
        }
    ...
    func collectionView(_: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        let vc = MomentOfTruthFullMoonVerityViewController(model: detailModel)
...
extension ToolViewController: JXPagingViewListViewDelegate {
...
extension ToolViewController {
    ...
    func endUserMedium(_ userModel: AuditoryImageMeasurable) {
