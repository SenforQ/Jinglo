...
/*: "Game Center" :*/
fileprivate let constWhiteValue:[Character] = ["G","a","m","e"," ","C","e","n","t","e","r"]

/*: "You've got no list yet." :*/
fileprivate let main_pointName:[Character] = ["Y","o","u","\'","v","e"," ","g","o","t"," ","n","o"," ","l","i","s","t"," "]
fileprivate let main_logData:String = "text label text equalyet."

/*: "list" :*/
fileprivate let dataRecordKey:String = "lisclick"
...
class StraddleViewController: SizeViewController {
    //: override func viewDidLoad() {
    override func viewDidLoad() {
        //: super.viewDidLoad()
        super.viewDidLoad()
        //: self.title = "Game Center".localized
        self.title = (String(constWhiteValue)).localized
        //: setupSubviews()
        wallop()
    ...
    private lazy var emptyView: EmptyView = {
        ...
        style.TipsTitle = (String(main_pointName) + String(main_logData.suffix(4))).localized
...
extension StraddleViewController {
    /// 获取游戏列表
    ...
    private func logList() {
        ...
        TampReactiveCompatible.bringCompletion(category: 1) { [weak self] _, result, _ in
            //: ProgressHUD.dismiss()
            DirectThen.cutWith()
            ...
            json[(dataRecordKey.replacingOccurrences(of: "click", with: "t"))].arrayObject?.forEach { dict in
...
extension StraddleViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.application(model: dataArr[indexPath.row], nameColor: .toEvent())
        //: return cell
        return cell
    }
