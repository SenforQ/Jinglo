...
fileprivate let notiCellUrl:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "btn_me_edit" :*/
fileprivate let k_managerName:[Character] = ["b","t","n","_","m","e","_"]
fileprivate let main_hourValue:String = "erequestit"

/*: "icon_me_lw" :*/
fileprivate let user_pageValue:[Character] = ["i","c","o","n","_","m","e","_","l"]
fileprivate let mainToSocialKey:[Character] = ["w"]

/*: " Reward" :*/
fileprivate let userFileTitle:String = " Rewabirth let report for"
fileprivate let notiToId:String = "rview"

/*: "FF506D" :*/
fileprivate let noti_inputPath:[Character] = ["F","F","5","0","6","D"]
...
// __CLOSE_PRINT__
//
//  PassePartoutReactiveCompatible.swift
//  AbroadTalking
//
...
class PassePartoutReactiveCompatible: UITableViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: notiCellUrl.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var editImag: UIImageView = {
        ...
        let imag = UIImageView(image: UIImage.thoughtImageBundleSizeText(name: (String(k_managerName) + main_hourValue.replacingOccurrences(of: "request", with: "d"))))
        //: return imag
        return imag
        //: }()
    }()
    ...
    private lazy var taskCenterBtn: UIButton = {
        ...
        btn.setImage(UIImage.thoughtImageBundleSizeText(name: (String(user_pageValue) + String(mainToSocialKey))), for: .normal)
        //: btn.setTitle(" Reward".localized, for: .normal)
        btn.setTitle((String(userFileTitle.prefix(5)) + notiToId.replacingOccurrences(of: "view", with: "d")).localized, for: .normal)
        //: btn.setTitleColor(UIColor.init(hex: "FF506D"), for: .normal)
        btn.setTitleColor(UIColor(hex: (String(noti_inputPath))), for: .normal)
...
extension PassePartoutReactiveCompatible {
    //: func setViewData(title: String, icon: String, row: Int, isLast: Bool) {
    func bindGold(title: String, icon: String, row: Int, isLast: Bool) {
        //: titleBLB.text = title.localized
        titleBLB.text = title.localized
        //: iconImag.image = UIImage.BundleImageNamed(name: icon)
        iconImag.image = UIImage.thoughtImageBundleSizeText(name: icon)
        //: index = row
        index = row
        //: islast = isLast
        islast = isLast
...
extension PassePartoutReactiveCompatible {
    ...
    private func appConstraint() {
        ...
        backView.frame = CGRect(x: 15, y: 0, width: kQueryValue - 30, height: backView_h)
        ...
        if index == 0, islast {
            ...
            backView.areaRemoveRadiiAt(width: backView.frame.size.width,
