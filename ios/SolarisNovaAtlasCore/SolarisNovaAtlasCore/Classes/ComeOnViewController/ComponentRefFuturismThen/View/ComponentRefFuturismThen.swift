...
fileprivate let showMethodData:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "img_home_shadow" :*/
fileprivate let dataPlayerTitle:[Character] = ["i","m","g","_","h","o","m"]
fileprivate let showMomentQuoteTitle:[Character] = ["e","_","s","h","a","d","o","w"]

/*: "#9610FF" :*/
fileprivate let appContactMsg:[Character] = ["#","9","6","1","0","F","F"]

/*: "#8566FF" :*/
fileprivate let mainColorViewMsg:String = "#8566FFframe rank in time"

/*: "Random Video" :*/
fileprivate let userEffectTitle:String = "make selection send fatalRa"
fileprivate let notiBarData:String = "ndoview"

/*: "icon_match_call_free" :*/
fileprivate let data_clickPlayerCardId:String = "off to title modelicon_"
fileprivate let constLanguagePath:String = "macolor"
fileprivate let mainBackgroundText:[Character] = ["h","_","c","a","l","l","_","f","r","e","e"]

/*: "Free" :*/
fileprivate let kOfTitle:String = "var input requestFree"

/*: "Scroll_upto_discove" :*/
fileprivate let dataModePath:String = "var varScro"
fileprivate let notiShareMsg:String = "o_disslide var make gesture manager"
fileprivate let noti_pathStr:String = "copage"

/*: "icon_VectorStroke" :*/
fileprivate let dataStyleMsg:String = "inameon"
fileprivate let data_scaleStr:String = "orStrokekey data image"

/*: "get json error" :*/
fileprivate let dataHiddenStr:[Character] = ["g","e","t"," ","j","s","o","n"," "]
fileprivate let dataLayerMessage:[Character] = ["e","r","r","o","r"]
...
// __CLOSE_PRINT__
//
//  ComponentRefFuturismThen.swift
//  AbroadTalking
//
...
class ComponentRefFuturismThen: UIView {
    ...
    var maxRemoveDistance = kQueryValue / 4.0 /// 最大移除距离(默认屏幕的1/4)
    //: var reusableCells = [TalkingClubCardViewCell]()
    var reusableCells = [RangeEqualView]() /// 重用卡片数组
    //: var isAlpha = true
    ...
    var dataSource: MessageObjectProtocol? /// 数据源
    //: var delegate: TalkingClubCardViewDelegate?
    var delegate: DoObjectProtocol?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: showMethodData.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var leftbackImageView: UIImageView = {
        ...
        imag.image = UIImage.thoughtImageBundleSizeText(name: (String(dataPlayerTitle) + String(showMomentQuoteTitle)))
    ...
    private lazy var randomBtn: UIButton = {
        ...
        btn.setBackgroundImage(UIImage.sinceNext(colors: [UIColor(hex: (String(appContactMsg)))!.cgColor, UIColor(hex: (String(mainColorViewMsg.prefix(7))))!.cgColor], size: CGSize(width: kQueryValue - 30, height: 54)), for: .normal)
        //: btn.layer.cornerRadius = 27
        btn.layer.cornerRadius = 27
        //: btn.layer.masksToBounds = true
        btn.layer.masksToBounds = true
        //: btn.setTitle("Random Video".localized, for: .normal)
        btn.setTitle((String(userEffectTitle.suffix(2)) + notiBarData.replacingOccurrences(of: "view", with: "m") + " Video").localized, for: .normal)
        //: btn.addTarget(self, action: #selector(randomBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(imageTitle), for: .touchUpInside)
        //: self.addSubview(btn)
        self.addSubview(btn)
    ...
    lazy var freeImag: UIButton = {
        ...
        img.setBackgroundImage(UIImage.thoughtImageBundleSizeText(name: (String(data_clickPlayerCardId.suffix(5)) + constLanguagePath.replacingOccurrences(of: "color", with: "tc") + String(mainBackgroundText))), for: .normal)
        //: img.setTitle("Free".localized, for: .normal)
        img.setTitle((String(kOfTitle.suffix(4))).localized, for: .normal)
...
extension ComponentRefFuturismThen {
    //: func dismissGuide() {
    func belowData() {
        //: if guideView != nil {
        if guideView != nil {
    ...
    private func mePage() {
        ...
        image.image = UIImage.thoughtImageBundleSizeText(name: (String(dataModePath.suffix(4)) + "ll_upt" + String(notiShareMsg.prefix(5)) + noti_pathStr.replacingOccurrences(of: "page", with: "ve")))
        ...
        image2.image = UIImage.thoughtImageBundleSizeText(name: (dataStyleMsg.replacingOccurrences(of: "name", with: "c") + "_Vect" + String(data_scaleStr.prefix(8))))
        ...
        frameSocial(player: player)
    }

    //: private func likeplayer(player: SVGAPlayer) {
    private func frameSocial(player: SVGAPlayer) {
        //: do {
        do {
            //: let url = SVGAEffectTool.default.getZipEffectPath(type: .Club_guidePage)
            let url = SkullSessionReactiveCompatible.default.region(type: .Club_guidePage)
            ...
            printLog(message: (String(dataHiddenStr) + String(dataLayerMessage)))
        }
    }

    //: @objc private func randomBtnClick() {
    @objc private func imageTitle() {
        //: self.delegate?.clubCardRandowBtnClick?()
        self.delegate?.baseballLeagueWillClick?()
    }

    //: func reloadDataAnimated(anim: Bool) {
    func selectCard(anim: Bool) {
        //: moveCount = 0
        moveCount = 0
        //: self.currentIndex = 0
        self.currentIndex = 0
        ...
        let maxCount = dataSource?.showView(cardView: self) ?? 0
        ...
        progressNeed(anim: anim)
    }

    /** 创建新的cell */
    //: func createCardViewCell(index: Int) {
    func viewSection(index: Int) {
        //: let cell = dataSource?.cardViewCellForRowAtIndex(cardView: self, index: index)
        let cell = dataSource?.bagAcrossVideo(cardView: self, index: index)
...
//        let showCount = Double(self.visibleCount - 1)
        ...
        containerView.insertSubview(cell ?? RangeEqualView(), at: 0)
    ...
    func progressNeed(anim: Bool) {
        ...
        for i in 0 ... count - 1 {
            ...
            let cell: RangeEqualView = self.visibleCells[i] as! RangeEqualView
            ...
            if isVisbleLast {
                ...
                self.delegate?.voiceOnAt?(cardViewCell: cell, forRowAtIndex: self.currentIndex - i)
            }

            //: if anim {
            if anim {
                //: updateConstraintsCell(cell: cell, transform: transform)
                cellPathUser(cell: cell, transform: transform)
                //: } else {
            } else {
    ...
    func cellPathUser(cell: RangeEqualView, transform: CGAffineTransform) {
    ...
    func toIdentifier(cellClass: AnyClass, forellIdentifier: String) {
    ...
    func talkStart(identifier: String) -> RangeEqualView? {
        ...
        if self.cellClass != nil {
            ...
            let cellclass = self.cellClass as! CountimateReactiveCompatible.Type
            //: let cell: TalkingClubCardItemCell = cellclass.init(reuseIdentifier: identifier )
            let cell: CountimateReactiveCompatible = cellclass.init(reuseIdentifier: identifier)
    ...
    func compartmentByIndex(index: Int) -> RangeEqualView {
        //: let visibleIndex = visibleIndex(inde: index)
        let visibleIndex = toolInde(inde: index)
        //: let cell = self.visibleCells[visibleIndex]
        let cell = self.visibleCells[visibleIndex]
        //: return cell as! TalkingClubCardViewCell
        return cell as! RangeEqualView
    }

    /// 获取cell对应的index
    //: func indexForCell(cell: TalkingClubCardViewCell) -> Int {
    func edgeFullMoonFoulUpCellCompartmentQuality(cell: RangeEqualView) -> Int {
        //: let arr = self.visibleCells as? [TalkingClubCardViewCell]
        let arr = self.visibleCells as? [RangeEqualView]
...
extension ComponentRefFuturismThen: CellObjectProtocol {
    ...
    func successfully(cell: RangeEqualView) {
        ...
        self.delegate?.targetSend?(cardView: self, cardViewCell: cell, forRowAtIndex: self.currentFirstIndex - 1)

        //: let count = self.dataSource?.numberOfCountInCardView(cardView: self) ?? 0
        let count = self.dataSource?.showView(cardView: self) ?? 0
        // 移除后的卡片是最后一张(没有更多)
        ...
        if self.visibleCells.count == 0 { // 只有最后一张卡片的时候
            ...
            self.delegate?.sawSetAdd?(cardViewCell: cell, forRowAtIndex: self.currentIndex)
            //: return
            return
        }
        // 当前数据源还有数据 继续创建cell
        ...
        if self.currentIndex < count - 1 {
            ...
            viewSection(index: self.currentIndex + 1)
        }
        // 更新布局
        //: updateLayoutVisibleCells(anim: true)
        progressNeed(anim: true)
    }
    ...
    func matchPoint(cell: RangeEqualView, forMovePoint: CGPoint) {
        ...
        self.delegate?.sura?(cardView: self, cardViewCell: cell, forMovePoint: forMovePoint)
    }
}
...
extension ComponentRefFuturismThen {
    ...
    func allButton() {
        ...
        let index = ColorHashableRepresentation.randow
