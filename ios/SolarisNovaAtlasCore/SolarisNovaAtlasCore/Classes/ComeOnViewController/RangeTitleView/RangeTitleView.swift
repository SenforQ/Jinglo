...
fileprivate let mainMessageStr:[UInt8] = [0xd0,0xd7,0xd0,0xcd,0x91,0xda,0xd6,0xdd,0xdc,0xcb,0x83,0x90,0x99,0xd1,0xd8,0xca,0x99,0xd7,0xd6,0xcd,0x99,0xdb,0xdc,0xdc,0xd7,0x99,0xd0,0xd4,0xc9,0xd5,0xdc,0xd4,0xdc,0xd7,0xcd,0xdc,0xdd]

private func typeGreet(label num: UInt8) -> UInt8 {
    return num ^ 185
}

/*: "gift_bg_dm" :*/
fileprivate let appDirectionUrl:[Character] = ["g","i","f","t","_","b","g","_","d","m"]

/*: "#FFF4B5" :*/
fileprivate let notiModelExceptTitle:String = "in make interval info frame#FFF4B5"

/*: "won " :*/
fileprivate let show_touchName:[Character] = ["w","o","n"," "]

/*: " coins in " :*/
fileprivate let mainMatchFormat:[Character] = [" ","c","o","i","n"]
fileprivate let user_domainAlbumKey:String = "s in if translate if pause"

/*: " x :*/
fileprivate let main_contentId:String = "else false x"

/*: "just sent" :*/
fileprivate let dataFrameStr:String = "equalust"
fileprivate let k_leadingContent:String = "handle"

/*: "  :*/
fileprivate let mainEnterItemMessage:[Character] = [" "]

/*: "#4120C1" :*/
fileprivate let constLocalValue:[Character] = ["#","4","1","2","0","C","1"]
...
class RangeTitleView: UIView {
    /// 礼物model
    //: private var giftModel = SocialBroadcastModel()
    private var giftModel = DataReactiveCompatible()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: mainMessageStr.map{typeGreet(label: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var bgView: UIImageView = {
        ...
        v.image = UIImage.thoughtImageBundleSizeText(name: (String(appDirectionUrl)))
    ...
    private lazy var icon: UIImageView = {
        ...
        v.layer.borderColor = UIColor(hex: (String(notiModelExceptTitle.suffix(7))))?.cgColor
    ...
    private lazy var nicknameLab: UILabel = {
        ...
        lab.textColor = UIColor(hex: (String(notiModelExceptTitle.suffix(7))))
...
extension RangeTitleView {
    /// 刷新视图
    //: func refreshGift(_ model: SocialBroadcastModel) {
    func refreshToLargess(_ model: DataReactiveCompatible) {
        //: giftModel = model
        giftModel = model
        //: icon.setUrlImage(urlStr: giftModel.headPic)
        icon.putBackThroughFinish(urlStr: giftModel.headPic)
        ...
        if giftModel.broadStatus == 1 { // 中奖
            ...
            contentLab.text = (String(show_touchName)).localized + "\(giftModel.totalCoin)" + (String(mainMatchFormat) + String(user_domainAlbumKey.prefix(5))).localized + giftModel.gameName
            ...
            giftImgV.putBackThroughFinish(urlStr: giftModel.img)
            //: giftNumLab.isHidden = false
            giftNumLab.isHidden = false
            //: giftNumLab.text = " x\(giftModel.num)"
            giftNumLab.text = " x\(giftModel.num)"
            //: let rangeText = "just sent".localized
            let rangeText = (dataFrameStr.replacingOccurrences(of: "equal", with: "j") + " sen" + k_leadingContent.replacingOccurrences(of: "handle", with: "t")).localized
            //: let giftName = changeGiftName(giftmodel: giftModel)
            let giftName = strongModel(giftmodel: giftModel)
            ...
            if let range = fullText.range(of: rangeText) {
                ...
                attributedString.addAttribute(.foregroundColor, value: UIColor(hex: (String(constLocalValue)))!, range: nsRange)
            }
    ...
    private func strongModel(giftmodel: DataReactiveCompatible) -> String {
...
extension RangeTitleView {
    ...
    private func economicAid() {
        ...
        self.frame = CGRect(x: 0, y: mainKeyValue, width: kQueryValue, height: 60)
