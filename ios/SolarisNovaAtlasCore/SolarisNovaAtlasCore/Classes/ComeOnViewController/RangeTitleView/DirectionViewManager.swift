...
/*: "position.x" :*/
fileprivate let app_instanceUrl:String = "pvaluesit"
fileprivate let show_labVideoId:[Character] = ["x"]

/*: "slideIn" :*/
fileprivate let const_pathNameTitle:String = "slistope"
fileprivate let showExtraName:String = "Inview height instance path"

/*: "slideOut" :*/
fileprivate let noti_playKey:String = "slitarget"
...
class DirectionViewManager: NSObject {
    //: static let shared = GiftFloatViewManager()
    static let shared = DirectionViewManager()
    //: private var giftArr = [SocialBroadcastModel]()
    private var giftArr = [DataReactiveCompatible]()
    ...
    private lazy var giftView: RangeTitleView = {
        //: let v = GiftFloatView()
        let v = RangeTitleView()
        //: PublisherReactiveCompatible.getWindow().addSubview(v)
        PublisherReactiveCompatible.exceptAction().addSubview(v)
        //: return v
        return v
        //: }()
    }()
...
extension DirectionViewManager {
    /// 添加礼物
    //: private func addGift(gift: SocialBroadcastModel) {
    private func userGift(gift: DataReactiveCompatible) {
        //: self.synchronized(self) {
        self.visibleChannel(self) {
            //: self.giftArr.append(gift)
            self.giftArr.append(gift)
            //: self.showNextGift(index: 0)
            self.nameIndex(index: 0)
        }
    }

    //: public func synchronized<T>(_ lock: AnyObject, _ body: () throws -> T) rethrows -> T {
    public func visibleChannel<T>(_ lock: AnyObject, _ body: () throws -> T) rethrows -> T {
    ...
    private func affect(_ model: DataReactiveCompatible) {
        ...
        let isRTL = MakeThen.shared.direction == .rightToLeft
        // 起始位置：屏幕外
        //: let startCenterX = isRTL ? -centerX : ScreenWidth + centerX
        let startCenterX = isRTL ? -centerX : kQueryValue + centerX
        // 目标位置：屏幕中间
        //: let targetCenterX = ScreenWidth / 2
        let targetCenterX = kQueryValue / 2
        // 结束位置：屏幕另一侧
        //: let endCenterX = isRTL ? ScreenWidth + centerX : -centerX
        let endCenterX = isRTL ? kQueryValue + centerX : -centerX
        ...
        let slideIn = CABasicAnimation(keyPath: (app_instanceUrl.replacingOccurrences(of: "value", with: "o") + "ion." + String(show_labVideoId)))
        ...
        self.giftView.layer.add(slideIn, forKey: (const_pathNameTitle.replacingOccurrences(of: "stop", with: "d") + String(showExtraName.prefix(2))))
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 3.0) { [weak self] in
            ...
            let slideOut = CABasicAnimation(keyPath: (app_instanceUrl.replacingOccurrences(of: "value", with: "o") + "ion." + String(show_labVideoId)))
            ...
            self.giftView.layer.add(slideOut, forKey: (noti_playKey.replacingOccurrences(of: "target", with: "d") + "eOut"))
...
extension DirectionViewManager {
    ...
    func popBy() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(self.attributeErase(noti:)),
                                               //: name: USER_RECIEVED_GIFT_NOTIFICATION,
                                               name: constBarContent,
                                               //: object: nil)
                                               object: nil)
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(self.detailBackground),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(self.generate),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(self.asBlock),
    ...
    @objc private func attributeErase(noti: Notification) {
        ...
        guard let data = noti.object,
              ...
              let gift = JSONDeserializer<DataReactiveCompatible>.deserializeFrom(dict: data as? [String: Any], designatedPath: nil),
              //: gift.broadType != BroadcastType.Unknow.rawValue
              gift.broadType != GetStartedHashableRepresentation.Unknow.rawValue
        //: else {
        else {
            //: return
            return
        }
        //: let currUserType = PunctuateThen.share.appUserConfigMode.userType
        let currUserType = PunctuateThen.share.appUserConfigMode.userType
        // 过滤接口异常
        ...
        // 非特殊用户 不展示 不同地区用户礼物
        //: if currUserType != 0, gift.userCountryType != PunctuateThen.share.appUserConfigMode.userCountryType {
        if currUserType != 0, gift.userCountryType != PunctuateThen.share.appUserConfigMode.userCountryType {
            //: return
            return
        }
