...
/*: "%@%@" :*/
fileprivate let showPanStr:[Character] = ["%","@","%","@"]

/*: "/TXCache" :*/
fileprivate let noti_labMsg:[Character] = ["/","T","X","C","a","c"]
fileprivate let k_intimateMsg:String = "hlayer"
...
// __CLOSE_PRINT__
//
//  MemoryReactiveCompatible.swift
//  AbroadTalking
//
...
protocol StartRecordThen: NSObject {
    //: func func__playerStatus(player: TalkingVideoPlayerManager, status: VideoPlayerStatus)
    func chessPlayer(player: MemoryReactiveCompatible, status: ToPlayerStatus)

    //: func func__playerDuration(player: TalkingVideoPlayerManager, duration: Int, currentTime: Int)
    func linksmanInput(player: MemoryReactiveCompatible, duration: Int, currentTime: Int)

    //: func func__playerProgress(player: TalkingVideoPlayerManager, progress: CGFloat)
    func assumedTitleView(player: MemoryReactiveCompatible, progress: CGFloat)
}

//: class TalkingVideoPlayerManager: NSObject {
class MemoryReactiveCompatible: NSObject {
    //: var isPlaying = false
    var isPlaying = false
    //: var isMute = false
    var isMute = false
    ...
    open weak var delegate: StartRecordThen?
    //: var playStatus: VideoPlayerStatus?
    var playStatus: ToPlayerStatus?
    //: static let shared = TalkingVideoPlayerManager()
    static let shared = MemoryReactiveCompatible()
    ...
    lazy var player: TXVodPlayer = {
        ...
        let cachePath = NSString(format: "%@%@", NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first!, (String(noti_labMsg) + k_intimateMsg.replacingOccurrences(of: "layer", with: "e"))) as String
    ...
    lazy var timer: Timer = {
        ...
        let timer = Timer.scheduledTimer(timeInterval: 0.25, target: self, selector: #selector(popular), userInfo: nil, repeats: true)
...
extension MemoryReactiveCompatible {
    //: func playerWithUrlAndVideoView(url: String, view: UIView) {
    func asDrunk(url: String, view: UIView) {
        //: let newUrl = url.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: ReplaceUrlDomain)
        let newUrl = url.replacingOccurrences(of: const_maxFormalText.recordBy(), with: k_detailData)
    ...
    @objc func popular() {
        ...
        if duration() > 0 {
            ...
            self.delegate?.assumedTitleView(player: self, progress: CGFloat(currentTime() / duration()))
        }
    }

    //: func setRenderMode(renderMode: EnumTypeRenderMode) {
    func deleteToMode(renderMode: WithRenderMode) {
    ...
    func valueDoing(bEnable: Bool) {
        //: var isMute = bEnable
        var isMute = bEnable
        //: if TalkingSocketManager.shared.workSpace == true {
        if WrittenRecordSocketDelegate.shared.workSpace == true { // 音视频通话时静音
            //: isMute = true
            isMute = true
        }
    ...
    func run() {
        ...
        if self.playStatus != .Pausing {
            ...
            self.delegate?.chessPlayer(player: self, status: self.playStatus!)
        }
    }

    //: func isPlayings() -> Bool {
    func aggregationPlayings() -> Bool {
...
extension MemoryReactiveCompatible: TXVodPlayListener {
    ...
    func onPlayEvent(_: TXVodPlayer!, event EvtID: Int32, withParam _: [AnyHashable: Any]!) {
        ...
        case PLAY_EVT_PLAY_PROGRESS.rawValue:
            ...
            self.delegate?.linksmanInput(player: self, duration: Int(duration), currentTime: Int(currentTime))
