...
fileprivate let main_hiddenData:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "#4FAAFF" :*/
fileprivate let dataBallSocialMainName:String = "#jump"
fileprivate let notiStateData:String = "soundAAsoundsound"

/*: "reply" :*/
fileprivate let kBladeId:[Character] = ["r","e","p","l","y"]

/*: "： :*/
fileprivate let mainModelName:String = "："

/*: "F5F5F5" :*/
fileprivate let app_sharedNameUrl:String = "valuevaluevalue"
...
class MotherView: UITableViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: main_hiddenData.reversed(), encoding: .utf8)!)
    }
...
extension MotherView {
    //: @objc func onLongTapCell(sender: UIGestureRecognizer) {
    @objc func willSender(sender _: UIGestureRecognizer) {}
}

//: extension TalkingCommentReplyItemCell {
extension MotherView {
    //: func configCell(model: TalkingCommentReplyModel) {
    func limitCompartment(model: PanModelType) {
        //: if model.allCountContent.count>0 {
        if model.allCountContent.count > 0 {
            //: contentLB.textColor = UIColor.init(hex: "#4FAAFF")
            contentLB.textColor = UIColor(hex: (dataBallSocialMainName.replacingOccurrences(of: "jump", with: "4") + notiStateData.replacingOccurrences(of: "sound", with: "F")))
            ...
            if model.replyUser.count > 0 {
                ...
                colorA = [UIColor.hockeyPlayer(), UIColor.toEvent()]
                //: fontA = [font, font]
                fontA = [font, font]
            }
    ...
    func bombUp(cell: UITableViewCell, tableView: UITableView, indexPath: NSIndexPath, radius: CGFloat, height _: CGFloat) {
        ...
        let bounds = CGRect(x: 51, y: 0, width: kQueryValue - 51 - 15, height: cell.bounds.size.height)
        ...
        layer.fillColor = UIColor(hex: (app_sharedNameUrl.replacingOccurrences(of: "value", with: "F5")))?.cgColor
...
extension MotherView {
    ...
    private func no() {
        ...
        let longtap = UILongPressGestureRecognizer(target: self, action: #selector(willSender(sender:)))
