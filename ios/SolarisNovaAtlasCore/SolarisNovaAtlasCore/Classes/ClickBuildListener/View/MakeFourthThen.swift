...
fileprivate let const_labelData:[UInt8] = [0xca,0xcd,0xca,0xd7,0x8b,0xc0,0xcc,0xc7,0xc6,0xd1,0x99,0x8a,0x83,0xcb,0xc2,0xd0,0x83,0xcd,0xcc,0xd7,0x83,0xc1,0xc6,0xc6,0xcd,0x83,0xca,0xce,0xd3,0xcf,0xc6,0xce,0xc6,0xcd,0xd7,0xc6,0xc7]

private func momentBottom(make num: UInt8) -> UInt8 {
    return num ^ 163
}
...
class MakeFourthThen: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: const_labelData.map{momentBottom(make: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var progressView: AvailableThen = {
        ...
        let v = AvailableThen(frame: self.frame, progressBgColor: self.progressBgColor, progressPlayedColor: self.progressPlayedColor, progressCachedColor: self.progressCachedColor)
    ...
    private lazy var panGesture: UIPanGestureRecognizer = {
        ...
        let v = UIPanGestureRecognizer(target: self, action: #selector(dataBy(gesture:)))
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var tapGesture: UIPanGestureRecognizer = {
        ...
        let v = UIPanGestureRecognizer(target: self, action: #selector(pick(gesture:)))
        //: return v
        return v
        //: }()
    }()
...
extension MakeFourthThen {
    //: @objc func panGesture(gesture: UIPanGestureRecognizer) {
    @objc func dataBy(gesture: UIPanGestureRecognizer) {
        // 移动的距离
    ...
    @objc func pick(gesture: UIPanGestureRecognizer) {
