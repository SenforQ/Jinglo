...
fileprivate let main_detailStr:[UInt8] = [0x55,0x52,0x55,0x48,0x14,0x5f,0x53,0x58,0x59,0x4e,0x6,0x15,0x1c,0x54,0x5d,0x4f,0x1c,0x52,0x53,0x48,0x1c,0x5e,0x59,0x59,0x52,0x1c,0x55,0x51,0x4c,0x50,0x59,0x51,0x59,0x52,0x48,0x59,0x58]
...
class ActionReusableView: UICollectionViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: main_detailStr.map{$0^60}, encoding: .utf8)!)
    }

    //: func customUI() {
    func revenue() {
        //: let tap = UITapGestureRecognizer(target: self, action: #selector(imageViewClick))
        let tap = UITapGestureRecognizer(target: self, action: #selector(popularItem))
        ...
        let doubletap = UITapGestureRecognizer(target: self, action: #selector(toggleAction))
...
extension ActionReusableView: UIScrollViewDelegate {
    //: @objc func imageViewClick(tap: UIGestureRecognizer) {
    @objc func popularItem(tap _: UIGestureRecognizer) {
        //: if sblock != nil {
        if sblock != nil {
            //: sblock()
            sblock()
    ...
    @objc func toggleAction(tap: UIGestureRecognizer) {
    ...
    func borderline(url: String) {
        //: imageView.setUrlImage(urlStr: url)
        imageView.putBackThroughFinish(urlStr: url)
    }

    //: func configImage(image: UIImage) {
    func conversationFor(image: UIImage) {
