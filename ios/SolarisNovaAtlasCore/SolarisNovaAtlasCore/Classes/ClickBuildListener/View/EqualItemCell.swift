...
fileprivate let main_equalMessage:[UInt8] = [0xfc,0xfb,0xfc,0xe1,0xbd,0xf6,0xfa,0xf1,0xf0,0xe7,0xaf,0xbc,0xb5,0xfd,0xf4,0xe6,0xb5,0xfb,0xfa,0xe1,0xb5,0xf7,0xf0,0xf0,0xfb,0xb5,0xfc,0xf8,0xe5,0xf9,0xf0,0xf8,0xf0,0xfb,0xe1,0xf0,0xf1]

private func shouldFile(post num: UInt8) -> UInt8 {
    return num ^ 149
}

/*: "F5F5F5" :*/
fileprivate let dataDocumentMsg:String = "view"
fileprivate let user_withPath:String = "5F5F5succeed add normal"
...
class EqualItemCell: UITableViewCell {
    ...
    var videoView = TopReactiveCompatible()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: main_equalMessage.map{shouldFile(post: $0)}, encoding: .utf8)!)
    }
    ...
    func backgroundFor(isListTableCell: Bool) {
    ...
    lazy var userInfoView: FileInfoView = {
    ...
    lazy var textContentView: ViewReactiveCompatible = {
    ...
    lazy var photoView: SizeDataSource = {
        ...
        let view = SizeDataSource(frame: CGRect(x: 0, y: 0, width: 0, height: 0), collectionViewLayout: flayout)
        //: backView.addSubview(view)
        backView.addSubview(view)
        //: return view
        return view
    ...
    lazy var bottomView: DigitiserBottomView = {
        //: let  view = TalkingMomentItemBottomView.init()
        let view = DigitiserBottomView()
        //: backView.addSubview(view)
        backView.addSubview(view)
        //: return view
        return view
    ...
    lazy var lineView: UIView = {
        ...
        view.backgroundColor = UIColor(hex: (dataDocumentMsg.replacingOccurrences(of: "view", with: "F") + String(user_withPath.prefix(5))))
        //: self.contentView.addSubview(view)
        self.contentView.addSubview(view)
        //: return view
        return view
...
extension EqualItemCell {
    ...
    func afterTarget(model: ViewMeasurable) {
        //: cunrrenModel = model
        cunrrenModel = model
        //: userInfoView.isHidden = false
        userInfoView.isHidden = false
        //: userInfoView.configInfo(model: model)
        userInfoView.titleModel(model: model)
        //: photoView.isMyhost = self.isMyhost
        photoView.isMyhost = self.isMyhost
        //: textContentView.configText(text: model.content ?? "", isMomentDetail: model.isMomentDetail ?? false)
        textContentView.trifle(text: model.content ?? "", isMomentDetail: model.isMomentDetail ?? false)

        //: if model.momentType == MomentType.Photo.rawValue || model.momentType == MomentType.Live.rawValue {
        if model.momentType == TypeOutputStream.Photo.rawValue || model.momentType == TypeOutputStream.Live.rawValue {
...
//            }
            ...
            photoView.ambagesModel(model: cunrrenModel)
...
//            }
            ...
            videoView.renderTop(model: model.videoInfo!, isTop: model.pinStatus)
        }

        //: bottomView.configModel(model: cunrrenModel)
        bottomView.statusClick(model: cunrrenModel)
...
//        }
    ...
    @objc func quickLocal() {
        //: let vc = TalkingMomentVideoVC.init(videoPath: cunrrenModel.videoInfo!.videoUrl)
        let vc = FailViewController(videoPath: cunrrenModel.videoInfo!.videoUrl)
        //: if cunrrenModel.sex != PunctuateThen.share.loginUserMode.sex {
        if cunrrenModel.sex != PunctuateThen.share.loginUserMode.sex {
    ...
    //: func playVideo() {
    func withSourceVideo() {
        //: if cunrrenModel.momentType == MomentType.Video.rawValue {
        if cunrrenModel.momentType == TypeOutputStream.Video.rawValue {
    ...
    //: func resume() {
    func outObserverModel() {
        //: videoView.resume()
        videoView.bullSEyeResume()
    }
}
...
extension EqualItemCell {
    ...
    private func by() {
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(quickLocal))
