...
/*: "id" :*/
fileprivate let show_normalBackId:[Character] = ["i","d"]
...
struct ViewMeasurable: HandyJSON {
    ...
    var pic = [StylePhotoModel]()
    //: var momentType: String?
    var momentType: String?
    //: var videoInfo: TalkingMomentVideoModel?
    var videoInfo: LikenessTransformable?
...
struct StylePhotoModel: HandyJSON {
    //: var thumbUrl: String?
    var thumbUrl: String?
    //: var imgUrl: String?
    var imgUrl: String?
...
struct LikenessTransformable: HandyJSON {
    //: var videoUrl = ""
    var videoUrl = ""
    //: var imgUrl = ""
    var imgUrl = ""
    ...
    var status: FourthColumnConvertible?
}

///
//: enum MomentVideoStatus: String {
enum FourthColumnConvertible: String {
    //: case NoPass = "-1"   // 未通过
    case NoPass = "-1" // 未通过
    //: case Audit = "0"     // 审核中
    case Audit = "0" // 审核中
...
extension ViewMeasurable {
    //: mutating func caculateItemHeight(isPost: Bool = false) {
    mutating func backPost(isPost: Bool = false) {
        ...
        if content!.count > 0 {
            ...
            textHeight = Int(NameSizeView.tinkle(text: content!, size: CGSize(width: kQueryValue - 54, height: CGFloat(MAXFLOAT)), font: UIFont.clickLarge(type: .Regular, fontSize: 17), lineNumber: lineNumber).height + (lineNumber == 0 ? 20 : 0))
        }

        //: if momentType == MomentType.Photo.rawValue || momentType == MomentType.Live.rawValue {
        if momentType == TypeOutputStream.Photo.rawValue || momentType == TypeOutputStream.Live.rawValue {
            //: if pic.count==1 {
            if pic.count == 1 {
                //: photesHeight = 212
                photesHeight = 212
