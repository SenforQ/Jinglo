...
/*: "id" :*/
fileprivate let kTextUserValue:String = "imake"

/*: "%@ reply %@：%@" :*/
fileprivate let data_voiceShowText:String = "%@ repscreen make self result"
fileprivate let main_acceptSuiteStr:[Character] = ["@"]

/*: " %@：%@" :*/
fileprivate let k_imageKey:[Character] = [" "]
fileprivate let appOmitToPath:String = "%@：%@"
...
extension PanModelType {
    //: mutating func caculateReplyItemHeight() {
    mutating func duringRejoin() {
        //: var tempContent = ""
        var tempContent = ""
        //: if replyUser.count>0 {
        if replyUser.count > 0 {
            //: tempContent = String(format: "%@ reply %@：%@", nickname, replyUser, content)
            tempContent = String(format: (String(data_voiceShowText.prefix(6)) + "ly %@：%" + String(main_acceptSuiteStr)), nickname, replyUser, content)
            //: } else {
        } else {
        ...
        let sizeheight = Int(NameSizeView.tinkle(text: tempContent, size: CGSize(width: kQueryValue - 82, height: CGFloat(MAXFLOAT)), font: UIFont.clickLarge(type: .Regular, fontSize: 14), lineNumber: 0).height)
