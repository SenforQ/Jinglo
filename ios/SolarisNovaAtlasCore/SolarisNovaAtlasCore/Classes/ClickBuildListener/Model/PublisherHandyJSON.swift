...
/*: "id" :*/
fileprivate let userCollectionName:[Character] = ["i","d"]

/*: "reply" :*/
fileprivate let noti_resultStr:String = "replplayer"

/*: :  :*/
fileprivate let constSizePath:String = "value "

/*:  : :*/
fileprivate let const_toValue:String = " empty"
...
struct PublisherHandyJSON: HandyJSON {
    //: var cid: String?
    ...
    mutating func along() {
        ...
        if replyUser?.count ?? 0 > 0 {
            ...
            if MakeThen.shared.direction == .leftToRight {
                ...
                tempContent = (noti_resultStr.replacingOccurrences(of: "player", with: "y")).localized + "\(replyUser!): \(content!)"
                //: } else {
            } else {
                //: tempContent = "\(content!) :\(replyUser!)" + "reply".localized
                tempContent = "\(content!) :\(replyUser!)" + (noti_resultStr.replacingOccurrences(of: "player", with: "y")).localized
            }
        ...
        let sizeheight = Int(NameSizeView.tinkle(text: tempContent, size: CGSize(width: kQueryValue - 70, height: CGFloat(MAXFLOAT)), font: UIFont.clickLarge(type: .Regular, fontSize: 16), lineNumber: 0).height)
