...
fileprivate let appWaitPath:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "8075F5" :*/
fileprivate let kIconMessage:String = "push position suite show image8075F5"

/*: "9A9B9C" :*/
fileprivate let data_modelId:String = "9A9B9Cview pop view"

/*: "kOpacityAnimation" :*/
fileprivate let dataReloadPath:String = "kOpacfield to type database thought"
fileprivate let user_firstValue:String = "countimatiocount"

/*: "opacity" :*/
fileprivate let constMainUrl:[Character] = ["o","p","a","c","i","t","y"]

/*: "forwards" :*/
fileprivate let app_modelValue:String = "apporwards"

/*: "easeIn" :*/
fileprivate let mainLayerId:String = "easeIntype gift with as table"

/*: "8C7AF8" :*/
fileprivate let k_labKey:[Character] = ["8","C","7","A","F"]
fileprivate let k_showUserName:String = "message"
...
class ForefrontReactiveCompatible: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: appWaitPath.reversed(), encoding: .utf8)!)
    }
    ...
    lazy var textField: UITextField = {
        ...
        view.addTarget(self, action: #selector(info(_:)), for: .editingChanged)
        //: return view
        return view
        //: }()
    }()
...
extension ForefrontReactiveCompatible {
    ...
    func modelView() {
        ...
        for index in 0 ..< codeNumber {
            ...
            if index < verStr.count {
                ...
                layer.shadowColor = UIColor(hex: (String(kIconMessage.suffix(6))))!.withAlphaComponent(0.6).cgColor
            }
            //: else {
            else {
                //: label.text = ""
                ...
                if index == verStr.count {
                    ...
                    layer.shadowColor = UIColor(hex: (String(data_modelId.prefix(6))))!.withAlphaComponent(0.25).cgColor
    ...
    fileprivate func equalImage(index: Int, hidden: Bool) {
        //: let line = shapeArray[index]
        let line = shapeArray[index]
        //: if hidden {
        if hidden {
            //: line.removeAnimation(forKey: "kOpacityAnimation")
            line.removeAnimation(forKey: (String(dataReloadPath.prefix(5)) + "ityA" + user_firstValue.replacingOccurrences(of: "count", with: "n")))
            //: } else {
        } else {
            //: line.add(opacityAnimation(), forKey: "kOpacityAnimation")
            line.add(perspective(), forKey: (String(dataReloadPath.prefix(5)) + "ityA" + user_firstValue.replacingOccurrences(of: "count", with: "n")))
        }
    ...
    fileprivate func perspective() -> CABasicAnimation {
        //: let animation = CABasicAnimation.init(keyPath: "opacity")
        let animation = CABasicAnimation(keyPath: (String(constMainUrl)))
        ...
        animation.fillMode = CAMediaTimingFillMode(rawValue: (app_modelValue.replacingOccurrences(of: "app", with: "f")))
        //: animation.timingFunction = CAMediaTimingFunction.init(name: CAMediaTimingFunctionName.init(rawValue: "easeIn"))
        animation.timingFunction = CAMediaTimingFunction(name: CAMediaTimingFunctionName(rawValue: (String(mainLayerId.prefix(6)))))
        //: return animation
        return animation
    }
}
...
extension ForefrontReactiveCompatible {
    ...
    private func subviewsApp() {
        ...
        for index in 0 ..< codeNumber {
            ...
            line.fillColor = UIColor(hex: (String(k_labKey) + k_showUserName.replacingOccurrences(of: "message", with: "8")))?.cgColor
