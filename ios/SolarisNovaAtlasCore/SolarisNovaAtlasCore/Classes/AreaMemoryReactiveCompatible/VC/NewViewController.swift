...
class NewViewController: SizeViewController {
    //: var type = LoginType.PhoneLogin
    var type = PublicCollection.PhoneLogin // 修改类型
    //: var content = ""                // 手机号
    var content = "" // 手机号
    //: var isNewPwd = false
    ...
    private lazy var pwdView: TableMemoryPwdView = {
        //: let v = TalkingSetNewPwdView(frame: .zero, isNewPwd: self.isNewPwd, loginType: self.type)
        let v = TableMemoryPwdView(frame: .zero, isNewPwd: self.isNewPwd, loginType: self.type)
...
extension NewViewController {
    ...
    private func countimate(_ pwd: String, _ vcode: String?) {
        //: ProgressHUD.show()
        ...
        ComponentRequestTool.sumeraction(pwd: pwd, vcode: vcode) { [weak self] succeed, _, _ in
            //: ProgressHUD.dismiss()
            DirectThen.cutWith()
            ...
            if self.isNewPwd == true {
                ...
                ComponentRequestTool.kind(self.type)
                //: } else {
            } else {
    ...
    private func quantityerchange() {
        ...
        ComponentRequestTool.equal { [weak self] succeed, _, _ in
            //: ProgressHUD.dismiss()
            DirectThen.cutWith()
            //: guard let self = self else { return }
            guard let self = self else { return }
