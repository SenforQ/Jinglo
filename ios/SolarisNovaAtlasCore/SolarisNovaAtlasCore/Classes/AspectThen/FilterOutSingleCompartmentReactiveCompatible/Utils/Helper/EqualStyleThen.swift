...
/*: "SENSEME" :*/
fileprivate let k_giftMessage:[Character] = ["S","E","N","S","E","M","E"]

/*: "lic" :*/
fileprivate let const_halfFormat:String = "LIC"

/*: "mf/Index/senseTime" :*/
fileprivate let k_equalName:[Character] = ["m","f","/","I","n","d","e","x","/","s","e","n","s","e","T","i","m"]
fileprivate let const_viewKey:String = "event"

/*: "url" :*/
fileprivate let const_frontImageName:String = "modell"

/*: "md5" :*/
fileprivate let showItemName:String = "index5"

/*: "SENSEMELIC" :*/
fileprivate let userDismissMsg:[Character] = ["S","E","N","S","E","M","E","L","I","C"]

/*: "App/ST/" :*/
fileprivate let noti_valueTopUrl:[Character] = ["A","p","p","/","S","T","/"]

/*: "GET" :*/
fileprivate let userRefreshData:[Character] = ["G","E","T"]
...
@objcMembers public class EqualStyleThen: NSObject {
    //: static public let share = STLicHelper()
    public static let share = EqualStyleThen()
    //: private override init() {}
    override private init() {}

    // 校验license有效性 先本地->后网络
    //: public func checkLicense() -> Bool {
    public func resultAdd() -> Bool {
    ...
    func sendDown() -> Bool {
        ...
        if let path = StatusPodspecBundle.bundle.path(forResource: (String(k_giftMessage)), ofType: (const_halfFormat.lowercased())) {
            //: let licenseData = NSData(contentsOfFile: path)
            let licenseData = NSData(contentsOfFile: path)
            //: if licenseData != nil {
            if licenseData != nil {
                //: return checkActiveCodeWithData(dataLicense: licenseData!)
                return groupLicense(dataLicense: licenseData!)
            }
        }
    ...
    func queryOver() -> Bool {
        ...
        if FileManager.default.fileExists(atPath: path as String) {
            ...
            if licData != nil {
                ...
                return groupLicense(dataLicense: licData!)
            }
        }
    ...
    func groupLicense(dataLicense: NSData) -> Bool {
...
extension EqualStyleThen {
    // 获取美颜license
    //: public func checkRemoteLicInfoWith(completion: @escaping Completionblock) {
    public func removeModel(completion: @escaping Completionblock) {
        //: let reqModel = TalkingRequestModel.init()
        let reqModel = SubRequestModel()
        //: reqModel.requestPath = "mf/Index/senseTime"
        reqModel.requestPath = (String(k_equalName) + const_viewKey.replacingOccurrences(of: "event", with: "e"))
        ...
        showTopStr.toFromLabel(model: reqModel) { _, result, _ in
            ...
            let licLink = json[(const_frontImageName.replacingOccurrences(of: "model", with: "ur"))].stringValue
            //: let md5 = json["md5"].stringValue
            let md5 = json[(showItemName.replacingOccurrences(of: "index", with: "md"))].stringValue
            //: if self.checkLicFileMd5With(md5: md5 as NSString) == false {
            if self.barTrademark(md5: md5 as NSString) == false {
                //: if  licLink.count > 0 {
                if licLink.count > 0 {
                    //: self.downloadLicsenWith(licUrl: licLink as NSString, completion: completion)
                    self.licsen(licUrl: licLink as NSString, completion: completion)
                    //: return
                    return
                }
            }
    ...
    func barTrademark(md5: NSString) -> Bool {
        //: if md5.length <= 0 {
        if md5.length <= 0 {
            //: return false
            return false
        ...
        let fileMd5: NSString = SendStartThen.shared.countimateModel(filePath: filePath as String) as NSString
    ...
    func side() -> NSString {
        ...
        let stDirectory = appearSaucer()
        //: return stDirectory.appendingPathComponent("SENSEMELIC") as NSString
        return stDirectory.appendingPathComponent((String(userDismissMsg))) as NSString
    }

    //: func licDiskDirectory() -> NSString {
    func appearSaucer() -> NSString {
        ...
        let stDirectory = docDirectory.appendingPathComponent((String(noti_valueTopUrl)))
    ...
    func licsen(licUrl: NSString, completion: @escaping Completionblock) {
        //: var urlOk = true
        var urlOk = true
        ...
        request.httpMethod = (String(userRefreshData))
        ...
        URLSession.shared.dataTask(with: request as URLRequest) { [weak self] data, _, _ in
            ...
            DispatchQueue.main.async {
                ...
                if data != nil {
                    ...
                    self.viewImage(licData: data! as NSData)
                    //: licValid = self.checkDownloadedLicense()
                    licValid = self.queryOver()
                }
