...
NSString *StringFromSimplicityData(Byte *data);        


//: STDisplayShader
Byte app_userData[] = {92, 15, 84, 13, 38, 242, 17, 178, 136, 185, 128, 124, 93, 255, 0, 240, 21, 31, 28, 24, 13, 37, 255, 20, 13, 16, 17, 30, 119};
...
- (instancetype)initWithContentBar:(CGRect)frame connection_strong:(EAGLContext *)context
{
    ...
    if (self) {
        ...
        [self weltanschauungProgram:StringFromSimplicityData(app_userData)
                //: fragmentShader:@"STDisplayShader"
                flake:StringFromSimplicityData(app_userData)
                    //: forProgram:&stDisplayProgram];
                    flakeOff:&stDisplayProgram];
...
- (BOOL)weltanschauungProgram:(NSString *)vertexShaderName
          //: fragmentShader:(NSString *)fragmentShaderName
          flake:(NSString *)fragmentShaderName
              //: forProgram:(GLuint *)programPointer;
              flakeOff:(GLuint *)programPointer;
{
    ...
    *programPointer = glCreateProgram();
    ...
    if (![self compilePathTwine:&vertexShader viewLenum:0x8B31 local:noti_successDrawUrl]) {
    ...
    if (![self compilePathTwine:&fragShader viewLenum:0x8B30 local:app_uniformMakePath]) {
...
- (BOOL)compilePathTwine:(GLuint *)shader viewLenum:(GLenum)type local:(const char *)str
{
    //: GLint status;
    GLint status;
    //: const GLchar *source;
...
- (void)renderVideoNameure:(GLuint)texture
{
...
Byte * SimplicityDataToCache(Byte *data) {
    int stateDisplay = data[0];
    int hoodoo = data[1];
    Byte context = data[2];
    int sub = data[3];
    if (!stateDisplay) return data + sub;
    for (int i = sub; i < sub + hoodoo; i++) {
...
NSString *StringFromSimplicityData(Byte *data) {
    return [NSString stringWithUTF8String:(char *)SimplicityDataToCache(data)];
}
