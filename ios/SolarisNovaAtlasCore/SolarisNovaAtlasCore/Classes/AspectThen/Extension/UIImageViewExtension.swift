...
/*: "add_header_cookies" :*/
fileprivate let const_logStr:String = "add_hetransform text send"
fileprivate let userTapValue:String = "_cookmodel label gift application right"
fileprivate let show_playerData:[Character] = ["i","e","s"]
...
public extension UIImageView {
    ...
    @objc func putBackThroughFinish(urlStr: String,
    ...
    internal func beforeRender(urlStr: String,
                               //: placeImg: UIImage = UIImage.placeImgSquare(),
                               placeImg: UIImage = UIImage.brother(),
    ...
    @objc func voiceAllStr(urlStr: String) {
        //: let imgUrl = urlStr.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: ReplaceUrlDomain)
        let imgUrl = urlStr.replacingOccurrences(of: const_maxFormalText.recordBy(), with: k_detailData)
        //: self.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage.giftPlaceHolderImage())
        self.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage.errorObject())
    }
    ...
    @objc func tillCamera(urlStr: String, completion: ((_ image: UIImage?) -> Void)? = nil) {
        // 允许请求携带cookie
        ...
        if var urlComponents = URLComponents(string: urlStr),
           ...
           let currURL = URL(string: LayerAddrTool.share.serverUrlStr)
        {
        ...
        let downloader = ImageDownloader(name: (String(const_logStr.prefix(6)) + "ader" + String(userTapValue.prefix(5)) + String(show_playerData)))
        ...
        self.kf.setImage(with: URL(string: newImgUrl), placeholder: UIImage.brother(), options: optionsInfo) { result in
            //: if completion != nil {
            if completion != nil {
                //: let image = try? result.get().image
                let image = try? result.get().image
