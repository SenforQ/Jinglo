...
- (UIImage *)cameraImage {
    ...
    return [UIImage imageWithData:[self collectionIntervalimate]];
}
...
+ (NSData *)error:(UIImage *)image secure:(NSInteger)fileSize{
    ...
    while (outIMGData.length>(fileSize*1024)) {
        ...
        thumIMG = [thumIMG price:scalesize device:NO];

        //: outIMGData = [thumIMG compressToFileSize:(fileSize*1024)];
        outIMGData = [thumIMG key:(fileSize*1024)];
    }
...
- (UIImage*)photoTarget:(CGSize)targetSize{
...
- (UIImage *)price:(CGSize)targetSize device:(BOOL)isScale{
...
- (UIImage *)memoryImage:(CGSize)newSize {
