...
/*: "recharge" :*/
fileprivate let notiCenterMsg:[Character] = ["r","e","c","h","a","r","g"]
fileprivate let dataClickMessage:String = "confirm"

/*: "recharge_ :*/
fileprivate let user_infoUrl:String = "recfeaturerg"
fileprivate let k_modelCapFormat:[Character] = ["e","_"]

/*: ".png" :*/
fileprivate let mainPathMessage:[Character] = [".","p","n","g"]

/*: "_ :*/
fileprivate let main_positionName:[Character] = ["_"]

/*: .png" :*/
fileprivate let const_menuMainFormat:String = "bottom generate self self.png"

/*: ".jpg" :*/
fileprivate let user_pushUrl:String = ".jpgcolor self make"

/*: .jpg" :*/
fileprivate let kEffectName:String = "self let self self.jpg"
...
enum ClickViewImageAlignment: NSInteger {
    //: case left = 0
    case left = 0
    //: case top
    case top
...
class StartImageButton: UIButton {
    ...
    var imageAlignment: ClickViewImageAlignment = .left
...
extension UIButton {
    ...
    func componentNow(_ interval: CGFloat) {
        //: verticalAlignImageAndTitle(interval: interval, topOffset: 0)
        mastheadImage(interval: interval, topOffset: 0)
    }
    ...
    func mastheadImage(interval: CGFloat, topOffset: CGFloat) {
    ...
    func digitiserActive(interval: CGFloat, imageSize: CGSize, topOffset: CGFloat) {
...
extension UIButton {
    ...
    func lineClick(color: UIColor, forState: UIControl.State) {
    ...
    func blockHalf(urlStr: String,
                   //: placeImg: UIImage? = UIImage.placeImgSquare(),
                   placeImg: UIImage? = UIImage.brother(),
                   ...
                   loadFinish: ((_ isSucceed: Bool) -> Void)? = nil)
    {
        ...
        let imgUrl = urlStr.replacingOccurrences(of: const_maxFormalText.recordBy(), with: k_detailData)
    ...
    func glassesByStr(urlStr: String) {
        //: guard !urlStr.isEmptyString else { return }
        guard !urlStr.isEmptyString else { return }
        //: var newUrl = ""
        var newUrl = ""
        //: if urlStr.contains("recharge") {
        if urlStr.contains((String(notiCenterMsg) + dataClickMessage.replacingOccurrences(of: "confirm", with: "e"))) {
            //: newUrl = urlStr.replacingOccurrences(of: "recharge", with: "recharge_\(PackageID)")
            newUrl = urlStr.replacingOccurrences(of: (String(notiCenterMsg) + dataClickMessage.replacingOccurrences(of: "confirm", with: "e")), with: (user_infoUrl.replacingOccurrences(of: "feature", with: "ha") + String(k_modelCapFormat)) + "\(show_systemValue)")
            //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
            self.blockHalf(urlStr: newUrl, placeImg: nil)
            //: return
            return
                //: } else if urlStr.contains(".png") {
        } else if urlStr.contains((String(mainPathMessage))) {
            //: newUrl = urlStr.replacingOccurrences(of: ".png", with: "_\(PackageID).png")
            newUrl = urlStr.replacingOccurrences(of: (String(mainPathMessage)), with: "_\(show_systemValue)" + (String(const_menuMainFormat.suffix(4))))

            //: } else if urlStr.contains(".jpg") {
        } else if urlStr.contains((String(user_pushUrl.prefix(4)))) {
            //: newUrl = urlStr.replacingOccurrences(of: ".jpg", with: "_\(PackageID).jpg")
            newUrl = urlStr.replacingOccurrences(of: (String(user_pushUrl.prefix(4))), with: "_\(show_systemValue)" + (String(kEffectName.suffix(4))))
        }
        //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
        self.blockHalf(urlStr: newUrl, placeImg: nil)
    }
