...
/*: "http://stat. :*/
fileprivate let main_labName:String = "hmentionmentionp"
fileprivate let notiLargeKey:String = "t.live app"

/*: .com" :*/
fileprivate let const_broadTextPath:[Character] = [".","c","o","m"]

/*: "http://" :*/
fileprivate let mainGreenKey:String = "http://register content error false"

/*: "https://" :*/
fileprivate let constNumberoValue:String = "data manager keyhttps://"

/*: "http://m. :*/
fileprivate let user_moreData:String = "indicatort"
fileprivate let notiModelUrl:String = "image for equal else layerp://m."

/*: "http:// :*/
fileprivate let data_equalStr:String = "http:/mode icon play var view"
fileprivate let mainBottomMsg:String = "path"

/*: "-1" :*/
fileprivate let constObjectKey:[Character] = ["-","1"]

/*: "3,90" :*/
fileprivate let notiShareStr:String = "cancel,90"

/*: "1990-01-01" :*/
fileprivate let constVideoMsg:String = "1990-"
fileprivate let showCornerValue:String = "01-01"

/*: "MM-dd-yyyy" :*/
fileprivate let appNameData:String = "MM-dd-ytrue screen hidden"
fileprivate let dataSaveFormat:String = "YYY"

/*: "id" :*/
fileprivate let dataBucketContent:String = "control"
...
// __CLOSE_PRINT__
//
//  MessageTransformableRequestBaseModel.swift
//  AbroadTalking
//
...
class SubRequestModel: NSObject {
    ...
    var requestType: NatureRequestType?
    ...
    override init() {
        ...
        self.requestServer = LayerAddrTool.share.serverUrlStr
...
struct SessionModelType: HandyJSON {
    //: var errno: Int!
...
public struct FeatureErrorResponse {
    //: let errorCode: Int
    let errorCode: Int
    //: let errorMsg: String
    let errorMsg: String
...
struct MessageTransformable: HandyJSON {
    ...
    var CSConfig = FileFailMeasurable() // 私聊官方客服提示消息模型
//    var jumpRandomVideo: Bool = false       // true表示可以跳转到随机视频匹配
    ...
    var reportDomain: String {
        ...
        if statDomain.isEmptyString {
            ...
            return (main_labName.replacingOccurrences(of: "mention", with: "t") + "://sta" + String(notiLargeKey.prefix(2))) + "\(k_detailData)" + (String(const_broadTextPath))
            //: } else {
        } else {
            //: if !statDomain.hasPrefix("http://") && !statDomain.hasPrefix("https://") {
            if !statDomain.hasPrefix((String(mainGreenKey.prefix(7)))) && !statDomain.hasPrefix((String(constNumberoValue.suffix(8)))) {
                //: return "http://".appending(statDomain)
                return (String(mainGreenKey.prefix(7))).appending(statDomain)
                //: }else {
            } else {
                //: return statDomain
                return statDomain
    ...
    var urlH5Domain: String {
        ...
        if urlDomain.isEmpty {
            ...
            return (user_moreData.replacingOccurrences(of: "indicator", with: "ht") + String(notiModelUrl.suffix(6))) + "\(k_detailData)" + (String(const_broadTextPath))
            //: } else {
        } else {
            //: return "http://\(urlDomain)"
            return (String(data_equalStr.prefix(6)) + mainBottomMsg.replacingOccurrences(of: "path", with: "/")) + "\(urlDomain)"
        }
    }
...
public class AppReactiveCompatible: NSObject, HandyJSON {
    ...
    var chatPriceSettings: [StartTransformable] = [] // 收费设置
    //: var videoPriceSettings: Array<ChatPriceModel> = []
    var videoPriceSettings: [StartTransformable] = [] // 视频收费设置
    //: var voicePriceSettings: Array<ChatPriceModel> = []
    var voicePriceSettings: [StartTransformable] = [] // 语音收费设置
    ...
    var vipSetting = [LoadTransformable]() // vip皮肤
    ...
    var tabPageBanner = [MakeSingleBannerModel]() // V1.6.4 一级页面广告
    ...
    var intimateLimit = ViewThermalBarrierModelType() // 1.7.6 各种操作的亲密度限制
...
@objcMembers public class MessageMeasurable: NSObject, HandyJSON {
    //: required public override init() {
    override public required init() {}
    ...
    var interest: [HiddenHandyJSON]?
    //: var aboutMe: Array<UserSeleteTagModel>?
    var aboutMe: [HiddenHandyJSON]?
    //: var partner: String?
    var partner: String?
    //: var full: Int?
    var full: Int?
    ...
    var callPopupSetting = (notiShareStr.replacingOccurrences(of: "cancel", with: "3")) // 表示来电弹窗配置，如”3,90”表示3s后弹第一次，后续90s，返回”0,0”表示不弹窗
    //: var callTabSwitch = 0
    ...
    public lazy var age: String = {
        ...
        let birthdayDate = NSDate.noFormat(self.birthday ?? (constVideoMsg.capitalized + showCornerValue.capitalized), dateFormat: (String(appNameData.prefix(7)) + dataSaveFormat.lowercased()))
        //: guard let currentYear = NSDate.getCurrentDateComponents().year else { return "0" }
        guard let currentYear = NSDate.baseballPlayer().year else { return "0" }
...
struct QueryMeasurable: HandyJSON {
    //: var id: String?
    var id: String?
    //: var url: String?
    var url: String?
...
public struct HiddenHandyJSON: HandyJSON, Equatable {
    //: var tag_id: String?
    var tag_id: String?
    //: var tag_name = ""
    var tag_name = ""
    ...
    func publish(compareTo: HiddenHandyJSON) -> Bool {
        //: return
        return
...
struct ModelHandyJSON: HandyJSON {
    ...
    var aboutMe: [ClickTypeModel]!
    //: var interest: Array<UserTagTypeModel>!
    var interest: [ClickTypeModel]!
}

//: struct UserTagTypeModel: HandyJSON {
struct ClickTypeModel: HandyJSON {
    //: var name = ""
    var name = ""
    //: var name_ar = ""
    var name_ar = ""
    ...
    var list: [HiddenHandyJSON]!
}

/// 官方客服提示消息模型
//: struct CSConfigModel: HandyJSON {
struct FileFailMeasurable: HandyJSON {
...
struct LoadTransformable: HandyJSON {
...
struct ViewThermalBarrierModelType: HandyJSON {
    //: var sendImg = 10
    var sendImg = 10 // 发送图片
    //: var sendVideo = 10
