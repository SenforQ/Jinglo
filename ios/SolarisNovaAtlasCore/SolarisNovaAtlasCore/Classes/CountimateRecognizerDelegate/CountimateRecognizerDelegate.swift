...
// __CLOSE_PRINT__
//
//  CountimateRecognizerDelegate.swift
//  AbroadTalking
//
...
class CountimateRecognizerDelegate: SizeViewController {
    //: var tranConfig: BasePresentTransition!
    var tranConfig: KeyAnimatedTransitioning!
    //: init() {
    init() {
        //: super.init(nibName: nil, bundle: nil)
        super.init(nibName: nil, bundle: nil)
        //: self.tranConfig = BasePresentTransition.init(target: self)
        self.tranConfig = KeyAnimatedTransitioning(target: self)
    }
...
class KeyAnimatedTransitioning: NSObject {
    ...
    var frameOfPresentedView = CGRect(x: 0, y: 0, width: kQueryValue, height: notiKeyTitle * 0.6)
    ...
    func pushData(corners: UIRectCorner, radius: CGFloat) {
...
extension KeyAnimatedTransitioning: UIViewControllerTransitioningDelegate {
    ...
    func presentationController(forPresented presented: UIViewController, presenting: UIViewController?, source _: UIViewController) -> UIPresentationController? {
        ...
        let res = SpecialSessionPresentationController(transition: self, presentedViewController: presented, presentingViewController: presenting)
        //: return res
        return res
    }
}
...
extension KeyAnimatedTransitioning: UIViewControllerAnimatedTransitioning {
    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        if toVC == targetVC {
            ...
            fossilisationFilterContext(transitionContext: transitionContext)
            //: } else {
        } else {
            //: dismissTransition(transitionContext: transitionContext)
            clean(transitionContext: transitionContext)
        }
    }

    //: private func presentTransition(transitionContext: UIViewControllerContextTransitioning) {
    private func fossilisationFilterContext(transitionContext: UIViewControllerContextTransitioning) {
        //: let toVC = transitionContext.viewController(forKey: .to)
        let toVC = transitionContext.viewController(forKey: .to)
    ...
    private func clean(transitionContext: UIViewControllerContextTransitioning) {
...
class SpecialSessionPresentationController: UIPresentationController, UIGestureRecognizerDelegate {
    //: var transition: BasePresentTransition!
    var transition: KeyAnimatedTransitioning!

    //: init(transition: BasePresentTransition, presentedViewController: UIViewController, presentingViewController: UIViewController?) {
    init(transition: KeyAnimatedTransitioning, presentedViewController: UIViewController, presentingViewController: UIViewController?) {
    ...
    func themeImage() {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(showUponExpensiveness))
        //: tap.delegate = self
        tap.delegate = self
    ...
    @objc func showUponExpensiveness() {
    ...
    lazy var visualView: UIView = {
        ...
        view.frame = containerView?.bounds ?? CGRect(x: 0, y: 0, width: kQueryValue, height: notiKeyTitle)
