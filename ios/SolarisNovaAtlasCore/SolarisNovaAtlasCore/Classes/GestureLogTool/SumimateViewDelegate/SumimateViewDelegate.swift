...
class SumimateViewDelegate: UIViewController {
    ...
    weak var delegate: NameNavigationDelegate?
    ...
    private lazy var controllerScrollView: UIScrollView = {
        ...
        let count = min(self.controllerItems.count, self.titleItems.count)
        //: var height = PublisherReactiveCompatible.getScreenHeight() - (topDistance + kTitle_H)
        var height = PublisherReactiveCompatible.high() - (topDistance + kTitle_H)
        //: if self.hasBttomTabBar {
        if self.hasBttomTabBar {
            //: height -= TabBarViewHeight
            height -= app_tableNoHeightName
        }
        ...
        //: let rect = CGRect(x: 0, y: topDistance + kTitle_H, width: PublisherReactiveCompatible.getScreenWidth(), height: height)
        let rect = CGRect(x: 0, y: topDistance + kTitle_H, width: PublisherReactiveCompatible.widthState(), height: height)
        //: let scrollView = UIScrollView(frame: rect)
        let scrollView = UIScrollView(frame: rect)
        //: scrollView.contentSize = CGSize(width: count * Int(PublisherReactiveCompatible.getScreenWidth()), height: 0)
        scrollView.contentSize = CGSize(width: count * Int(PublisherReactiveCompatible.widthState()), height: 0)
        ...
        scrollView.semanticContentAttribute = MakeThen.shared.direction == .leftToRight ? .forceLeftToRight:.forceRightToLeft
        //: return scrollView
        return scrollView
    //: }()
    }()
    ...
    private lazy var titleScrollView: UIScrollView = {
        //: var rect = CGRect(x: 15, y: topDistance, width: PublisherReactiveCompatible.getScreenWidth()-rightSpace, height: kTitle_H)
        var rect = CGRect(x: 15, y: topDistance, width: PublisherReactiveCompatible.widthState()-rightSpace, height: kTitle_H)
        //: if isBisect {
        if isBisect {
            //: rect = CGRect(x: 0, y: topDistance, width: PublisherReactiveCompatible.getScreenWidth()-rightSpace, height: kTitle_H)
            rect = CGRect(x: 0, y: topDistance, width: PublisherReactiveCompatible.widthState()-rightSpace, height: kTitle_H)
        }
        //: let scrollView = UIScrollView(frame: rect)
        let scrollView = UIScrollView(frame: rect)
        ...
        var originX: CGFloat = MakeThen.shared.direction == .leftToRight ? 0:25
        ...
        self.titleItems.enumerated().forEach { [weak self] index, obj in
            ...
            titleBtn.addTarget(self, action: #selector(toRow), for: .touchDown)
            //: var titleWidth = 0.0
            var titleWidth = 0.0
            //: if isBisect {
            if isBisect {
                //: titleWidth = PublisherReactiveCompatible.getScreenWidth() / Double(self.titleItems.count)
                titleWidth = PublisherReactiveCompatible.widthState() / Double(self.titleItems.count)
            //: } else {
            } else {
                //: titleWidth = (titleBtn.titleLabel?.sizeThatFits(CGSize(width: CGFLOAT_MAX, height: kTitle_H)).width ?? 0) + 22
                titleWidth = (titleBtn.titleLabel?.sizeThatFits(CGSize(width: CGFLOAT_MAX, height: kTitle_H)).width ?? 0) + 22
            }
            //: let x = LanguageManager.shared.direction == .leftToRight ? originX:(PublisherReactiveCompatible.getScreenWidth()-originX-titleWidth)
            let x = MakeThen.shared.direction == .leftToRight ? originX:(PublisherReactiveCompatible.widthState()-originX-titleWidth)
...
extension SumimateViewDelegate {
    //: @objc private func titleButtonClick(_ sender: UIButton) {
    @objc private func toRow(_ sender: UIButton) {
        //: currentSelectedTitleButton(sender)
        adminButton(sender)
        //: let index = sender.tag - 1000
        let index = sender.tag - 1000
        //: let offset_x = CGFloat(index) * PublisherReactiveCompatible.getScreenWidth()
        let offset_x = CGFloat(index) * PublisherReactiveCompatible.widthState()
        //: controllerScrollView.contentOffset = CGPoint(x: offset_x, y: 0)
        controllerScrollView.contentOffset = CGPoint(x: offset_x, y: 0)
        //: addControllerView(withIndex: index)
        styleRelease(withIndex: index)
        //: seletedViewController(withIndex: index)
        cannulise(withIndex: index)
    }
    
    //: private func addControllerView(withIndex index: Int) {
    private func styleRelease(withIndex index: Int) {
        //: if index >= children.count {
        if index >= children.count {
            //: return
            return
        }
        //: let origin_x = CGFloat(index) * PublisherReactiveCompatible.getScreenWidth()
        let origin_x = CGFloat(index) * PublisherReactiveCompatible.widthState()
        ...
        //: vc.view.frame = CGRect(x: origin_x, y: 0, width: PublisherReactiveCompatible.getScreenWidth(), height: controllerScrollView.frame.size.height)
        vc.view.frame = CGRect(x: origin_x, y: 0, width: PublisherReactiveCompatible.widthState(), height: controllerScrollView.frame.size.height)
        //: controllerScrollView.addSubview(vc.view)
        controllerScrollView.addSubview(vc.view)
    }
    
    //: private func currentSelectedTitleButton(_ button: UIButton) {
    private func adminButton(_ button: UIButton) {
    ...
    func layer(_ sender: UIButton) {
        //: if titleScrollView.contentSize.width <= PublisherReactiveCompatible.getScreenWidth() {
        if titleScrollView.contentSize.width <= PublisherReactiveCompatible.widthState() {
            //: return
            return
        }
        //: var offset_x = sender.center.x - PublisherReactiveCompatible.getScreenWidth() * 0.5
        var offset_x = sender.center.x - PublisherReactiveCompatible.widthState() * 0.5
        //: if offset_x < 0 {
        if offset_x < 0 {
            //: offset_x = 0
            offset_x = 0
        }
        //: let maxOffset = titleScrollView.contentSize.width - PublisherReactiveCompatible.getScreenWidth()
        let maxOffset = titleScrollView.contentSize.width - PublisherReactiveCompatible.widthState()
...
extension SumimateViewDelegate {
    ...
    func cannulise(withIndex index: Int) {
        //: guard index < children.count else { return }
        guard index < children.count else { return }
        //: delegate?.selectedViewController(children[index], index: index)
        delegate?.toScale(children[index], index: index)
    }
}

// MARK: - UIScrollViewDelegate
//: extension QYSlideNavigationViewController: UIScrollViewDelegate {
extension SumimateViewDelegate: UIScrollViewDelegate {
    //: func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
    func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
        //: let index = Int(controllerScrollView.contentOffset.x / PublisherReactiveCompatible.getScreenWidth())
        let index = Int(controllerScrollView.contentOffset.x / PublisherReactiveCompatible.widthState())
        //: if let selectedBtn = titleScrollView.viewWithTag(index + 1000) as? UIButton {
        if let selectedBtn = titleScrollView.viewWithTag(index + 1000) as? UIButton {
            //: currentSelectedTitleButton(selectedBtn)
            adminButton(selectedBtn)
        }
        //: addControllerView(withIndex: index)
        styleRelease(withIndex: index)
        //: seletedViewController(withIndex: index)
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        //: let index = Int(scrollView.contentOffset.x / PublisherReactiveCompatible.getScreenWidth())
        let index = Int(scrollView.contentOffset.x / PublisherReactiveCompatible.widthState())
        ...
        //: let scaleR = scrollView.contentOffset.x / PublisherReactiveCompatible.getScreenWidth() - CGFloat(index)
        let scaleR = scrollView.contentOffset.x / PublisherReactiveCompatible.widthState() - CGFloat(index)
        //: let scaleL = 1 - scaleR
        let scaleL = 1 - scaleR
