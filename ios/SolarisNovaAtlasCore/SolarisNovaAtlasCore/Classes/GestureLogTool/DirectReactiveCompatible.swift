...
/*: "Finish" :*/
fileprivate let mainShareHiddenInfoName:[Character] = ["F","i","n","i","s","h"]

/*: "Cancel" :*/
fileprivate let mainPointName:[Character] = ["C","a","n","c","e","l"]

/*: "btn_photo_choice_pre" :*/
fileprivate let app_allIconUrl:[Character] = ["b","t","n","_","p","h","o","t","o","_","c","h","o","i","c"]
fileprivate let noti_countMsg:String = "e_prevideo self interval"

/*: "btn_photo_add_nor" :*/
fileprivate let data_makeStr:String = "btn_layer if"
fileprivate let user_modelName:String = "min max view tool true_add_no"
fileprivate let noti_cancelFormat:String = "raw"

/*: "( :*/
fileprivate let noti_infoContent:String = "("

/*: )" :*/
fileprivate let data_directionName:String = ")"

/*: "/tmp" :*/
fileprivate let dataCurrencyMessage:String = "present case/tmp"

/*: "Export session failed" :*/
fileprivate let const_makePath:String = "view"
fileprivate let appRawKey:[Character] = ["x","p","o","r","t"," ","s","e","s","s","i","o","n"," ","f","a","i","l","e","d"]

/*: "Export canceled" :*/
fileprivate let show_timeExtraFormat:String = "Expolet as wait equal"
fileprivate let noti_resultMsg:String = "anofl"
fileprivate let userThoughtFormat:String = "ED"

/*: "Successful!" :*/
fileprivate let k_colorKey:String = "model profile i make rawSucces"

/*: "Unsupported video formats" :*/
fileprivate let dataIconBottomLabelFormat:[Character] = ["U","n","s","u","p","p","o","r","t","e","d"," ","v","i","d","e","o"," ","f","o","r","m","a","t","s"]

/*: "yyyy-MM-dd-HH:mm:ss-SSS" :*/
fileprivate let showMenuChangeValue:String = "yyyy-status data"
fileprivate let dataTransactionMessage:String = "kit content icon false modeMM-dd"
fileprivate let dataMatchTitle:String = "string extension face return make-HH:mm"

/*: "/tmp/video- :*/
fileprivate let mainLeadingPath:String = "as log any view succeed/t"
fileprivate let userSexStr:String = "mp/vself type top"

/*: - :*/
fileprivate let user_listId:[Character] = ["-"]

/*: .mp4" :*/
fileprivate let showLogText:String = ".mp4type var"
...
class DirectReactiveCompatible: NSObject {
    ...
    class func sizeUponTelecasting(allowImgCrop: Bool = false, needCircleCrop: Bool = false, maxCount: Int, allowPhoto: Bool, allowVideo: Bool) -> TZImagePickerController {
...
//        imagePicker?.preferredLanguage = "en"
        ...
        if allowImgCrop {
            ...
            var height = kQueryValue * 488 / 375
            ...
            imagePicker?.cropRect = CGRect(x: 0, y: (notiKeyTitle - height) / 2, width: kQueryValue, height: height)
        }
        //: if allowVideo {
        if allowVideo {
        ...
        imagePicker?.doneBtnTitleStr = (String(mainShareHiddenInfoName)).localized
        //: imagePicker?.cancelBtnTitleStr = "Cancel".localized
        imagePicker?.cancelBtnTitleStr = (String(mainPointName)).localized

        //: imagePicker?.barItemTextFont = UIFont.pingfangFont(type: .Regular, fontSize: 15)
        imagePicker?.barItemTextFont = UIFont.clickLarge(type: .Regular, fontSize: 15)
        //: imagePicker?.barItemTextColor = UIColor.appValueColor()
        imagePicker?.barItemTextColor = UIColor.imageColor()
        ...
        imagePicker?.naviTitleFont = UIFont.clickLarge(type: .Medium, fontSize: 17)
        //: imagePicker?.naviTitleColor = UIColor.appTitleColor()
        imagePicker?.naviTitleColor = UIColor.toEvent()
        ...
        imagePicker?.iconThemeColor = UIColor.readRemove()

        //: imagePicker?.allowPickingOriginalPhoto = false
        imagePicker?.allowPickingOriginalPhoto = false
        //: imagePicker?.photoSelImage = UIImage.BundleImageNamed(name: "btn_photo_choice_pre")
        imagePicker?.photoSelImage = UIImage.thoughtImageBundleSizeText(name: (String(app_allIconUrl) + String(noti_countMsg.prefix(5))))
        //: imagePicker?.takePictureImage = UIImage.BundleImageNamed(name: "btn_photo_add_nor")
        imagePicker?.takePictureImage = UIImage.thoughtImageBundleSizeText(name: (String(data_makeStr.prefix(4)) + "photo" + String(user_modelName.suffix(7)) + noti_cancelFormat.replacingOccurrences(of: "raw", with: "r")))
        ...
        imagePicker?.photoPickerPageDidRefreshStateBlock = { (_: UICollectionView?,
                ...
                doneButton?.titleLabel?.font = UIFont.clickLarge(type: .Medium, fontSize: 14)
                ...
                if text.count > 0 && Int(text)! > 0 {
                    ...
                    doneButton?.setTitle((String(mainShareHiddenInfoName)).localized + "(\(text))", for: .normal)
                    //: } else {
                } else {
                    //: doneButton?.setTitle("Finish".localized, for: .normal)
                    doneButton?.setTitle((String(mainShareHiddenInfoName)).localized, for: .normal)
                }
                //: return
        }
        ...
        imagePicker?.photoPickerPageDidLayoutSubviewsBlock = { (_: UICollectionView?,
                ...
                doneButton?.backgroundColor = UIColor.readRemove()
                //: let text = numberLabel?.text ?? "0"
                let text = numberLabel?.text ?? "0"
                //: if text.count > 0 && Int(text)! > 0 {
                if text.count > 0 && Int(text)! > 0 {
        ...
        imagePicker?.photoPreviewPageDidLayoutSubviewsBlock = { (_: UICollectionView?,
                ...
                selectButton?.frame = CGRect(x: kQueryValue - 5 - (selectButton?.frame.size.width)!, y: (selectButton?.frame.origin.y)!, width: (selectButton?.frame.size.width)!, height: (selectButton?.frame.size.height)!)
                //: let text = numberLabel?.text ?? "0"
                let text = numberLabel?.text ?? "0"
                //: if text.count > 0 && Int(text)! > 0 {
                if text.count > 0 && Int(text)! > 0 {
    ...
    class func take(asset: PHAsset?, completion: @escaping (_ filePath: URL?) -> Void) {
        ...
        PHImageManager.default().requestAVAsset(forVideo: asset!, options: options) { (asset: AVAsset?, _: AVAudioMix?, _) in
            ...
            if asset!.isKind(of: AVURLAsset.self) {
                ...
                if !FileManager.default.fileExists(atPath: NSHomeDirectory().appending((String(dataCurrencyMessage.suffix(4))))) {
                    //: do {
                    do {
                        //: try FileManager.default.createDirectory(atPath: NSHomeDirectory().appending("/tmp"), withIntermediateDirectories: true)
                        try FileManager.default.createDirectory(atPath: NSHomeDirectory().appending((String(dataCurrencyMessage.suffix(4)))), withIntermediateDirectories: true)
                        //: } catch {
                    } catch {}
                }
                ...
                exportSession?.exportAsynchronously(completionHandler: {
                    ...
                    case .failed:
                        ...
                        printLog(message: (const_makePath.replacingOccurrences(of: "view", with: "E") + String(appRawKey)))
                    //: case .cancelled:
                    case .cancelled:
                        //: printLog(message: "Export canceled")
                        printLog(message: (String(show_timeExtraFormat.prefix(4)) + "rt c" + noti_resultMsg.replacingOccurrences(of: "of", with: "ce") + userThoughtFormat.lowercased()))
                    //: case .completed:
                    case .completed:
                        //: printLog(message: "Successful!")
                        printLog(message: (String(k_colorKey.suffix(6)) + "sful!"))
                        //: DispatchQueue.main.async {
                        DispatchQueue.main.async {
                ...
                printLog(message: (String(dataIconBottomLabelFormat)))
            }
        }
    }
    ...
    private class func addStatus() -> String {
        //: let formater = DateFormatter()
        let formater = DateFormatter()
        //: formater.dateFormat = "yyyy-MM-dd-HH:mm:ss-SSS"
        formater.dateFormat = (String(showMenuChangeValue.prefix(5)) + String(dataTransactionMessage.suffix(5)) + String(dataMatchTitle.suffix(6)) + ":ss-SSS")
        //: let outputPath = NSHomeDirectory().appending("/tmp/video-\(formater.string(from: Date()))-\(Int.random(in: 0...10000000)).mp4")
        let outputPath = NSHomeDirectory().appending((String(mainLeadingPath.suffix(2)) + String(userSexStr.prefix(4)) + "ideo-") + "\(formater.string(from: Date()))-\(Int.random(in: 0 ... 10_000_000))" + (String(showLogText.prefix(4))))
        //: return outputPath
        return outputPath
    }
}
