...
/*: "cellID" :*/
fileprivate let constLivePath:[Character] = ["c","e","l","l","I","D"]

/*: "init(coder:) has not been implemented" :*/
fileprivate let dataSituationName:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "Video Call" :*/
fileprivate let userMakeFormat:String = "Videinterval add self"
fileprivate let constRequestStr:String = "name return makeo Call"

/*: "Video Call (%@ Points/Min)" :*/
fileprivate let kOfDataName:String = "else height selfVideo"
fileprivate let appEffectToPath:[Character] = ["(","%","@"," "]
fileprivate let k_viewFormat:[Character] = ["P","o","i","n","t","s","/","M","i","n",")"]

/*: "Voice Call" :*/
fileprivate let mainButtonMessage:[Character] = ["V","o","i","c","e"," ","C","a","l","l"]

/*: "Voice Call (%@ Points/Min)" :*/
fileprivate let dataLabContent:String = "Voicelarge top center self data"
fileprivate let constBackUpgradeKey:String = "l (%@super birth let between"
fileprivate let main_titleEmptyMsg:String = "super case special countnts/Min)"

/*: "#F8F8F8" :*/
fileprivate let main_nameTitle:String = "true var accept color#F"
fileprivate let showThoughtTitle:String = "8F8F8mode take progress load"

/*: "Cancel" :*/
fileprivate let app_alongMomentData:String = "social"
fileprivate let showLayerTitle:[Character] = ["a","n","c","e","l"]

/*: "#F5F5F5" :*/
fileprivate let noti_keyText:String = "user center#F5F5F"
fileprivate let showErrorUrl:[Character] = ["5"]

/*: "icon_lounge" :*/
fileprivate let notiModelSourceMsg:[Character] = ["i","c","o","n","_","l"]
fileprivate let appContentMsg:[Character] = ["o","u","n","g","e"]
...
// __CLOSE_PRINT__
//
//  SelectReactiveCompatible.swift
//  AbroadTalking
//
...
class SelectReactiveCompatible: UIView {
    //: var munuBlock: MunuBlock?
    var munuBlock: MunuBlock?

    //: private var type: MunuPopType = .Unknown
    private var type: SessionVideoPopType = .Unknown // 当前展示类型
    ...
    var cellTitleColor = UIColor.toEvent()
    /// cell字体大小
    //: var cellTitleFont = UIFont.pingfangFont(type: .Regular, fontSize: 16)
    var cellTitleFont = UIFont.clickLarge(type: .Regular, fontSize: 16)
    /// 是否有"Cancel"菜单
    //: var hasCancel = true
    var hasCancel = true
    /// cancel字体颜色
    //: var cancelColor = UIColor.appValueDetailColor()
    var cancelColor = UIColor.hockeyPlayer()
    /// cancel字体大小
    //: var cancelTitleFont = UIFont.pingfangFont(type: .Regular, fontSize: 16)
    var cancelTitleFont = UIFont.clickLarge(type: .Regular, fontSize: 16)
    /// 标题
    //: var title: String?
    var title: String?
    /// 标题颜色
    //: var titleColor = UIColor.appThemeColor()
    var titleColor = UIColor.readRemove()
    /// 标题字体大小
    //: var titleFont = UIFont.atomic(fontSize: 16)
    var titleFont = UIFont.atomic(fontSize: 16)

    //: var tableView = UITableView()
    var tableView = UITableView()
    //: let cellID = "cellID"
    let cellID = (String(constLivePath))
    ...
    fileprivate var footerViewHight: CGFloat {
        ...
        return hasCancel ? (footerViewLineHight + 50 + k_netId) : k_netId
    }
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: dataSituationName.reversed(), encoding: .utf8)!)
    }

    //: func initwithList(cellTitleList: [String]!) {
    func fullPhaseOfTheMoon(cellTitleList: [String]!) {
        // 初始化
    ...
    func exhibitPrice(type: SessionVideoPopType, videoPrice: String?, voicePrice: String?) {
        //: self.type = type
        self.type = type
        //: if self.type == .Call {
        if self.type == .Call { // 音视频通话
            ...
            if PunctuateThen.share.appStatus == HiddenHashableRepresentation.normal.rawValue {
                ...
                if PunctuateThen.share.loginUserMode.sex == TopComparable.male.rawValue {
                    ...
                    let videoAttrStr = String.inclose(VideoPrice: Int(videoPrice ?? "0") ?? 0)
                    //: let voiceAttrStr = String.setVoicePriceStr(VoicePrice: Int(voicePrice ?? "0") ?? 0)
                    let voiceAttrStr = String.fromEqual(VoicePrice: Int(voicePrice ?? "0") ?? 0)
                    ...
                    let attributes = [.foregroundColor: UIColor.readRemove(), .font: UIFont.clickLarge(type: .Regular, fontSize: 14)] as [NSAttributedString.Key: Any]
                    //: let VideoCall = "Video Call".localized
                    let VideoCall = (String(userMakeFormat.prefix(4)) + String(constRequestStr.suffix(6))).localized
                    //: let videoStr = "Video Call (%@ Points/Min)".localizedArguments(videoPrice ?? "0")
                    let videoStr = (String(kOfDataName.suffix(5)) + " Call " + String(appEffectToPath) + String(k_viewFormat)).viewfinder(videoPrice ?? "0")
                    ...
                    let VoiceCall = (String(mainButtonMessage)).localized
                    //: let voiceStr = "Voice Call (%@ Points/Min)".localizedArguments(voicePrice ?? "0")
                    let voiceStr = (String(dataLabContent.prefix(5)) + " Cal" + String(constBackUpgradeKey.prefix(5)) + " Poi" + String(main_titleEmptyMsg.suffix(8))).viewfinder(voicePrice ?? "0")
                ...
                self.cellTitleList = [(String(userMakeFormat.prefix(4)) + String(constRequestStr.suffix(6))).localized, (String(mainButtonMessage)).localized]
            }
        }
    ...
    public func iconImage() {
        ...
        tableView = UITableView(frame: CGRect(x: 0, y: notiKeyTitle - tableViewHight, width: kQueryValue, height: tableViewHight), style: .plain)
        ...
        tableView.register(ViewContentCell.self, forCellReuseIdentifier: cellID)
        ...
        if title == nil {
            ...
            tableView.areaRemoveRadiiAt(width: kQueryValue,
...
extension SelectReactiveCompatible: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        var cell = tableView.dequeueReusableCell(withIdentifier: cellID) as? ViewContentCell
        //: if cell == nil {
        if cell == nil {
            //: cell = TitleCell(style: .default, reuseIdentifier: cellID)
            cell = ViewContentCell(style: .default, reuseIdentifier: cellID)
        }

        //: if PunctuateThen.share.loginUserMode.sex == Gender.male.rawValue,
        if PunctuateThen.share.loginUserMode.sex == TopComparable.male.rawValue,
           //: PunctuateThen.share.loginUserMode.loungePlus == true,
           PunctuateThen.share.loginUserMode.loungePlus == true,
           //: type == .Call {
           type == .Call
        {
        ...
        cell!.titleLab.font = cellTitleFont
        //: if self.type == .Call && PunctuateThen.share.appStatus == AppSkinStatus.normal.rawValue {
        if self.type == .Call && PunctuateThen.share.appStatus == HiddenHashableRepresentation.normal.rawValue {
    ...
    func tableView(_: UITableView, viewForFooterInSection _: Int) -> UIView? {
        ...
        let footerView = UIView(frame: CGRect(x: 0, y: 0, width: kQueryValue, height: footerViewHight))
        //: let lineView = UIView(frame: CGRect(x: 0, y: 0, width: ScreenWidth, height: footerViewLineHight))
        let lineView = UIView(frame: CGRect(x: 0, y: 0, width: kQueryValue, height: footerViewLineHight))
        //: lineView.backgroundColor = UIColor(hex: "#F8F8F8")
        lineView.backgroundColor = UIColor(hex: (String(main_nameTitle.suffix(2)) + String(showThoughtTitle.prefix(5))))
        ...
        btn.frame = CGRect(x: 0, y: footerViewLineHight, width: kQueryValue, height: 50)
        //: footerView.addSubview(btn)
        footerView.addSubview(btn)
        //: btn.setTitle("Cancel".localized, for: .normal)
        btn.setTitle((app_alongMomentData.replacingOccurrences(of: "social", with: "C") + String(showLayerTitle)).localized, for: .normal)
    ...
    func tableView(_: UITableView, viewForHeaderInSection _: Int) -> UIView? {
        ...
        let headerView = UIView(frame: CGRect(x: 0, y: 0, width: kQueryValue, height: headerViewHight))

        //: let titleLab = UILabel(frame: CGRect(x: 15, y: 0, width: ScreenWidth-30, height: headerViewHight))
        let titleLab = UILabel(frame: CGRect(x: 15, y: 0, width: kQueryValue - 30, height: headerViewHight))
        ...
        let lineView = UIView(frame: CGRect(x: 0, y: headerViewHight - 1, width: kQueryValue, height: 1))
        //: lineView.backgroundColor = UIColor.init(hex: "#F5F5F5")
        lineView.backgroundColor = UIColor(hex: (String(noti_keyText.suffix(6)) + String(showErrorUrl)))
...
class ViewContentCell: UITableViewCell {
    ...
    lazy var vipIcon: UIImageView = {
        ...
        img.image = UIImage.thoughtImageBundleSizeText(name: (String(notiModelSourceMsg) + String(appContentMsg)))
        //: img.isHidden = true
        img.isHidden = true
        //: return img
        return img
    ...
    lazy var lineLab: UILabel = {
        ...
        lab.backgroundColor = UIColor(hex: (String(noti_keyText.suffix(6)) + String(showErrorUrl)))
        //: return lab
        return lab
        //: }()
    }()
