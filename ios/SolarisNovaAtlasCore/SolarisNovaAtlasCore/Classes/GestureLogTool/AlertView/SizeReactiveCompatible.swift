...
fileprivate let k_insertFormat:[Character] = ["b","t","n","_","s","h","o","w","_"]
fileprivate let main_performText:[Character] = ["a","l","e","r","t","B"]
fileprivate let k_extraMsg:[Character] = ["a","c","k","g","r","o","u","n","d"]

/*: "btn_alertBackground_false" :*/
fileprivate let const_managerId:String = "btn_avideo time result return data"
fileprivate let kCameraColorKey:String = "ackgempty"
fileprivate let showCornerValue:[Character] = ["l","s","e"]
...
public class SizeReactiveCompatible: NSObject {
    /// 通用回调
...
public extension SizeReactiveCompatible {
    /// 适配器回调,用于给适配器参数赋值
    //: public typealias ConfigAlert = ((_ config: ShowAlertConfig) -> Void)
    typealias ConfigAlert = (_ config: CurtilageAlertConfig) -> Void
    ...
    internal class func exceptHeader(title: String? = nil,
                                     //: message: String?  = nil,
                                     message: String? = nil,
                                     ...
                                     config: CurtilageAlertConfig? = nil)
    {
        //: customAlert(title: title,
        buttonEnter(title: title,
                    //: message: message,
                    message: message,
    ...
    @objc internal class func popGreet(title: String? = nil,
    ...
    class func buttonEnter(title: String? = nil,
                           ...
                           config: CurtilageAlertConfig? = nil)
    {
        ...
        let alertView = EqualRangeView(title: title,
        ...
        //: PublisherReactiveCompatible.getWindow().addSubview(alertView)
        PublisherReactiveCompatible.exceptAction().addSubview(alertView)
    ...
    class func obstruct() {
        //: PublisherReactiveCompatible.getWindow().subviews.forEach { (view) in
        PublisherReactiveCompatible.exceptAction().subviews.forEach { view in
            //: if view.isKind(of: TalkingAlertView.self) {
            if view.isKind(of: EqualRangeView.self) {
    ...
    class func surpriseReply(title: String, isTouch: Bool) {
        //: PublisherReactiveCompatible.getWindow().subviews.forEach { (view) in
        PublisherReactiveCompatible.exceptAction().subviews.forEach { view in
            //: if view.isKind(of: TalkingAlertView.self) {
            if view.isKind(of: EqualRangeView.self) {
                //: let cunrenView: TalkingAlertView = view as! TalkingAlertView
                let cunrenView: EqualRangeView = view as! EqualRangeView
                //: cunrenView.changeRightBtn(title: title, isTouch: isTouch)
                cunrenView.atDoing(title: title, isTouch: isTouch)
            }
        }
    }
}
...
public class CurtilageAlertConfig {
    /// 执行动画时间
    ...
    public var maskType: WithMaskType = .color
    /// 阴影
    ...
    public var titleFont: UIFont = .clickLarge(type: .Medium, fontSize: 18)
    /// alert标题字体颜色
    //: public var titleColor  = UIColor.appTitleColor()
    public var titleColor = UIColor.toEvent()
    /// alert信息字体
    //: public var textFont: UIFont = .pingfangFont(type: .Medium, fontSize: 16)
    public var textFont: UIFont = .clickLarge(type: .Medium, fontSize: 16)
    //: public var textColor  = UIColor.appTitleColor()
    public var textColor = UIColor.toEvent()
    ...
    public var buttonFont: UIFont = .clickLarge(type: .Medium, fontSize: 16)
    /// alert按钮字体颜色
    //: public var leftColor  = UIColor.appThemeColor()
    public var leftColor = UIColor.readRemove()
    //: public var rightColor = UIColor.white
    public var rightColor = UIColor.white
    //: public var rightBackgroundImage = "btn_show_alertBackground"
    public var rightBackgroundImage = (String(k_insertFormat) + String(main_performText) + String(k_extraMsg))

    //: public var rightBackgroundImagefalse = "btn_alertBackground_false"
    public var rightBackgroundImagefalse = (String(const_managerId.prefix(5)) + "lertB" + kCameraColorKey.replacingOccurrences(of: "empty", with: "r") + "ound_fa" + String(showCornerValue))
    //: public var rightFont: UIFont = .pingfangFont(type: .Medium, fontSize: 16)
    public var rightFont: UIFont = .clickLarge(type: .Medium, fontSize: 16)
