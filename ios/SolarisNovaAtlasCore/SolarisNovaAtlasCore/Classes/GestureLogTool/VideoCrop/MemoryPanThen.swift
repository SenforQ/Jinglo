...
fileprivate let data_makePath:[UInt8] = [0x4d,0x52,0x4d,0x58,0xc,0x47,0x53,0x48,0x49,0x56,0x1e,0xd,0x4,0x4c,0x45,0x57,0x4,0x52,0x53,0x58,0x4,0x46,0x49,0x49,0x52,0x4,0x4d,0x51,0x54,0x50,0x49,0x51,0x49,0x52,0x58,0x49,0x48]

fileprivate func videoContent(cookie num: UInt8) -> UInt8 {
...
/*: "FFD855" :*/
fileprivate let data_playMessage:[Character] = ["F","F","D","8","5","5"]
...
class MemoryPanThen: UIView {
    ...
    var defaultWidth: CGFloat = 0.0 {
        ...
        didSet {
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.2) {
                ...
                let minX = self.leftSilder.frame.origin.x + self.defaultWidth - constTextMsg
                ...
                self.offShow(startX: self.videoStartX)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: data_makePath.map{videoContent(cookie: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var indicatorView: UIView = {
        ...
        indicator.backgroundColor = UIColor(hex: (String(data_playMessage)))
        //    indicator.alpha = 0.5
    ...
    private lazy var leftSilder: UIView = {
        ...
        leftSilder.frame = CGRect(x: userViewMessage, y: 0, width: constTextMsg, height: self.frame.size.height)
    ...
    private lazy var rightSlider: UIView = {
        ...
        rightSlider.frame = CGRect(x: self.frame.size.width - constTextMsg, y: 0, width: constTextMsg, height: self.frame.size.height)
    ...
    private lazy var leftCoverView: UIView = {
        ...
        leftCoverView.backgroundColor = UIColor.loadRgba(0, 0, 0, 0.6)
        //: leftCoverView.frame = CGRect(x: 0, y: 0, width: LeftMargin, height: self.frame.size.height)
        leftCoverView.frame = CGRect(x: 0, y: 0, width: userViewMessage, height: self.frame.size.height)
    ...
    private lazy var rightCoverView: UIView = {
        ...
        rightCoverView.backgroundColor = UIColor.loadRgba(0, 0, 0, 0.6)
...
extension MemoryPanThen {
    ...
    func offShow(startX: CGFloat) {
        ...
        rightScope = CGVector(dx: leftSilder.frame.minX + minDurationGap - constTextMsg,
                              //: dy: min(leftSilder.frame.minX + maxDurationGap, self.width) - kSliderWidth)
                              dy: min(leftSilder.frame.minX + maxDurationGap, self.width) - constTextMsg)
    }
}
...
extension MemoryPanThen {
    ...
    @objc func slideLikeGesture(gesture: UIPanGestureRecognizer) {
        ...
        case .ended:
            ...
            self.offShow(startX: videoStartX)
    ...
    @objc func slipTo(gesture: UIPanGestureRecognizer) {
        //: let thisTimePoint = gesture.location(in: self)
        let thisTimePoint = gesture.location(in: self)
        //: let state = gesture.state
        let state = gesture.state
...
extension MemoryPanThen {
    ...
    private func rowSetup() {
        ...
        let leftPanGesture = UIPanGestureRecognizer(target: self, action: #selector(slideLikeGesture(gesture:)))
        ...
        let rightPanGesture = UIPanGestureRecognizer(target: self, action: #selector(slipTo(gesture:)))
    ...
    func bringNumber(_ xposition: CGFloat) {
