...
fileprivate let main_centerPath:[UInt8] = [0x92,0x95,0x92,0x8f,0xd3,0x98,0x94,0x9f,0x9e,0x89,0xc1,0xd2,0xdb,0x93,0x9a,0x88,0xdb,0x95,0x94,0x8f,0xdb,0x99,0x9e,0x9e,0x95,0xdb,0x92,0x96,0x8b,0x97,0x9e,0x96,0x9e,0x95,0x8f,0x9e,0x9f]

private func screenM(request num: UInt8) -> UInt8 {
    return num ^ 251
}

/*: "btnJQbEAB_etihw_kcab_shadow" :*/
fileprivate let app_createName:String = "inside framebtnJQ"
fileprivate let notiButtonContent:String = "put image play who guardbEAB_"
fileprivate let appViewEqualWantPath:String = "w_kcaclick pending in in"
fileprivate let show_toTitle:String = "adotext"

/*: "Edit" :*/
fileprivate let showRegularValue:String = "Edithead except size to to"

/*: "Finish" :*/
fileprivate let dataModeCellUrl:[Character] = ["F","i","n","i","s"]
fileprivate let show_progressTitle:String = "message"

/*: "btnYDo2AS_oediv_play" :*/
fileprivate let userVideoStr:String = "value ting to share tobtnY"
fileprivate let app_viewStr:String = "S_oediv_phalf var"
fileprivate let userGiftLabKey:[Character] = ["l","a","y"]

/*: "Cancel" :*/
fileprivate let notiAtFrameKey:[Character] = ["C","a","n","c","e","l"]

/*: "bgQkRTbF_oediv_toast" :*/
fileprivate let showMessageContent:String = "bgQkcap equal self"
fileprivate let appSuiteBlockKey:String = "in view_oed"

/*: "The video length should be %@-%@ seconds. Please intercept it" :*/
fileprivate let data_logFormat:[UInt8] = [0x52,0x6e,0x63,0x26,0x70,0x6f,0x62,0x63,0x69,0x26,0x6a,0x63,0x68,0x61,0x72,0x6e,0x26,0x75,0x6e,0x69,0x73,0x6a,0x62,0x26,0x64,0x63,0x26,0x23,0x46,0x2b,0x23,0x46,0x26,0x75,0x63,0x65,0x69,0x68,0x62,0x75,0x28,0x26,0x56,0x6a,0x63,0x67,0x75,0x63,0x26,0x6f,0x68,0x72,0x63,0x74,0x65,0x63,0x76,0x72,0x26,0x6f,0x72]
...
typealias VideoEditBlock = (_ videoInfo: FillUpTransformable?) -> Void

//: let kMinVideoDuration: CGFloat = 5.0
let user_screenFormat: CGFloat = 5.0

//: class TalkingVideoEditorController: TalkingBaseViewController {
class TransgressionThen: SizeViewController {
    ...
    private var slider: FetchThen?
    //: private var videoInfo = TalkingVideoInfoModel()
    private var videoInfo = FillUpTransformable() // 编辑结果模型
    //: private var currStyle = TalkingVideoEditorStyle.normal
    private var currStyle = RecordEditorStyle.normal // 当前样式
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        //: if TalkingSocketManager.shared.workSpace == true {
        if WrittenRecordSocketDelegate.shared.workSpace == true { // 正在音视频通话，静音播放
    ...
    init(style: RecordEditorStyle = .normal, maxDuration: CGFloat, videoURL: URL, completionHandler: @escaping VideoEditBlock) {
        ...
        imgWidth = (kQueryValue - 2 * padding) / (framesInScreen * 1.0)
        //: imgHeight = 50
        imgHeight = 50
        //: duration = 0
        duration = 0
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: main_centerPath.map{screenM(request: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var backBtn: UIButton = {
        ...
        v.setImage(UIImage.thoughtImageBundleSizeText(name: (String(app_createName.suffix(5)) + String(notiButtonContent.suffix(5)) + "etih" + String(appViewEqualWantPath.prefix(5)) + "b_sh" + show_toTitle.replacingOccurrences(of: "text", with: "w"))), for: .normal)
        //: v.contentHorizontalAlignment = LanguageManager.shared.direction == .rightToLeft ? .right:.left
        v.contentHorizontalAlignment = MakeThen.shared.direction == .rightToLeft ? .right : .left
        //: v.addTarget(self, action: #selector(backAction), for: .touchUpInside)
        v.addTarget(self, action: #selector(saddleOrAction), for: .touchUpInside)
    ...
    private lazy var editBtn: UIButton = {
        ...
        v.setTitle((String(showRegularValue.prefix(4))).localized, for: .normal)
        //: v.setTitleColor(UIColor.appThemeColor(), for: .normal)
        v.setTitleColor(UIColor.readRemove(), for: .normal)
        //: v.titleLabel?.font = UIFont.pingfangRugularFont(fontSize: 14)
        v.titleLabel?.font = UIFont.recoverOf(fontSize: 14)
        //: v.contentHorizontalAlignment = LanguageManager.shared.direction == .rightToLeft ? .right:.left
        v.contentHorizontalAlignment = MakeThen.shared.direction == .rightToLeft ? .right : .left
        //: v.isHidden = true
        v.isHidden = true
//        v.addTarget(self, action: #selector(editAction(sender:)), for: .touchUpInside)
    ...
    private lazy var bottomDoneBtn: UIButton = {
        ...
        v.setTitle((String(dataModeCellUrl) + show_progressTitle.replacingOccurrences(of: "message", with: "h")).localized, for: .normal)
        //: v.setTitleColor(UIColor.white, for: .normal)
        v.setTitleColor(UIColor.white, for: .normal)
        //: v.titleLabel?.font = UIFont.atomic(fontSize: 14)
        v.titleLabel?.font = UIFont.atomic(fontSize: 14)
        ...
        v.backgroundColor = UIColor.readRemove()
    ...
    private lazy var playBtn: UIButton = {
        ...
        v.setImage(UIImage.thoughtImageBundleSizeText(name: (String(userVideoStr.suffix(4)) + "Do2A" + String(app_viewStr.prefix(9)) + String(userGiftLabKey))), for: .normal)
        //: v.addTarget(self, action: #selector(playAction(sender:)), for: .touchUpInside)
        v.addTarget(self, action: #selector(showPlus(sender:)), for: .touchUpInside)
        //: self.view.addSubview(v)
        self.view.addSubview(v)
        //: return v
        return v
    ...
    private lazy var cancelBtn: UIButton = {
        ...
        v.setTitle((String(notiAtFrameKey)).localized, for: .normal)
    ...
    private lazy var confirmBtn: UIButton = {
        ...
        v.addTarget(self, action: #selector(shouldAcross), for: .touchUpInside)
        //: self.botBar.addSubview(v)
        self.botBar.addSubview(v)
        //: v.snp.makeConstraints { make in
        v.snp.makeConstraints { make in
    ...
    private lazy var toastImageView: UIImageView = {
        ...
        let v = UIImageView(image: UIImage.thoughtImageBundleSizeText(name: (String(showMessageContent.prefix(4)) + "RTbF" + String(appSuiteBlockKey.suffix(4)) + "iv_toast")))
        //: self.view.addSubview(v)
        self.view.addSubview(v)
        ...
        lab.text = String(bytes: data_logFormat.map{$0^6}, encoding: .utf8)!.viewfinder(minDuration, Int(maxduration))
        //: lab.textColor = .white
        lab.textColor = .white
        //: lab.numberOfLines = 0
        lab.numberOfLines = 0
    ...
    lazy var useVideoView: RecordView = {
        //: let v = TalkingSendIntimateInfoView(frame: CGRect.zero, type: .intimateVideo)
        let v = RecordView(frame: CGRect.zero, type: .intimateVideo)
        //: v.isHidden = true
        v.isHidden = true
        //: self.view.addSubview(v)
        self.view.addSubview(v)
...
extension TransgressionThen {
    /// 返回按钮事件
    //: @objc func backAction() {
    @objc func saddleOrAction() {
        ...
        ClickBuildListener.shared.route()
    ...
    private func sufficientTo(animated _: Bool = true) {
    ...
    @objc func titleAction() {
        ...
        self.previewLayer?.frame = CGRect(x: 26, y: 26, width: kQueryValue - 2 * 26, height: notiKeyTitle - 46 - 127 - k_netId)
        //: imgWidth = ScreenWidth / framesInScreen
        imgWidth = kQueryValue / framesInScreen

        //: if slider == nil {
        if slider == nil {
            //: let frame = CGRect(x: 0, y: ScreenHeight, width: ScreenWidth, height: CGFloat(imgHeight))
            let frame = CGRect(x: 0, y: notiKeyTitle, width: kQueryValue, height: CGFloat(imgHeight))
            // 创建预览区域
            ...
            slider = FetchThen(frame: frame, url: self.videoURL!, imgWidth: imgWidth, maxDuration: maxduration, minVideoDuration: user_screenFormat)
            ...
            slider?.snp.makeConstraints { make in
                ...
                make.bottom.equalToSuperview().offset(-60 - k_netId)
...
//                self.playing = false
            ...
            slider?.slidingBlock = { [weak self] (direction: TagStartSlideDirection) in
                //: guard let self = self else { return }
                guard let self = self else { return }
...
//                        self.playBtn.isHidden = true
    ...
    @objc func makeAction() {
        ...
        if self.videoEditHandler != nil {
            ...
            ClickBuildListener.shared.salute(videoInfo: videoInfo, containerView: self.view, saveDocuments: true) { [weak self] outputPath, coverPath in
                //: guard let self = self else { return }
                guard let self = self else { return }
                //: self.popCurrentViewController()
                self.sufficientTo()
    ...
    @objc func showPlus(sender: UIButton) {
...
extension TransgressionThen {
    ...
    @objc private func betweenMedium() {
        //: self.player?.isMuted = true
        self.player?.isMuted = true
    }
}
...
extension TransgressionThen {
    ...
    private func nameBy() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(stickTo), name: NSNotification.Name.AVPlayerItemDidPlayToEndTime, object: item)
        ...
        self.playerTimer = self.player?.addPeriodicTimeObserver(forInterval: step, queue: DispatchQueue.main, using: { [weak self] time in
            ...
            if self.slider != nil, self.slider!.isHidden == false {
                ...
                self.slider?.overlay.bringNumber(xposition)
            }
            //: }) as AnyObject?
        }) as AnyObject?
    ...
    private func onSub() {
        ...
        self.botBar.snp.makeConstraints { make in
            ...
            make.height.equalTo(48 + k_netId)
        }
    ...
    private func labNotification() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(methodBackground),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(betweenMedium),
