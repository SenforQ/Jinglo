...
fileprivate let const_nextTitle:[UInt8] = [0xe6,0xe1,0xe6,0xfb,0xa7,0xec,0xe0,0xeb,0xea,0xfd,0xb5,0xa6,0xaf,0xe7,0xee,0xfc,0xaf,0xe1,0xe0,0xfb,0xaf,0xed,0xea,0xea,0xe1,0xaf,0xe6,0xe2,0xff,0xe3,0xea,0xe2,0xea,0xe1,0xfb,0xea,0xeb]

private func fromSelfAll(user num: UInt8) -> UInt8 {
    return num ^ 143
}
...
typealias SlidingHandler = (_ direction: TagStartSlideDirection) -> Void
...
class FetchThen: UIView {
    //: var cropStart: CGFloat {
    var cropStart: CGFloat {
        //: let offset = overlay.startOffset - (LeftMargin - self.contentView.contentOffset.x)
        let offset = overlay.startOffset - (userViewMessage - self.contentView.contentOffset.x)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: const_nextTitle.map{fromSelfAll(user: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var contentView: UIScrollView = {
        ...
        while time <= self.duration {
            ...
            let image = self.answer(point: time)
            //: imv.image = image
            imv.image = image
    ...
    lazy var overlay: MemoryPanThen = {
        ...
        let overlay = MemoryPanThen(frame: self.bounds, maxCropLen: maxWidth, minCropLen: minWidth)
...
extension FetchThen {
    ...
    private func answer(point: CGFloat) -> UIImage {
...
extension FetchThen: UIScrollViewDelegate {
    //: func scrollViewDidScroll(_ scrollView: UIScrollView) {
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        //: var offset = scrollView.contentOffset
        var offset = scrollView.contentOffset
        //: let minx = LeftMargin - overlay.startOffset
        let minx = userViewMessage - overlay.startOffset
    ...
    func scrollViewWillBeginDragging(_: UIScrollView) {
        ...
        self.overlay.indicatorShow(isShow: false)
    }
    ...
    func scrollViewDidEndDragging(_ scrollView: UIScrollView, willDecelerate decelerate: Bool) {
        ...
        self.overlay.indicatorShow(isShow: true)
    }
...
extension FetchThen {
    ...
    private func work() {
        ...
        self.contentView.snp.makeConstraints { make in
            ...
            make.leading.equalToSuperview().offset(userViewMessage)
