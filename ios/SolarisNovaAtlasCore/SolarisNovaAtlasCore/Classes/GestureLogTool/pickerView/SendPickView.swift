...
/*: "Select a date" :*/
fileprivate let appLeadingModelKey:String = "Selectbottom view item"
fileprivate let k_tempName:[Character] = ["e"]

/*: M" :*/
fileprivate let constErrorId:String = "hide"

/*: D" :*/
fileprivate let appLabelValue:String = "d"

/*: Y" :*/
fileprivate let constReloadMakeName:String = "count"
...
class SendPickView: HiddenPickView, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    override func noTip() {
        ...
        titleString = (String(appLeadingModelKey.prefix(6)) + " a dat" + String(k_tempName)).localized
        ...
        popDay(year: defaultYear!, month: defaultMonth!, day: defaultDay!)
    }

    //: func setDefaultDate(year: Int, month: Int, day: Int) {
    func popDay(year: Int, month: Int, day: Int) {
        //: currentYear = year
        currentYear = year
        //: if year == defaultYear {
        if year == defaultYear {
...
//        }
    ...
    override func playButton(button: UIButton) {
        //: super.confirmClick(button: button)
        super.playButton(button: button)
        //: refreshPickViewData()
        noGesture()
        ...
        if let day = selectDay {
            ...
            delegate?.board(year: selectYear!, month: selectMonth!, day: day)
            //: } else {
        } else {
            //: delegate?.pickDateView(year: selectYear!, month: selectMonth!, day: 0)
            delegate?.board(year: selectYear!, month: selectMonth!, day: 0)
        }
    }
        ...
        if isShowDay {
            ...
            if component == 0 {
                ...
                if currentYear == yearSum + minShowYear - 1 {
                    ...
                    return afterColor(year: year, month: month)
                }
                //: } else {
            } else {
                //: return self.yearSum
    ...
    func afterColor(year: Int, month: Int) -> Int {
