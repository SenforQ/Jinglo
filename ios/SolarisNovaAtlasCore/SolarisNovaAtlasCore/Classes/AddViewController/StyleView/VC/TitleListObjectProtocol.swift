...
/*: "Search" :*/
fileprivate let notiMakeFileMsg:[Character] = ["S","e","a","r","c","h"]

/*: "#F5F5F5" :*/
fileprivate let showTableMessage:[Character] = ["#","F","5","F","5","F"]
fileprivate let appErrorId:String = "model"

/*: "Search by UID" :*/
fileprivate let main_interactionStr:String = "let true path max edgeSearch"

/*: "icon_search_sea" :*/
fileprivate let mainImageData:[Character] = ["i"]
fileprivate let notiInfoName:String = "skip make message part viewcon_se"

/*: "Can’t find~" :*/
fileprivate let dataViewStr:String = "Can’tmodel effect time failure case"
fileprivate let showGiftMsg:String = "for index if find~"

/*: "icon_kong_kong_default" :*/
fileprivate let kModelKey:String = "icon_image var equal"
fileprivate let k_bottomSumryText:String = "image rangekong_"
fileprivate let showSucceedImageFormat:[Character] = ["d","e","f","a","u","l","t"]

/*: "Please enter the query UID" :*/
fileprivate let showListValue:String = "equal filterPlea"
fileprivate let showManagerLinePath:String = "size tonter "
fileprivate let appSubmitData:[Character] = ["r","y"," ","U","I","D"]
...
class TitleListObjectProtocol: SizeViewController {
    ...
    private lazy var searchBtn: UIButton = {
        ...
        btn.setTitle((String(notiMakeFileMsg)).localized, for: .normal)
        ...
        btn.setBackgroundImage(UIImage.sinceNext(colors: UIColor.analogDigitalConverterColor(), size: CGSize(width: 72, height: 32)), for: .normal)
        //: btn.layer.cornerRadius = 16
        btn.layer.cornerRadius = 16
        //: btn.clipsToBounds = true
        btn.clipsToBounds = true
        //: btn.addTarget(self, action: #selector(searchButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(tuitionFeeClick), for: .touchUpInside)
        //: btn.alpha = 0.5
        btn.alpha = 0.5
    ...
    private lazy var searchTF: UITextField = {
        ...
        tf.backgroundColor = UIColor(hex: (String(showTableMessage) + appErrorId.replacingOccurrences(of: "model", with: "5")))
        //: tf.textColor = UIColor.appTitleColor()
        tf.textColor = UIColor.toEvent()
        //: tf.font = UIFont.pingfangRugularFont(fontSize: 16)
        tf.font = UIFont.recoverOf(fontSize: 16)
        ...
        tf.placeholder = (String(main_interactionStr.suffix(6)) + " by UID").localized
    ...
    private lazy var unlessEditingView: UIView = {
        ...
        let image = UIImage.thoughtImageBundleSizeText(name: (String(mainImageData) + String(notiInfoName.suffix(6)) + "arch_sea"))
    ...
    private lazy var tableView: UITableView = {
        ...
        tabV.register(StyleView.self, forCellReuseIdentifier: StyleView.className())
    ...
    private lazy var emptyV: EmptyView = {
        ...
        style.TipsTitle = (String(dataViewStr.prefix(5)) + String(showGiftMsg.suffix(6))).localized
        //: style.TipsIcon = "icon_kong_kong_default"
        style.TipsIcon = (String(kModelKey.prefix(5)) + "kong_" + String(k_bottomSumryText.suffix(5)) + String(showSucceedImageFormat))
        ...
        let v = EmptyView(frame: CGRect(x: 0, y: 14 + constGameData, width: kQueryValue, height: notiKeyTitle - (14 + constGameData + mainKeyValue)), style: style)
    ...
    private lazy var reqManager: RecommenceSelectManager = //: return RecommenceSelectManager()
        .init()
    //: }()
}
...
extension TitleListObjectProtocol {
    // 根据uid查询用户
    ...
    private func executeData() {
        ...
        guard searchTF.text?.count ?? 0 > 0 else {
            ...
            self.drawObserver(showMsg: (String(showListValue.suffix(4)) + "se e" + String(showManagerLinePath.suffix(5)) + "the que" + String(appSubmitData)).localized)
            //: return
            return
        }
        ...
        reqManager.imageFinishInput(uid: searchTF.text ?? "") { [weak self] _, _, _ in
            //: ProgressHUD.dismiss()
            DirectThen.cutWith()
...
extension TitleListObjectProtocol {
    ...
    @objc private func tuitionFeeClick() {
        //: req_searchUid()
        executeData()
    }
...
extension TitleListObjectProtocol: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: StyleView.className(), for: indexPath) as! StyleView
        //: let model = reqManager.searchListDataArr[indexPath.row]
        let model = reqManager.searchListDataArr[indexPath.row]
        //: cell.refreshCell(model: model)
        cell.headLive(model: model)
...
extension TitleListObjectProtocol: UITextFieldDelegate {
...
extension TitleListObjectProtocol: JXPagingViewListViewDelegate {
