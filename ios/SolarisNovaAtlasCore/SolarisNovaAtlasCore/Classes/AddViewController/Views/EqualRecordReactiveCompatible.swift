...
fileprivate let data_appName:[UInt8] = [0x6b,0x6c,0x6b,0x76,0x2a,0x61,0x6d,0x66,0x67,0x70,0x38,0x2b,0x22,0x6a,0x63,0x71,0x22,0x6c,0x6d,0x76,0x22,0x60,0x67,0x67,0x6c,0x22,0x6b,0x6f,0x72,0x6e,0x67,0x6f,0x67,0x6c,0x76,0x67,0x66]

private func byLeading(name num: UInt8) -> UInt8 {
    return num ^ 2
}

/*: "bg_medals_podium" :*/
fileprivate let dataLabUrl:String = "else databg_medals"
fileprivate let app_makeId:[Character] = ["_","p","o","d","i","u","m"]

/*: "icon_rank_first_bg" :*/
fileprivate let k_requestValue:String = "icoto"
fileprivate let dataEqualMessage:String = "first_bgstack number view value var"

/*: "icon_rank_first" :*/
fileprivate let show_makeUrl:[Character] = ["i","c","o","n","_","r","a","n","k","_","f","i","r","s","t"]

/*: "icon_ammount_points" :*/
fileprivate let main_valueName:String = "ismalln"
fileprivate let constLargePath:String = "to top addnt_p"
fileprivate let user_beautyName:[Character] = ["o","i","n","t","s"]

/*: "icon_lounge" :*/
fileprivate let mainPhotoData:String = "icon_lvalue let guard container"

/*: "icon_rank_coin" :*/
fileprivate let mainUserValue:String = "error back gapicon_r"
fileprivate let userMemberFormat:String = "imagen"
fileprivate let main_frameFormat:String = "k_coinattribute mini nor size"

/*: "icon_rank_coin2" :*/
fileprivate let noti_targetTitle:String = "value toicon_ra"
fileprivate let constMiniContent:String = "guard self self bar usernk_coin2"

/*: "icon_rank_second" :*/
fileprivate let k_slideImageId:[Character] = ["i","c","o","n","_","r","a","n","k","_","s","e"]
fileprivate let k_withFormat:String = "comodel"

/*: "icon_rank_third" :*/
fileprivate let app_goldUrl:[Character] = ["i","c","o","n","_","r","a","n","k"]
fileprivate let kAccountErrorId:String = "_thirdguard size label error reply"
...
// __CLOSE_PRINT__
//
//  EqualRecordReactiveCompatible.swift
//  AbroadTalking
//
...
class EqualRecordReactiveCompatible: UIView {
    //: var  index = 0
    var index = 0
    ...
    var dataArray: [StylePanMeasurable] = [] {
        //: didSet {
        didSet {
    ...
    override init(frame: CGRect) {
        ...
        super.init(frame: frame)
        //: self.setupSubviews()
        self.technicalitySubviews()
        //: self.setupSubViewsConstraint()
        self.actionConstraint()
        //: self.bindInteraction()
        self.noTotalerchangeConstraint()
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: data_appName.map{byLeading(name: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var medalsView = UIImageView().then {
        ...
        $0.image = UIImage.thoughtImageBundleSizeText(name: (String(dataLabUrl.suffix(9)) + String(app_makeId)))
    }
    ...
    lazy var firstView = SocialReactiveCompatible(frame: .zero, rankType: .firstRankType).then {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(featureGesture))
...
extension EqualRecordReactiveCompatible {
    //: private func bindInteraction() {
    private func noTotalerchangeConstraint() {}

    //: private func updateSubviewsWithData() {
    private func plumb() {
        //: firstView.updateItemWithModel(model: dataArray[0], type: self.index)
        firstView.viewType(model: dataArray[0], type: self.index)
        //: secondView.updateItemWithModel(model: dataArray.count>1 ? dataArray[1]: nil, type: self.index)
        secondView.viewType(model: dataArray.count > 1 ? dataArray[1] : nil, type: self.index)
        //: thirdView.updateItemWithModel(model: dataArray.count>2 ? dataArray[2]: nil, type: self.index)
        thirdView.viewType(model: dataArray.count > 2 ? dataArray[2] : nil, type: self.index)
    }

    //: @objc func registTapAction(tapGesture: UITapGestureRecognizer) {
    @objc func featureGesture(tapGesture: UITapGestureRecognizer) {
...
extension EqualRecordReactiveCompatible {
    ...
    private func technicalitySubviews() {
    ...
    private func actionConstraint() {
        ...
        let firstWidth = (kQueryValue - actualWidth(w: 68)) / 3 + actualWidth(w: 10)
        //: let itemWidth = (ScreenWidth - actualWidth(w: 68)) / 3 - actualWidth(w: 10)
        let itemWidth = (kQueryValue - actualWidth(w: 68)) / 3 - actualWidth(w: 10)
...
class SocialReactiveCompatible: UIView {
    ...
    public init(frame: CGRect, rankType: ItemTo) {
        //: super.init(frame: frame)
        super.init(frame: frame)
        //: self.setupItemViews(type: rankType)
        self.itemEnter(type: rankType)
    }
    ...
    lazy var headPicBgView = UIImageView().then {
        ...
        $0.image = UIImage.thoughtImageBundleSizeText(name: (k_requestValue.replacingOccurrences(of: "to", with: "n") + "_rank_" + String(dataEqualMessage.prefix(8))))
    }
    ...
    lazy var crownImgView = UIImageView().then {
        ...
        $0.image = UIImage.thoughtImageBundleSizeText(name: (String(show_makeUrl)))
    }
    ...
    lazy var nickLabel = UILabel().then {
        ...
        $0.font = .clickLarge(type: .Medium, fontSize: 18)
        //: $0.textAlignment = .center
        $0.textAlignment = .center
    }

    //: lazy var coinButton = TalkingButton().then {
    lazy var coinButton = StartImageButton().then {
        //: $0.isUserInteractionEnabled = false
        $0.isUserInteractionEnabled = false
        //: $0.setImage(UIImage.BundleImageNamed(name: "icon_ammount_points"), for: .normal)
        $0.setImage(UIImage.thoughtImageBundleSizeText(name: (main_valueName.replacingOccurrences(of: "small", with: "co") + "_ammou" + String(constLargePath.suffix(4)) + String(user_beautyName))), for: .normal)
        //: $0.titleLabel?.font = .pingfangFont(type: .Regular, fontSize: 13)
        $0.titleLabel?.font = .clickLarge(type: .Regular, fontSize: 13)
    ...
    lazy var loungeImgV = UIImageView().then {
        ...
        $0.image = UIImage.thoughtImageBundleSizeText(name: (String(mainPhotoData.prefix(6)) + "ounge"))
    }
}
...
extension SocialReactiveCompatible {
    ...
    func viewType(model: StylePanMeasurable?, type: Int) {
        ...
        headPic.beforeRender(urlStr: model!.headPic, placeImg: UIImage.imaginationImageSex(sex: model!.sex))
        ...
        if type == 0 {
            ...
            coinButton.setImage(UIImage.thoughtImageBundleSizeText(name: (String(mainUserValue.suffix(6)) + userMemberFormat.replacingOccurrences(of: "image", with: "a") + String(main_frameFormat.prefix(6)))), for: .normal)
            //: } else {
        } else {
            //: coinButton.setImage(UIImage.BundleImageNamed(name: "icon_rank_coin2"), for: .normal)
            coinButton.setImage(UIImage.thoughtImageBundleSizeText(name: (String(noti_targetTitle.suffix(7)) + String(constMiniContent.suffix(8)))), for: .normal)
        }
    }
}
...
//        var topCrownMargin = actualHeight(h: 4)
        ...
        case .firstRankType:
            ...
            crownImgView.image = UIImage.thoughtImageBundleSizeText(name: (String(show_makeUrl)))
            //: headPic.layer.borderColor = UIColor.RGBA(255, 224, 58, 1).cgColor
            headPic.layer.borderColor = UIColor.loadRgba(255, 224, 58, 1).cgColor
...
//            topCrownMargin = actualHeight(h: 10)
        ...
        case .secondRankType:
            ...
            crownImgView.image = UIImage.thoughtImageBundleSizeText(name: (String(k_slideImageId) + k_withFormat.replacingOccurrences(of: "model", with: "nd")))
            //: headPic.layer.borderColor = UIColor.RGBA(182, 199, 254, 1).cgColor
            headPic.layer.borderColor = UIColor.loadRgba(182, 199, 254, 1).cgColor
        ...
        case .thirdRankType:
            ...
            crownImgView.image = UIImage.thoughtImageBundleSizeText(name: (String(app_goldUrl) + String(kAccountErrorId.prefix(6))))
            //: headPic.layer.borderColor = UIColor.RGBA(251, 166, 109, 1).cgColor
            headPic.layer.borderColor = UIColor.loadRgba(251, 166, 109, 1).cgColor
            //: headPic.layer.cornerRadius = actualWidth(w: 31)
            headPic.layer.cornerRadius = actualWidth(w: 31)
            //: break
        }
