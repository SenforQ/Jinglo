...
fileprivate let show_errorKey:[UInt8] = [0xaf,0xa8,0xaf,0xb2,0xee,0xa5,0xa9,0xa2,0xa3,0xb4,0xfc,0xef,0xe6,0xae,0xa7,0xb5,0xe6,0xa8,0xa9,0xb2,0xe6,0xa4,0xa3,0xa3,0xa8,0xe6,0xaf,0xab,0xb6,0xaa,0xa3,0xab,0xa3,0xa8,0xb2,0xa3,0xa2]

private func makeTop(share num: UInt8) -> UInt8 {
    return num ^ 198
}

/*: "icon_rank_coin" :*/
fileprivate let user_minName:[UInt8] = [0xd6,0xdc,0xd0,0xd1,0xe0,0xcd,0xde,0xd1,0xd4,0xe0,0xdc,0xd0,0xd6,0xd1]

private func toolDirection(model num: UInt8) -> UInt8 {
    return num ^ 191
}

/*: "icon_lounge" :*/
fileprivate let k_imageDataValue:[Character] = ["i","c","o","n","_","l","o","u","n","g","e"]

/*: "icon_rank_coin2" :*/
fileprivate let notiBlockName:String = "icostyle"
fileprivate let k_fromData:String = "ousern2"
...
class SumimateThen: UITableViewCell {
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        self.skinColorConstraint()
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: show_errorKey.map{makeTop(share: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var rankLabel = UILabel().then {
        ...
        $0.textColor = UIColor.hockeyPlayer()
        //: $0.font = .pingfangFont(type: .Semibold, fontSize: 20)
        $0.font = .clickLarge(type: .Semibold, fontSize: 20)
    }
    ...
    lazy var nickLabel = UILabel().then {
        ...
        $0.textColor = UIColor.toEvent()
        //: $0.font = .pingfangFont(type: .Medium, fontSize: 16)
        $0.font = .clickLarge(type: .Medium, fontSize: 16)
    ...
    lazy var coinIconBtn = StartImageButton().then {
        //: $0.setImage(UIImage.BundleImageNamed(name: "icon_rank_coin"), for: .normal)
        $0.setImage(UIImage.thoughtImageBundleSizeText(name: String(bytes: user_minName.map{toolDirection(model: $0)}, encoding: .utf8)!), for: .normal)
        //: $0.setTitleColor(UIColor.white, for: .normal)
        $0.setTitleColor(UIColor.white, for: .normal)
        //: $0.titleLabel?.font = .pingfangFont(type: .Medium, fontSize: 15)
        $0.titleLabel?.font = .clickLarge(type: .Medium, fontSize: 15)
        ...
        $0.setBackgroundImage(UIImage.sinceNext(colors: UIColor.analogDigitalConverterColor(), size: CGSize(width: actualWidth(w: 80), height: actualWidth(w: 33))), for: .normal)
    ...
    lazy var loungeImgV = UIImageView().then {
        ...
        $0.image = UIImage.thoughtImageBundleSizeText(name: (String(k_imageDataValue)))
    }
}
...
extension SumimateThen {
    //: func updateRankCell(model: SocialRankItemModel, index: Int) {
    func makeDownFrom(model: StylePanMeasurable, index: Int) {
        ...
        headPic.putBackThroughFinish(urlStr: model.headPic)
    ...
    func specify(type: Int) {
        //: let imageStr = type == 0 ? "icon_rank_coin":"icon_rank_coin2"
        let imageStr = type == 0 ? String(bytes: user_minName.map{toolDirection(model: $0)}, encoding: .utf8)! : (notiBlockName.replacingOccurrences(of: "style", with: "n") + "_rank_c" + k_fromData.replacingOccurrences(of: "user", with: "i"))
        //: coinIconBtn.setImage(UIImage.BundleImageNamed(name: imageStr), for: .normal)
        coinIconBtn.setImage(UIImage.thoughtImageBundleSizeText(name: imageStr), for: .normal)
    }
}
...
extension SumimateThen {
    ...
    private func skinColorConstraint() {
