...
extension PanTool {
    ...
    func submit(_ frontCamera: Bool) {
        ...
        self.expensiveness(front: frontCamera)
    }
    
    /// 摄像头开启与关闭
    /// - Parameter openCamera: 是否开启
    //: func useCamera(_ openCamera: Bool) {
    func bigFrom(_ openCamera: Bool) {
        ...
        self.putBackCameraDoingOpen(isOpen: openCamera)
    }
    
    /// 更改视频采集渲染展示位置
    //: func changeRenderView() {
    func scintillationNameView() {
        ...
        DispatchQueue.main.async {
            ...
            self.buildTo(smallWindow: self.infoModel?.renderPostion != .selfOnBg)
...
extension PanTool {
    ...
    private func expensiveness(front: Bool) {
    ...
    private func putBackCameraDoingOpen(isOpen: Bool) {
        //: guard SenseTime_Use == true else {
        guard const_useId == true else {
            //: if isOpen {
            if isOpen {
...
extension PanTool {
    ...
    func engraftSettings() {
        // 监听 App 切后台
        //: NotificationCenter.default.addObserver(self,
        NotificationCenter.default.addObserver(self,
                                               //: selector: #selector(appDidEnterBackground(notif:)),
                                               selector: #selector(betweenComponent(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(addRecord(notif:)),
    ...
    @objc private func betweenComponent(notif: Notification?) {
        //: if notif == nil {
        if notif == nil {
            // 静音
    ...
    @objc private func addRecord(notif: Notification?) {
        //: if notif == nil { 
    ...
    @objc private func write(notif: Notification) {
    ...
    @objc func train(notif: Notification) {
