...
fileprivate let data_giftUrl:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "iosSvgaEffect" :*/
fileprivate let notiMakeTextKey:String = "case image taskiosSvg"

/*: "iosEffect" :*/
fileprivate let notiUserName:String = "iomoment"
fileprivate let show_tingKey:[Character] = ["E","f","f","e","c","t"]

/*: "entityId" :*/
fileprivate let kDefineStr:[Character] = ["e","n","t","i","t","y"]
fileprivate let notiViewUrl:[Character] = ["I","d"]
...
class VoiceOverAnimatView: UIView {
    //: var currentEffectMsgModel: TalkingAnimatMsgModel?
    var currentEffectMsgModel: StateHandyJSON?
    ...
    var animatedView: ParadigmThen?

    //: open weak var delegate: AnimatViewStepProtocol?
    open weak var delegate: AspectSizeReactiveCompatible?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: data_giftUrl.reversed(), encoding: .utf8)!)
    }
...
extension VoiceOverAnimatView {
    //: func playAnimation(infoDic: Dictionary<String, Any>) {
    func collectionAt(infoDic: [String: Any]) {
        //: let dict = infoDic
        let dict = infoDic
        //: if dict.keys.isEmpty == false {
        if dict.keys.isEmpty == false {
            //: var iosEffect: String = dict["iosSvgaEffect"] as! String
            var iosEffect: String = dict[(String(notiMakeTextKey.suffix(6)) + "aEffect")] as! String
            //: if iosEffect.isEmptyString {
            if iosEffect.isEmptyString {
                //: iosEffect = dict["iosEffect"] as! String
                iosEffect = dict[(notiUserName.replacingOccurrences(of: "moment", with: "s") + String(show_tingKey))] as! String
            }

            //: let entityId = dict["entityId"] as! Int
            let entityId = dict[(String(kDefineStr) + String(notiViewUrl))] as! Int
            ...
            if iosEffect.isEmptyString == false {
                ...
                let successs = have(msgModel: effectMsgModel)
            }
        }
    }
    ...
    func have(msgModel: Any?) -> Bool {
        ...
        if msgModel is StateHandyJSON {
            ...
            if giftMsgModel.animationTimes > 1 {
                ...
                tmpGiftMsgModel.setDismissAfter(giftNum: 1) // 礼物个数设置为1，每次动效显示
                ...
                var repeatEfftect = StateHandyJSON()
            ...
            if giftMsgModel.showType == DogTagEquatable.myStery.rawValue || giftMsgModel.showType == DogTagEquatable.king.rawValue {
        ...
        if self.currentEffectMsgModel != nil || paused {
            ...
            if effectMsgModel?.top ?? false && self.effectMsgModelArray.count > 0 {
                ...
                for i in 0 ..< self.effectMsgModelArray.count {
                    ...
                    let aModel: StateHandyJSON = self.effectMsgModelArray[i] as! StateHandyJSON
                    //: if aModel.top == false {
                    if aModel.top == false {
                        //: index = i
                        index = i
                ...
                if index == -1 {
                    ...
                    self.effectMsgModelArray.add(effectMsgModel ?? StateHandyJSON())
                    //: } else {
                } else {
                    //: self.effectMsgModelArray.insert(effectMsgModel ?? TalkingAnimatMsgModel.init(), at: index)
                    self.effectMsgModelArray.insert(effectMsgModel ?? StateHandyJSON(), at: index)
                }
            ...
            computerSimulation(effectMsgModel: effectMsgModel!)
        }
...
extension VoiceOverAnimatView {
    ...
    func numberInput() {
        ...
        if self.currentEffectMsgModel == nil {
            ...
            if self.effectMsgModelArray.count > 0 {
                ...
                guard let removeModel = messageModel() else { return }
                //: firstShow(effectMsgModel: removeModel)
                computerSimulation(effectMsgModel: removeModel)
            }
            //: } else {
        } else {
            //: self.animatedView?.resumeAnimation()
            self.animatedView?.positionPrompt()
        }
    ...
    func messageModel() -> StateHandyJSON? {
        //: if (self.effectMsgModelArray.count == 0) {
        if self.effectMsgModelArray.count == 0 {
            //: return TalkingAnimatMsgModel.init()
            return StateHandyJSON()
        }
        ...
        if effectMsgModel is StateHandyJSON {
            //: let lastEffectMsgModel = effectMsgModel as! TalkingAnimatMsgModel
            let lastEffectMsgModel = effectMsgModel as! StateHandyJSON
        ...
        return effectMsgModel as? StateHandyJSON
    }
}
...
extension VoiceOverAnimatView {
    ...
    func computerSimulation(effectMsgModel: StateHandyJSON) {
        ...
        self.delegate?.clipToView(effectView: self, obj: effectMsgModel)

        //: TalkingGiftAnimatTool.shared.loadEffectData(effectMsgModel: effectMsgModel, completion: { [weak self] success, giftModel in
        SendStartThen.shared.anLoad(effectMsgModel: effectMsgModel, completion: { [weak self] success, giftModel in
            ...
            self.occurrentName(effectMsgModel: effectMsgModel, success: success, effectModel: giftModel ?? BlockAnimatModel())
            //: })
        })
    }
    ...
    func occurrentName(effectMsgModel: StateHandyJSON, success: Bool, effectModel: BlockAnimatModel) {
        // 资源下载完成
        //: self.delegate?.animatViewWithDidEndLoadingResource(effectView: self, obj: effectMsgModel, success: success)
        self.delegate?.topSuccess(effectView: self, obj: effectMsgModel, success: success)
        ...
        if success {
            ...
            if self.paused {
                ...
                if self.currentEffectMsgModel?.eid == effectMsgModel.eid, self.currentEffectMsgModel?.effectUrl == effectMsgModel.effectUrl {
                    ...
                    byProduct(effectMsgModel: effectMsgModel, model: effectModel)
                    //: return
                    return
                }
            }
    ...
    func formationInsideSimulation(effectMsgModel: StateHandyJSON, effectModel: BlockAnimatModel) {
        //: if self.animatedView != nil {
        if self.animatedView != nil {
            //: self.animatedView?.stopAnimating()
            self.animatedView?.titleAnimating()
            //: self.animatedView?.cleanAnimating()
            self.animatedView?.conversationPrice()
        ...
        className = SendStartThen.shared.listName(oldName: className ?? "")
        ...
        let realClass = AnimatedViewClass as! ParadigmThen.Type
...
extension VoiceOverAnimatView: QueryThen {
    ...
    func library(effectItemView: ParadigmThen, success: Bool) {
        ...
        if !success {
            ...
            doInsertSuccess(success: success)
        }
        ...
        self.delegate?.asUserSuccess(effectView: self, obj: effectItemView.effectMsgModel ?? StateHandyJSON(), success: success)
    }
    ...
    func can(effectItemView: ParadigmThen) {
        ...
        self.delegate?.adjudicate(effectView: self, obj: effectMsgModel!)
    }
}
...
extension VoiceOverAnimatView {
    ...
    func singingVoice() {
        ...
        if animatedView != nil {
            //: self.animatedView?.stopAnimating()
            self.animatedView?.titleAnimating() // stop animation
            //: self.animatedView?.cleanAnimating()
            self.animatedView?.conversationPrice() // clean animation data
    ...
    func doInsertSuccess(success: Bool) {
        //: cleanData()
        singingVoice()
        ...
        if self.effectMsgModelArray.count > 0, self.needStopAll == false {
            ...
            let effectMsgModel = messageModel()
