...
/*: "id" :*/
fileprivate let constTapName:String = "label"

/*: "num" :*/
fileprivate let user_listUrl:String = "NUM"
...
@objc public enum DogTagEquatable: Int {
    //: case normal  = 1
    case normal = 1 // 普通礼物
    //: case myStery = 2
    case myStery = 2 // 盲盒礼物
...
struct ListHandyJSON: HandyJSON {
    //: init() {
    init() {}
    ...
    var animationTimes = 0
    //: var showType = 0
    var showType = 0 /// DogTagEquatable
    ...
    mutating func setDismissAfter(giftNum: Int) {
        //: self.num = giftNum
        self.num = giftNum
    }
}
...
struct RubricAnimatModel: HandyJSON {
    //: init() {
    init() {}
    ...
    mutating func mapping(mapper: HelpingMapper) {
        ...
        mapper <<<
            //: self.giftNum <-- "num"
            self.giftNum <-- (user_listUrl.lowercased())
    }
}
