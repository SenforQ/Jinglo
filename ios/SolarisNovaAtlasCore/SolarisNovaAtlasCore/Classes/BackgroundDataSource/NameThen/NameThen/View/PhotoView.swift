...
/*: "#ffffff" :*/
fileprivate let constToFormat:String = "#fimage frame"
fileprivate let app_nameMessage:String = "FFFFF"

/*: "/dist/loungePlus/index.html" :*/
fileprivate let mainGiftMsg:String = "background let/dist"
fileprivate let const_viewData:String = "ngePlusmake view"
fileprivate let k_photoOpDirectionMessage:String = "/ilab up let list"
fileprivate let main_imageKey:String = "ndex.htmlview add type self session"

/*: "https://" :*/
fileprivate let k_thoughtTitle:String = "HTT"
fileprivate let notiMessageMsg:String = "let model in homeps://"

/*: "http://" :*/
fileprivate let k_postStrokeUrl:String = "color showhttp://"
...
class PhotoView: UIView {
    ...
    let Broadcast_TextColor = (String(constToFormat.prefix(2)) + app_nameMessage.lowercased())
    //: let Broadcast_ContentFont: UIFont = UIFont.atomic(fontSize: 12)
    let Broadcast_ContentFont: UIFont = .atomic(fontSize: 12)
    ...
    convenience init(model: SendTransformable?) {
        //: self.init()
        self.init()
    ...
    private func currentModel() {
        ...
        if danumuMsgModel.content.count > 0 {
            ...
            var mutableAtt = parse.elementIndex(withFontSize: 12, insert: "", atIndex: 0)
            // 限制字符长度
    ...
    func makeCustom(withUrl url: String) {
        //: self.backgroundImageView.setUrlImage(urlStr: url) {[weak self] result, image in
        self.backgroundImageView.beforeRender(urlStr: url) { [weak self] _, image in
            ...
            if let image {
                ...
                if image.size.height > self.Broadcast_HeightOfBackgroundImageView {
                    ...
                    newImage = image.power(with: CGSize(width: targetWidth, height: self.Broadcast_HeightOfBackgroundImageView))
                }
            }
    ...
    var danumuMsgModel: SendTransformable? {
        //: didSet {
        didSet {
            //: configModel()
            currentModel()
    ...
    var totalTime: CGFloat {
        ...
        return (kQueryValue + widthValue) * Broadcast_SpendTime / kQueryValue
    }

    //: var totalTimeOfLine: CGFloat {
    var totalTimeOfLine: CGFloat {
        //: return widthValue * Broadcast_LineSpendTime / ScreenWidth
        return widthValue * Broadcast_LineSpendTime / kQueryValue
    }

    //: var hide2ShowTime: CGFloat {
    var hide2ShowTime: CGFloat {
        //: return widthValue * Broadcast_SpendTime / ScreenWidth
        return widthValue * Broadcast_SpendTime / kQueryValue
    }
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(showAction(_:)))
        //: return t
        return t
        //: }()
    }()
    ...
    lazy var contentLabel: UILabel = {
        //: let l = UILabel()
        let l = UILabel()
        //: l.font = UIFont.atomic(fontSize: 12)
        l.font = UIFont.atomic(fontSize: 12)
        //: l.textColor = .white
        l.textColor = .white
        //: l.numberOfLines = 1
...
extension PhotoView {
    ...
    @objc func showAction(_: UITapGestureRecognizer) {
        ...
        //: case "1": // 个人中心
        case "1": // 个人中心
            //: RangePushManager.share.func__pushToUserDetailVC(uid: self.danumuMsgModel?.jumpKey)
            RangePushManager.share.clickOfUid(uid: self.danumuMsgModel?.jumpKey)

        //: case "2": // 直播间
        case "2": // 直播间
            //: if let uid = self.danumuMsgModel?.jumpKey {
            if let uid = self.danumuMsgModel?.jumpKey {
                //: RangePushManager.share.func_audiencePushToLiveRoomVC(uid: uid, enterType: .unknown)
                RangePushManager.share.titleAllList(uid: uid, enterType: .unknown)
            }
        ...
        case "3": // H5页面
            ...
            if let url = self.danumuMsgModel?.jumpKey {
                ...
                if url.contains((String(mainGiftMsg.suffix(5)) + "/lou" + String(const_viewData.prefix(7)) + String(k_photoOpDirectionMessage.prefix(2)) + String(main_imageKey.prefix(9)))) {
                    //: RangePushManager.share.pushOn()
                    RangePushManager.share.pushOn()
                    //: return
                    return
                }
                //: let httpsUrl = PunctuateThen.share.appConfigMode.urlH5Domain+url
                let httpsUrl = PunctuateThen.share.appConfigMode.urlH5Domain + url
                //: let httpUrl = httpsUrl.replacingOccurrences(of: "https://", with: "http://")
                let httpUrl = httpsUrl.replacingOccurrences(of: (k_thoughtTitle.lowercased() + String(notiMessageMsg.suffix(5))), with: (String(k_postStrokeUrl.suffix(7))))
                //: RangePushManager.share.func__pushToWebVC(urlStr: httpUrl)
                RangePushManager.share.userBlade(urlStr: httpUrl)
            }
