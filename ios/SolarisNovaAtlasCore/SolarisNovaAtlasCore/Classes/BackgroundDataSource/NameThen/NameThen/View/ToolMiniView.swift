...
/*: "99+" :*/
fileprivate let show_netUrl:String = "99+"

/*: "init(coder:) has not been implemented" :*/
fileprivate let constMFlushMsg:[UInt8] = [0xdb,0xdc,0xdb,0xc6,0x9a,0xd1,0xdd,0xd6,0xd7,0xc0,0x88,0x9b,0x92,0xda,0xd3,0xc1,0x92,0xdc,0xdd,0xc6,0x92,0xd0,0xd7,0xd7,0xdc,0x92,0xdb,0xdf,0xc2,0xde,0xd7,0xdf,0xd7,0xdc,0xc6,0xd7,0xd6]

private func dataImage(equal num: UInt8) -> UInt8 {
    return num ^ 178
}

/*: "#2DF2FF" :*/
fileprivate let userPointName:[Character] = ["#","2","D","F","2","F","F"]

/*: "Live" :*/
fileprivate let dataLargeFormat:[Character] = ["L","i","v","e"]

/*: "img_home_shadow_icon" :*/
fileprivate let k_timeTitle:[Character] = ["i","m","g","_","h","o"]
fileprivate let main_playerKey:[Character] = ["m","e","_","s"]
fileprivate let main_labelStr:[Character] = ["h","a","d","o","w","_","i","c","o","n"]

/*: "get json error" :*/
fileprivate let mainTagKey:[Character] = ["g","e","t"," ","j","s","o","n"," ","e"]
fileprivate let data_productFormat:String = "modelmodelomodel"

/*: "#FF2348" :*/
fileprivate let user_observerContentIconPath:String = "collection"
fileprivate let const_addDirectionStr:String = "ff2348"

/*: "#35C759" :*/
fileprivate let mainObjectId:[Character] = ["#","3","5","C","7","5","9"]

/*: "mfBean" :*/
fileprivate let notiBrightSampleFormat:String = "dark background textmfBean"

/*: "%.1f" :*/
fileprivate let kWriteName:[Character] = ["%",".","1","f"]

/*: "points\n≈ $" :*/
fileprivate let constNoseText:String = "poindeed"
fileprivate let dataSourceKey:[Character] = ["\n","≈"," ","$"]

/*:   :*/
fileprivate let show_phaseArcText:String = "array"
...
public var k_closeName: Int {
    //: return PunctuateThen.share.appConfigMode.countdown1 + PunctuateThen.share.appConfigMode.countdown2
    return PunctuateThen.share.appConfigMode.countdown1 + PunctuateThen.share.appConfigMode.countdown2
}

//: class TalkingLiveMiniView: TalkingBaseMiniView {
class ToolMiniView: ShortListThen {
    // 未检测出人脸时长（间隔1秒）
    ...
    var noFaceTime: Int = 0 {
        ...
        didSet {
            ...
            if noFaceTime >= PunctuateThen.share.appConfigMode.countdown1 { // 展示倒计时
                ...
                svgaNumLab.text = "\(k_closeName - noFaceTime)"
                //: } else {
            } else {
                //: refreshMiniView()
                writtenAccount()
    ...
    override init(frame: CGRect) {
        ...
        //: if Double(PunctuateThen.share.loginUserMode.mf_bean) > 0 {
        if Double(PunctuateThen.share.loginUserMode.mf_bean) > 0 {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: constMFlushMsg.map{dataImage(equal: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var liveStatusImgV: StartImageButton = {
        ...
        let image = UIImage.textDown(color: UIColor(hex: (String(userPointName)))!, size: CGSize(width: 8, height: 8))
        //: img.setImage(image.shareOutput(), for: .normal)
        img.setImage(image.shareOutput(), for: .normal)
        //: img.setTitle("Live".localized, for: .normal)
        img.setTitle((String(dataLargeFormat)).localized, for: .normal)
        //: img.isHidden = true
        img.isHidden = true
        //: self.addSubview(img)
        self.addSubview(img)
    ...
    private lazy var shadowImg: UIImageView = {
        ...
        img.image = UIImage.thoughtImageBundleSizeText(name: (String(k_timeTitle) + String(main_playerKey) + String(main_labelStr)))
        //: img.layer.cornerRadius = 8
        img.layer.cornerRadius = 8
        //: img.layer.masksToBounds = true
        img.layer.masksToBounds = true
    ...
    private lazy var svgaPlayer: SVGAPlayer = {
        ...
        do {
            ...
            printLog(message: (String(mainTagKey) + data_productFormat.replacingOccurrences(of: "model", with: "r")))
        }
        //: return player
        return player
        //: }()
    ...
    private lazy var svgaNumLab: UILabel = {
        //: let lab = UILabel()
        let lab = UILabel()
        //: lab.font = UIFont.atomic(fontSize: 20)
        lab.font = UIFont.atomic(fontSize: 20)
        //: lab.textAlignment = .center
        lab.textAlignment = .center
        //: lab.textColor = UIColor(hex: "#FF2348")
        lab.textColor = UIColor(hex: (user_observerContentIconPath.replacingOccurrences(of: "collection", with: "#") + const_addDirectionStr.uppercased()))
        //: self.addSubview(lab)
        self.addSubview(lab)
        //: return lab
        return lab
    ...
    private lazy var msgCountLab: UILabel = {
        ...
        lab.backgroundColor = UIColor(hex: (String(mainObjectId)))
        //: lab.font = UIFont.pingfangFont(type: .Medium, fontSize: 11)
        lab.font = UIFont.clickLarge(type: .Medium, fontSize: 11)
        //: lab.textColor = .white
        lab.textColor = .white
        //: lab.textAlignment = .center
        lab.textAlignment = .center
...
extension ToolMiniView {
    /// 构建视频小窗口
    //: static func buildLiveMiniView() -> TalkingLiveMiniView {
    static func finishMini() -> ToolMiniView {
        //: let view = TalkingLiveMiniView(frame: .zero)
        let view = ToolMiniView(frame: .zero)
        //: let position = TalkingSocketManager.shared.liveMiniPosition
        let position = WrittenRecordSocketDelegate.shared.liveMiniPosition
    ...
    func writtenAccount() {
        ...
        liveStatusImgV.isHidden = !LogLiveManager.anUp().isLive
...
extension ToolMiniView {
    ...
    private func countercalate() {
        //: NotificationCenter.default.addObserver(self,
        NotificationCenter.default.addObserver(self,
                                               //: selector: #selector(pointchange(notification:)),
                                               selector: #selector(jumpImage(notification:)),
    ...
    @objc private func jumpImage(notification: NSNotification) {
        //: let userinfo = notification.userInfo as! [String: AnyObject]
        let userinfo = notification.userInfo as! [String: AnyObject]
        //: guard let mfBean = userinfo["mfBean"] as? Double else { return }
        guard let mfBean = userinfo[(String(notiBrightSampleFormat.suffix(6)))] as? Double else { return }
        //: setPointText(mfBean: mfBean)
        length(mfBean: mfBean)
    }

    //: func setPointText(mfBean: Double) {
    func length(mfBean: Double) {
        //: let nowBean = PunctuateThen.share.loginUserMode.mf_bean + mfBean
        let nowBean = PunctuateThen.share.loginUserMode.mf_bean + mfBean
        //: PunctuateThen.share.loginUserMode.mf_bean = nowBean
        PunctuateThen.share.loginUserMode.mf_bean = nowBean
        ...
        mfBeanLB.text = "\(Int(nowBean))\((constNoseText.replacingOccurrences(of: "deed", with: "ts") + String(dataSourceKey)).localized) \(divisor)"
