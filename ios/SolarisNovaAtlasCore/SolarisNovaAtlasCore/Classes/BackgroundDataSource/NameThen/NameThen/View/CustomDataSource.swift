...
fileprivate let k_onViewModelMsg:[UInt8] = [0xd4,0xd3,0xd4,0xc9,0x95,0xde,0xd2,0xd9,0xd8,0xcf,0x87,0x94,0x9d,0xd5,0xdc,0xce,0x9d,0xd3,0xd2,0xc9,0x9d,0xdf,0xd8,0xd8,0xd3,0x9d,0xd4,0xd0,0xcd,0xd1,0xd8,0xd0,0xd8,0xd3,0xc9,0xd8,0xd9]

private func titleData(info num: UInt8) -> UInt8 {
    return num ^ 189
}
...
@objc protocol MakeObjectProtocol: NSObjectProtocol {
    ...
    @objc optional func screenName()

    //: @objc optional func endScroll()
    @objc optional func cellScroll()
}

//: class TalkingDanmuMsgListTableView: UIView {
class CustomDataSource: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_onViewModelMsg.map{titleData(info: $0)}, encoding: .utf8)!)
    }
    ...
    override func layoutSubviews() {
        ...
        setupTextRestrictionConstraint()
    }
...
extension CustomDataSource {
    ...
    func rowTimer() {
        ...
        refreshTimer = Timer.scheduledTimer(timeInterval: reloadTimeSpan, target: self, selector: #selector(requestTip), userInfo: nil, repeats: true)
    }
...
extension CustomDataSource {
    ...
    func atItem(msgModel: PublisherTransformable?) {
    ...
    func toPerform() {
        ...
        if AllHeight > main_jumpName {
            //: if self.msgTableView.height < MsgTableViewHeight {
            if self.msgTableView.height < main_jumpName {
                ...
                self.msgTableView.height = main_jumpName
            }
            //: } else {
        } else {
            //: var frame = self.msgTableView.frame
            ...
            frame.origin.y = main_jumpName - AllHeight
        ...
        liveTo(animated: false)
    }

    /** 执行插入动画并滚动 */
    //: func scrollToBottom(animated: Bool) {
    func liveTo(animated: Bool) {
...
extension CustomDataSource: UITableViewDelegate, UITableViewDataSource {
    ...
    func someProfilePage(_ scrollView: UIScrollView) {
        //: self.delegate?.endScroll?()
        self.delegate?.cellScroll?()
    ...
    func tableView(_: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        return msgModel.msgHeight + Double(dataBarInfoRecordTitle)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = ParentBassCell.quantityeractionTableLoad(tableView: tableView, msg: msgModel, indexPath: indexPath)
...
extension CustomDataSource {
    ...
    func setupTextRestrictionConstraint() {
