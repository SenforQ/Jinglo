...
fileprivate let user_norPath:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "move" :*/
fileprivate let main_soundUrl:String = "mocomment"
...
class TagReactiveCompatible: UIView {
    ...
    var danumuMsgArray: [SendTransformable] = []
    //: var danumuItemViewArray: [TalkingFloatScreenItemView] = []
    var danumuItemViewArray: [PhotoView] = []

    //: let TapAction: PublishSubject<TalkingFloatingScreenModel?> = PublishSubject()
    let TapAction: PublishSubject<SendTransformable?> = PublishSubject()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: user_norPath.reversed(), encoding: .utf8)!)
    }
    ...
    func thenEffect(_ danumuMsgModel: SendTransformable?) {
    ...
    @objc func pointInTime(_: UITapGestureRecognizer) {}
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(pointInTime(_:)))
        //: t.delegate = self
        t.delegate = self
        //: return t
        return t
...
extension TagReactiveCompatible: UIGestureRecognizerDelegate {
    ...
    func saveEqualFirstborn(for danumuMsgModel: SendTransformable?, isFirst: Bool) {
        //: let itemView = TalkingFloatScreenItemView(model: danumuMsgModel)
        let itemView = PhotoView(model: danumuMsgModel)
        ...
        if let anim = POPBasicAnimation(propertyNamed: kPOPViewFrame) {
            ...
            itemView.pop_add(anim, forKey: (main_soundUrl.replacingOccurrences(of: "comment", with: "ve")))
    ...
    func talkChannel(_ isFirst: Bool) {
        ...
        if danumuMsgArray.count > 0 {
            ...
            if let model {
                ...
                saveEqualFirstborn(for: model, isFirst: isFirst)
