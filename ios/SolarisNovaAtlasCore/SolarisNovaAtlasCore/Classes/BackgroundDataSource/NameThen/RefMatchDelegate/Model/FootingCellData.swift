...
/*: ：" :*/
fileprivate let data_textTitle:String = "："

/*: "#BAFBFF" :*/
fileprivate let mainGiftMsg:[Character] = ["#","B","A","F","B","F","F"]
...
class FootingCellData: NSObject {
    ...
    func componentVideoModel(model: TagHandyJSON) -> TagHandyJSON {
        //: self.model = model
        self.model = model
        /// 普通文本消息
        //: if self.model.messageType == 1 {
    ...
    func pushSend() {
        ...
        let paraStyle = bottomFrom()
        //: paraStyle.lineSpacing = 3.0
        paraStyle.lineSpacing = 3.0 // 行间距
        // 名字
        //: let firstStr = "\(self.model.nickname)："
        let firstStr = "\(self.model.nickname)："
        //: let name = getAttributed(text: firstStr, font: UIFont.atomic(fontSize: 14), color: UIColor.init(hex: "#BAFBFF")!)
        let name = comeUpon(text: firstStr, font: UIFont.atomic(fontSize: 14), color: UIColor(hex: (String(mainGiftMsg)))!)
        // 内容
        //: let content = getAttributed(text: self.model.content, font: UIFont.atomic(fontSize: 14), color: UIColor.white)
        let content = comeUpon(text: self.model.content, font: UIFont.atomic(fontSize: 14), color: UIColor.white)
        //: name.append(content)
        name.append(content)
        ...
        pathFor(attribText: self.model.msgAttribText)
    }
    ...
    func trans() {
        ...
        let name = comeUpon(text: firstStr, font: UIFont.atomic(fontSize: 14), color: UIColor(hex: (String(mainGiftMsg)))!)
        // 内容
        //: let content = getAttributed(text: self.model.transContent, font: UIFont.atomic(fontSize: 14), color: UIColor.white)
        let content = comeUpon(text: self.model.transContent, font: UIFont.atomic(fontSize: 14), color: UIColor.white)
        //: name.append(content)
        name.append(content)
        //: name.yy_paragraphStyle = paraStyle
        name.yy_paragraphStyle = paraStyle
    ...
    func bottomFrom() -> NSMutableParagraphStyle {
    ...
    func comeUpon(text: String, font: UIFont, color: UIColor) -> NSMutableAttributedString {
...
//            attribute.yy_writingDirection = [3]
        ...
        attribute.yy_paragraphStyle = bottomFrom()
        //: return attribute
        return attribute
    }
    ...
    func pathFor(attribText: NSMutableAttributedString) {
        // 距离左边8  距离右边也为8
        //: let maxWidth = VideoMsgTableViewWidth - 36
        let maxWidth = main_intimateKey - 36
