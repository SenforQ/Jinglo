...
/*: "call_bgm" :*/
fileprivate let user_stopFormat:String = "call_string title background"
fileprivate let notiEqualFormat:String = "BGM"

/*: "call_response_bgm" :*/
fileprivate let main_buttonInsidePath:[Character] = ["c","a","l","l","_","r","e"]
fileprivate let noti_withFormat:[Character] = ["s","p","o","n","s","e","_","b","g","m"]
...
class RangePlayer: NSObject {
    //: var type = CallBGMPlayerType.Call
    var type = EqualStrideable.Call
    //: var player = AVPlayer()
    var player = AVPlayer()
    //: var filePath = ""
    var filePath = ""
    ...
    class func pathVariation(type: EqualStrideable) -> RangePlayer {
        //: let player = TalkingCallBGMPlayer()
        let player = RangePlayer()
        //: player.type = type
        player.type = type
        //: player.getFilePath()
        player.appPath()
...
extension RangePlayer {
    ...
    private func textErrorConstraint() {
    ...
    private func equalQueryed(notify: Notification) {
    ...
    private func playerTitle(notify: Notification) {
    ...
    private func appPath() {
        //: var fileName = "call_bgm"
        var fileName = (String(user_stopFormat.prefix(5)) + notiEqualFormat.lowercased())
        //: if self.type == .Response {
        if self.type == .Response {
            //: fileName = "call_response_bgm"
            fileName = (String(main_buttonInsidePath) + String(noti_withFormat))
        }
        // 收到通话推送要播放mp3声音，不能放到.zip中
        //: let path = SVGAEffectTool.default.getMp3Path(name: fileName)
        let path = SkullSessionReactiveCompatible.default.statusFilter(name: fileName)
        //: self.filePath = path
        self.filePath = path
    }
}
...
extension RangePlayer: TXVodPlayListener {
