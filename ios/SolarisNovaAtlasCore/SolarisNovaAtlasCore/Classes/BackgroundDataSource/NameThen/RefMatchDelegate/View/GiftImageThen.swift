...
fileprivate let main_willName:[UInt8] = [0x97,0x9c,0x97,0xa2,0x56,0x91,0x9d,0x92,0x93,0xa0,0x68,0x57,0x4e,0x96,0x8f,0xa1,0x4e,0x9c,0x9d,0xa2,0x4e,0x90,0x93,0x93,0x9c,0x4e,0x97,0x9b,0x9e,0x9a,0x93,0x9b,0x93,0x9c,0xa2,0x93,0x92]

fileprivate func voiceName(path num: UInt8) -> UInt8 {
...
class GiftImageThen: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: main_willName.map{voiceName(path: $0)}, encoding: .utf8)!)
    }
...
extension GiftImageThen {
    ...
    override func hitTest(_ point: CGPoint, with _: UIEvent?) -> UIView? {
        ...
        for subview in self.subviews {
            ...
            let itemview: DirectToReactiveCompatible = subview as! DirectToReactiveCompatible
            ...
            if #available(iOS 11.0, *) {
                ...
                if itemview.avatarImage.frame.contains(point) {
                    //: RangePushManager.share.func__pushToUserDetailVC(uid: "\(itemview.giftMsgModel.fromUid ?? 0)")
                    RangePushManager.share.clickOfUid(uid: "\(itemview.giftMsgModel.fromUid ?? 0)")
                    //: break
                    break
                }
                //: } else {
    ...
    func usModel(model: ListHandyJSON) {
        //  1.相同礼物时则连击
        //: for  subview in self.subviews {
        for subview in self.subviews {
            //: let itemview: TalkingGiftTrackItemView = subview as! TalkingGiftTrackItemView
            ...
            if group(giftMsgModel: itemview.giftMsgModel, withGiftMsgModel: model) {
                //: itemview.comboHitWith(giftMsgModel: model)
                itemview.pathReload(giftMsgModel: model)
                //: itemview.setGiftMsgModel(model: model)
                itemview.isImage(model: model)
                //: return
                return
            }
        }
        ...
        for index in 0 ... 2 {
            ...
            let subView: DirectToReactiveCompatible? = self.viewWithTag(tag) as! DirectToReactiveCompatible?
            //: if subView == nil {
            if subView == nil {
                //: self.addNewGiftDanumWith(giftMsgModel: model, atIndex: index)
                self.property(giftMsgModel: model, atIndex: index)
                //: return
                return
            }
        }
        ...
        for index in 0 ... 2 {
            ...
            if index == 1 || index == 2 {
                ...
                let subView1: DirectToReactiveCompatible = self.viewWithTag(tag) as! DirectToReactiveCompatible
    ...
    func group(giftMsgModel: ListHandyJSON, withGiftMsgModel: ListHandyJSON) -> (Bool) {
...
//        subView.showUserInfoViewBlock   = self.showUserInfoViewBlock
        ...
        subView.isImage(model: giftMsgModel)
        //: self.addSubview(subView)
        self.addSubview(subView)

        //: subView.showGiftAnimate(model: giftMsgModel, animate: true)
        subView.timeLab(model: giftMsgModel, animate: true)
    }
}
