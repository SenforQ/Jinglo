...
fileprivate let show_makeStr:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "#171717" :*/
fileprivate let showDeviceMsg:String = "#1range1range1range"

/*: "icon_cameraoff" :*/
fileprivate let showResignPath:[Character] = ["i","c","o","n"]
fileprivate let show_userFormat:String = "return model view in in_camer"
...
class EqualView: UIView {
    ...
    override init(frame: CGRect) {
        ...
        self.backgroundColor = UIColor.loadRgba(51, 51, 51, 1)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: show_makeStr.reversed(), encoding: .utf8)!)
    }
    ...
    lazy var bgView: UIImageView = {
        ...
        alphaV.backgroundColor = UIColor(hex: (showDeviceMsg.replacingOccurrences(of: "range", with: "7")))
        //: alphaV.alpha = 0.6
        alphaV.alpha = 0.6
        //: v.addSubview(alphaV)
        v.addSubview(alphaV)
    ...
    private lazy var icon: UIImageView = {
        ...
        v.image = UIImage.thoughtImageBundleSizeText(name: (String(showResignPath) + String(show_userFormat.suffix(6)) + "aoff"))
        //: v.isHidden = true
        v.isHidden = true
        //: return v
        return v
...
extension EqualView {
    ...
    func formatAt(urlStr: String?, isOpenCamera: TvCameraStatus) {
...
extension EqualView: UIGestureRecognizerDelegate {
    /// 拖拽位移手势
    /// - Parameter pan: 手势
    //: @objc private func dragWindow(pan: UIPanGestureRecognizer) {
    @objc private func soybean(pan: UIPanGestureRecognizer) {
        ...
        newCenterX = min(newCenterX, kQueryValue - self.frame.size.width * 0.5)
        //: newCenterY = max(self.frame.size.height * 0.5, newCenterY)
        newCenterY = max(self.frame.size.height * 0.5, newCenterY)
        //: newCenterY = min(newCenterY, ScreenHeight - self.frame.size.height * 0.5)
        newCenterY = min(newCenterY, notiKeyTitle - self.frame.size.height * 0.5)
...
extension EqualView {
    ...
    private func designate() {
        // 切换视角手势
        //: let tap = UITapGestureRecognizer(target: self, action: #selector(exchangeRenderContainer))
        let tap = UITapGestureRecognizer(target: self, action: #selector(gleam))
        ...
        let pan = UIPanGestureRecognizer(target: self, action: #selector(soybean(pan:)))
        //: pan.delegate = self
        pan.delegate = self
