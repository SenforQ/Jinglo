...
fileprivate let data_handleKey:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]
...
class ShortListThen: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: data_handleKey.reversed(), encoding: .utf8)!)
    }
    ...
    override func willMove(toSuperview _: UIView?) {
        ...
        WrittenRecordSocketDelegate.shared.containerView?.isHidden = false
    }
    ...
    func birth() {
        ...
        UIView.animate(withDuration: 0.3, delay: 2, options: .curveEaseOut) {
            ...
            let x = (self.center.x <= kQueryValue * 0.5) ? -self.width : kQueryValue + self.width
            ...
            WrittenRecordSocketDelegate.shared.containerView?.isHidden = true
        }
    }
}
...
extension ShortListThen {
    /// 添加点击手势
    //: @discardableResult
    @discardableResult
    //: func addTapGesture() -> UITapGestureRecognizer {
    func videoIndex() -> UITapGestureRecognizer {
        //: let tap = UITapGestureRecognizer(target: self, action: #selector(self.tapGestureEvent(sender:)))
        let tap = UITapGestureRecognizer(target: self, action: #selector(self.recordSender(sender:)))
    ...
    func threeAccount() {
        ...
        let pan = UIPanGestureRecognizer(target: self, action: #selector(self.makeupClick(sender:)))
        //: let tap = self.addTapGesture()
        let tap = self.videoIndex()
    ...
    @objc func makeupClick(sender: UIPanGestureRecognizer) {
        ...
        centerX = min(kQueryValue - self.width * 0.5, centerX)
        //: centerY = max(self.height*0.5, centerY)
        centerY = max(self.height * 0.5, centerY)
        //: centerY = min(ScreenHeight-self.height*0.5, centerY)
        centerY = min(notiKeyTitle - self.height * 0.5, centerY)
        ...
        if sender.state == .ended {
            ...
            var endCenterX = kQueryValue - self.width * 0.5
            //: var recordX = ScreenWidth
            var recordX = kQueryValue
            //: if centerX <= ScreenWidth*0.5 {
            if centerX <= kQueryValue * 0.5 {
            ...
            WrittenRecordSocketDelegate.shared.videoMiniPosition = CGPoint(x: recordX, y: recordY)
        }
    }

    /// 点击手势
    //: @objc private func tapGestureEvent(sender: UIPanGestureRecognizer) {
    @objc private func recordSender(sender _: UIPanGestureRecognizer) {
