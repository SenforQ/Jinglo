...
fileprivate let kClickUrl:[UInt8] = [0xd2,0xd5,0xd2,0xcf,0x93,0xd8,0xd4,0xdf,0xde,0xc9,0x81,0x92,0x9b,0xd3,0xda,0xc8,0x9b,0xd5,0xd4,0xcf,0x9b,0xd9,0xde,0xde,0xd5,0x9b,0xd2,0xd6,0xcb,0xd7,0xde,0xd6,0xde,0xd5,0xcf,0xde,0xdf]

private func sectionTarget(present num: UInt8) -> UInt8 {
    return num ^ 187
}
...
@objc protocol KeyObjectProtocol: NSObjectProtocol {
    ///
    //: @objc optional func startScroll()
    @objc optional func utility()

    //: @objc optional func endScroll()
    @objc optional func endTo()
}
...
class LogTableView: UIView {
    ...
    override init(frame: CGRect) {
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(methodCutSend))
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kClickUrl.map{sectionTarget(present: $0)}, encoding: .utf8)!)
    }
...
extension LogTableView {
    ...
    func timerBar() {
        ...
        refreshTimer = Timer.scheduledTimer(timeInterval: reloadTimeSpan, target: self, selector: #selector(managerArc), userInfo: nil, repeats: true)
    }
    ...
    @objc func managerArc() {
        //: tryToappendAndScrollToBottom()
        totalubate()
    }

    //: @objc func tapAction() {
    @objc func methodCutSend() {
...
extension LogTableView {
    ...
    func ghb(msgModel: TagHandyJSON?) {
    ...
    func appForNeed() {
        ...
        if AllHeight > data_keyFormat {
            //: if self.msgTableView.height < VideoMsgTableViewHeight {
            if self.msgTableView.height < data_keyFormat {
                ...
                self.msgTableView.height = data_keyFormat
            }
            //: } else {
        } else {
            //: var frame = self.msgTableView.frame
            ...
            frame.origin.y = data_keyFormat - AllHeight
        ...
        playCard(animated: false)
    }

    /** 执行插入动画并滚动 */
    //: func scrollToBottom(animated: Bool) {
    func playCard(animated: Bool) {
    ...
    func itemPending(inPending: Bool) {
...
extension LogTableView: UITableViewDelegate, UITableViewDataSource {
    ...
    func scrollViewDidEndDragging(_ scrollView: UIScrollView, willDecelerate decelerate: Bool) {
        ...
        if !decelerate {
            ...
            finishDiskMainStreetQuantityelligentErase(scrollView)
        }
    }
    ...
    func finishDiskMainStreetQuantityelligentErase(_ scrollView: UIScrollView) {
    ...
    func tableView(_: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        return msgModel.msgHeight + Double(main_userPath)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = PanViewCell.tradingCard(tableView: tableView, msg: msgModel, indexPath: indexPath)
...
extension LogTableView {
    ...
    func reportAdd() {
        ...
        self.msgTableView.frame = CGRect(x: 0, y: 0, width: main_intimateKey, height: data_keyFormat)
    }
}
