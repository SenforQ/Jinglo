...
/*: "mf/call/GetUserInfo" :*/
fileprivate let app_sharedText:String = "request let add value selfmf/c"
fileprivate let app_picTitle:String = "etUseradd live"
fileprivate let userTempKey:String = "view row varInfo"

/*: "uid" :*/
fileprivate let show_sourceName:[UInt8] = [0x18,0xc,0x7]

fileprivate func regulatorLine(gift num: UInt8) -> UInt8 {
...
/*: "TRTC 随机视频匹配中." :*/
fileprivate let mainMakeImageUrl:String = "TRTC 随view thought array center greet"
fileprivate let const_originTitle:[Character] = ["机","视","频","匹","配","\u{4e2d}","."]

/*: "TRTC  :*/
fileprivate let kModelName:String = "TRTC image height"

/*: "发起" :*/
fileprivate let showColorPath:[UInt8] = [0x3f,0x55,0x4b,0x32,0x6f,0x6d]

/*: "收到" :*/
fileprivate let const_labelTitle:[Character] = ["收","\u{5230}"]

/*: 视频通话. logId: :*/
fileprivate let show_endData:String = "视频\u{901a}话"

/*: ." :*/
fileprivate let dataDeadlineBorderTitleMessage:String = "make"

/*: "s- :*/
fileprivate let user_editMsg:String = "image input make models-"

/*: - :*/
fileprivate let data_attentionText:String = "-"

/*: "TRTC 视频通话销毁. logId： :*/
fileprivate let user_showEndPathMsg:[Character] = ["T","R","T","C"," ","\u{89c6}","频","\u{901a}","话","销","毁","."]
fileprivate let show_sexValue:[Character] = [" ","l"]
fileprivate let mainObjectName:[Character] = ["o","g","I","d","："]

/*: "TRTC 视频通话错误. errCode: :*/
fileprivate let k_toLabMessage:String = "TRTC view source height make info"
fileprivate let notiProStr:[Character] = ["视","频","通","话","\u{9519}","误","."," ","e","r","r","C","o","d","e",":"]

/*: , errMsg: :*/
fileprivate let constPartySourcePath:[Character] = [","," ","e"]
fileprivate let show_normalFormat:[Character] = ["r","r","M","s","g",":"]

/*: "TRTC 视频通话SDK收到警告. warningCode: :*/
fileprivate let app_moreUrl:String = "TRTC \u{89c6}"
fileprivate let dataValueName:String = "K收到警view else model for listen"
fileprivate let appAtMsg:String = "addrnin"
fileprivate let dataModelMessage:[Character] = [":"]

/*: , warningMsg: :*/
fileprivate let data_successFormat:[Character] = [","," ","w","a","r","n","i","n","g","M","s","g",":"]

/*: "TRTC 视频通话进入房间. result: :*/
fileprivate let dataQuickName:[Character] = ["T","R","T","C"," "]
fileprivate let app_sizeName:[Character] = ["\u{89c6}","频","\u{901a}","话","进","入","\u{623f}"]
fileprivate let show_viewId:[Character] = ["间","."," ","r","e","s","u","l","t",":"]

/*: , logId： :*/
fileprivate let notiEqualUrl:String = ", logId\u{ff1a}"

/*:  加入了房间" :*/
fileprivate let const_makeUrl:String = " \u{52a0}入了房\u{95f4}"

/*:  离开了房间" :*/
fileprivate let noti_iconMessage:String = " \u{79bb}开了\u{623f}间"
...
class TitleIntervalimateChatConfig: NSObject {
    ...
    var remoteCameraStatus = TvCameraStatus.Normal // 对方摄像头状态
}

//: class TalkingVideoChatManager: NSObject {
class SubSizeThen: NSObject {
    //: var infoModel: TalkingVideoChatModel?
    var infoModel: ExamineRequestChatModel? // 视频通话model
    //: var config = TalkingVideoChatConfig()
    var config = TitleIntervalimateChatConfig() // 视频渲染配置类
    ...
    private var trtcView: VideoVoiceViewDelegete? // 预览view（使用商汤美颜）
...
extension SubSizeThen {
    /// 开始视频速配（男）
    /// - Parameters:
    ///   - uid: 接收者的id
    ///   - completion: 回调
    //: class func req_callGetUserInfo(uid: String, completion: @escaping FinishBlock) {
    class func poking(uid: String, completion: @escaping FinishBlock) {
        //: let reqModel = TalkingRequestModel.init()
        let reqModel = SubRequestModel()
        //: reqModel.requestPath = "mf/call/GetUserInfo"
        reqModel.requestPath = (String(app_sharedText.suffix(4)) + "all/G" + String(app_picTitle.prefix(6)) + String(userTempKey.suffix(4)))
        //: reqModel.requestType = .GET
        reqModel.requestType = .GET
        //: reqModel.params = ["uid": uid]
        reqModel.params = [String(bytes: show_sourceName.map{regulatorLine(gift: $0)}, encoding: .utf8)!: uid]
...
extension SubSizeThen {
    ...
    func useForPortraitCamera(_ frontCamera: Bool) {
        ...
        self.playUser(front: frontCamera)
    }

    /// 摄像头开启与关闭
    /// - Parameter openCamera: 是否开启
    //: func useCamera(_ openCamera: Bool) {
    func makeObject(_ openCamera: Bool) {
        ...
        self.distanceCount(isOpen: openCamera)
    }
    ...
    func renderView() {
        ...
        DispatchQueue.main.async {
            ...
            self.capture(smallWindow: self.infoModel?.renderPostion != .selfOnBg)
...
extension SubSizeThen {
    ...
    private func playUser(front: Bool) {
        ...
        self.trtcView?.sourceFront(isFront: front)
    }

    /// 是否开启摄像头
    /// - Parameter isOpen: 是否开启
    //: private func setTRTCCamera(isOpen: Bool) {
    private func distanceCount(isOpen: Bool) {
        //: guard SenseTime_Use == true else {
        guard const_useId == true else {
            //: if isOpen {
            if isOpen {
        ...
        self.trtcView?.loseToOpen(isOpen: isOpen)
...
extension SubSizeThen {
    ...
    func statuteTitle() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(loadFile(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(starUp(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(hideAddAttribute(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(audioAndData(notif:)),
    ...
    @objc private func loadFile(notif: Notification?) {
        //: if notif == nil {
        if notif == nil {
            // 静音
    ...
    @objc private func starUp(notif: Notification?) {
        //: if notif == nil {
    ...
    @objc private func hideAddAttribute(notif: Notification) {
    ...
    @objc func audioAndData(notif: Notification) {
...
extension SubSizeThen {
    ...
    func outEqual(isMatch: Bool = false) {
        //: if isMatch {
        if isMatch {
            //: UploadLogTool.writeLog(msg: "TRTC 随机视频匹配中.")
            GestureLogTool.playChangeImage(msg: (String(mainMakeImageUrl.prefix(6)) + String(const_originTitle)))
            //: } else {
        } else {
            //: UploadLogTool.writeLog(msg: "TRTC \((infoModel?.videoStage == .Waiting) ? "发起":"收到")视频通话. logId:\(infoModel?.logId ?? 0).")
            GestureLogTool.playChangeImage(msg: (String(kModelName.prefix(5))) + "\((infoModel?.videoStage == .Waiting) ? "发起" : "收到")" + (show_endData + ". logId:") + "\(infoModel?.logId ?? 0).")
        }
        //: if self.infoModel?.videoStage == .Waiting {
        if self.infoModel?.videoStage == .Waiting {
            //: startTRTCPreview(smallWindow: false)
            capture(smallWindow: false)
    ...
    func capture(smallWindow: Bool) {
        ...
        if self.trtcView == nil {
            ...
            self.trtcView = VideoVoiceViewDelegete()
        }
        //: self.trtcView?.startPreView(isSmallWindow: smallWindow)
        self.trtcView?.messageLive(isSmallWindow: smallWindow)
        //: if smallWindow {
    ...
    private func pointPlayer() {
        ...
        params.sdkAppId = main_tableSearchStr
        //: params.roomId = UInt32(roomId)
        params.roomId = UInt32(roomId)
        //: params.userId = PunctuateThen.share.loginUserMode.userID
        params.userId = PunctuateThen.share.loginUserMode.userID
        //: params.userSig = PunctuateThen.share.appUserConfigMode.uSigTrtc
        params.userSig = PunctuateThen.share.appUserConfigMode.uSigTrtc
    ...
    private func sightPic() {
        //: UploadLogTool.writeLog(msg: "TRTC 视频通话销毁. logId：\(infoModel?.logId ?? 0).")
        GestureLogTool.playChangeImage(msg: (String(user_showEndPathMsg) + String(show_sexValue) + String(mainObjectName)) + "\(infoModel?.logId ?? 0).")
...
extension SubSizeThen: TRTCCloudDelegate {
    ...
    func onError(_ errCode: TXLiteAVError, errMsg: String?, extInfo _: [AnyHashable: Any]?) {
        ...
        GestureLogTool.playChangeImage(msg: (String(k_toLabMessage.prefix(5)) + String(notiProStr)) + "\(errCode)" + (String(constPartySourcePath) + String(show_normalFormat)) + "\(String(describing: errMsg)).")
        //: if errMsg != nil {
        if errMsg != nil {
            //: self.func__showStatusBarErrorMsg(showMsg: errMsg!)
            self.drawObserver(showMsg: errMsg!)
        }
    }
    ...
    func onWarning(_ warningCode: TXLiteAVWarning, warningMsg: String?, extInfo _: [AnyHashable: Any]?) {
        ...
        GestureLogTool.playChangeImage(msg: (app_moreUrl + "频通话SD" + String(dataValueName.prefix(4)) + "告. w" + appAtMsg.replacingOccurrences(of: "add", with: "a") + "gCode" + String(dataModelMessage)) + "\(warningCode)" + (String(data_successFormat)) + "\(String(describing: warningMsg)).")
        ...
        if self.config.remoteCameraStatus == .Normal {
            ...
            NotificationCenter.default.post(name: const_textInfoName, object: self)
        }
    }
    ...
    func onEnterRoom(_ result: Int) {
        ...
        GestureLogTool.playChangeImage(msg: (String(dataQuickName) + String(app_sizeName) + String(show_viewId)) + "\(result)" + (notiEqualUrl) + "\(infoModel?.logId ?? 0).")
        //: if result < 0 {
        if result < 0 { // 加入失败
            ...
            WrittenRecordSocketDelegate.shared.crossfire()
            //: } else {
        } else {
            //: self.config.disconnected = false
            self.config.disconnected = false
    ...
    func onUserVideoAvailable(_ userId: String, available: Bool) {
        //: guard userId != PunctuateThen.share.loginUserMode.userID else { return }
        guard userId != PunctuateThen.share.loginUserMode.userID else { return }
