...
/*: "Speaker On" :*/
fileprivate let k_fileStr:String = "Spcolor return time"
fileprivate let appMessageValue:[Character] = ["e","a","k","e","r"," ","O","n"]

/*: "Speaker Off" :*/
fileprivate let main_addUrl:String = "Speaall since else type"

/*: "s- :*/
fileprivate let appFormatKey:String = "gift let lets-"

/*: - :*/
fileprivate let show_streamFormat:String = "-"

/*: "TRTC 音频通话销毁. logId： :*/
fileprivate let user_makeName:[Character] = ["T","R","T","C"," ","音","频","通","话","销"]
fileprivate let userEndUrl:[Character] = ["毁","."," ","l","o","g","I","d","："]

/*: ." :*/
fileprivate let kLiveData:String = "actual"

/*: "TRTC 音频通话错误. errCode: :*/
fileprivate let show_midName:String = "self array self eventTRTC 音"
fileprivate let app_modelStr:[Character] = ["频","通","话","错","误","."," ","e","r","r","C","o","d","e",":"]

/*: , errMsg: :*/
fileprivate let app_pushFormat:[Character] = [","," ","e","r","r"]
fileprivate let notiMagnitudeimateStr:String = "Msg:to case make view first"

/*: "TRTC 音频通话SDK收到警告. warningCode: :*/
fileprivate let appDefineSystemKey:[Character] = ["T","R","T","C"," "]
fileprivate let appTabPostName:String = "音\u{9891}通\u{8bdd}S"
fileprivate let k_centerData:[Character] = ["D","K","收","\u{5230}","警","告","."," ","w","a","r","n","i","n","g","C","o","d","e",":"]

/*: , warningMsg: :*/
fileprivate let const_infoStr:[Character] = [","," ","w","a","r","n","i","n","g","M","s","g"]
fileprivate let appToKey:[Character] = [":"]

/*: "TRTC 音频通话进入房间. result: :*/
fileprivate let constTitleUrl:String = "TRTresult"
fileprivate let userCancelName:String = "\u{95f4}. re"

/*: , logId： :*/
fileprivate let k_sectionFormat:[Character] = [","]
fileprivate let user_topMessage:String = " logId\u{ff1a}"

/*:  加入了房间" :*/
fileprivate let show_instanceMessage:String = " 加入了房间"

/*:  离开了房间" :*/
fileprivate let app_textId:String = " 离\u{5f00}了房间"
...
private class FailChatConfig: NSObject {
...
class ToReactiveCompatible: NSObject {
    ...
    private var config = FailChatConfig() // 音频渲染配置类
    //: private var logId = 0
...
extension ToReactiveCompatible {
    ...
    func wall(roomId: Int) {
        //: self.handsfreeBtn?.isEnabled = !hasHeadSet()
        self.handsfreeBtn?.isEnabled = !alongView()
    ...
    func asSuiteHandsfree(isHandsfree: Bool) {
        //: guard hasHeadSet() == false else { return }
        guard alongView() == false else { return }
        ...
        self.handsfreeDesLab?.text = isHandsfree ? (String(k_fileStr.prefix(2)) + String(appMessageValue)).localized : (String(main_addUrl.prefix(4)) + "ker Off").localized
        //: updateWhenPlayDeviceChanged(isHandsfree: isHandsfree)
        ptolemaicSystem(isHandsfree: isHandsfree)
    }
}
...
extension ToReactiveCompatible {
    ...
    func lock() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(numbererval(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(wantAudio(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(methodContent(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(labNotif(notif:)),
    ...
    @objc private func numbererval(notif: Notification?) {
        //: if notif == nil {
        if notif == nil {
    ...
    @objc private func wantAudio(notif: Notification?) {
        //: if notif == nil {
        if notif == nil {
    ...
    @objc private func methodContent(notif: Notification) {
    ...
    @objc func labNotif(notif: Notification) {
        ...
        case .newDeviceAvailable: // 听筒
            ...
            ptolemaicSystem(isHandsfree: false)
        ...
        case .oldDeviceUnavailable: // 免提
            ...
            self.handsfreeDesLab?.text = (String(k_fileStr.prefix(2)) + String(appMessageValue)).localized
            //: updateWhenPlayDeviceChanged(isHandsfree: self.config.isHandsfree)
            ptolemaicSystem(isHandsfree: self.config.isHandsfree)
...
extension ToReactiveCompatible {
    ...
    private func ptolemaicSystem(isHandsfree: Bool) {
    ...
    private func upFrame() {
        ...
        GestureLogTool.playChangeImage(msg: (String(user_makeName) + String(userEndUrl)) + "\(logId).")
        //: guard self.config.workSpace else { return }
        guard self.config.workSpace else { return }

        //: if self.config.isHandsfree == false {
        if self.config.isHandsfree == false { // 恢复系统扬声器播放声音
            //: updateWhenPlayDeviceChanged(isHandsfree: true)
            ptolemaicSystem(isHandsfree: true)
        }
...
extension ToReactiveCompatible: TRTCCloudDelegate {
    ...
    func onError(_ errCode: TXLiteAVError, errMsg: String?, extInfo _: [AnyHashable: Any]?) {
        ...
        GestureLogTool.playChangeImage(msg: (String(show_midName.suffix(6)) + String(app_modelStr)) + "\(errCode)" + (String(app_pushFormat) + String(notiMagnitudeimateStr.prefix(4))) + "\(String(describing: errMsg)).")
        //: if errMsg != nil {
        if errMsg != nil {
    ...
    func onWarning(_ warningCode: TXLiteAVWarning, warningMsg: String?, extInfo _: [AnyHashable: Any]?) {
        ...
        GestureLogTool.playChangeImage(msg: (String(appDefineSystemKey) + appTabPostName + String(k_centerData)) + "\(warningCode)" + (String(const_infoStr) + String(appToKey)) + "\(String(describing: warningMsg)).")
    }

    //: func onEnterRoom(_ result: Int) {
    func onEnterRoom(_ result: Int) {
        //: UploadLogTool.writeLog(msg: "TRTC 音频通话进入房间. result:\(result), logId：\(logId).")
        GestureLogTool.playChangeImage(msg: (constTitleUrl.replacingOccurrences(of: "result", with: "C") + " 音频通话进入房" + userCancelName + "sult:") + "\(result)" + (String(k_sectionFormat) + user_topMessage) + "\(logId).")
        //: if result < 0 {
        if result < 0 { // 加入失败
