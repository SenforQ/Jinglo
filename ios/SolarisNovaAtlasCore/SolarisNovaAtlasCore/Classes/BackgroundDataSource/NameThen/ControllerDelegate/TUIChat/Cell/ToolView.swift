...
fileprivate let main_viewData:[Character] = ["T","S","y","s","t","e","m","M","e","s","s","a","g","e","C","e"]
fileprivate let show_contentData:String = "make"
fileprivate let main_valueUrl:[Character] = ["_","R","e","u","s","e","I","d"]

/*: "init(coder:) has not been implemented" :*/
fileprivate let k_matchValue:[UInt8] = [0x8a,0x8d,0x8a,0x97,0xcb,0x80,0x8c,0x87,0x86,0x91,0xd9,0xca,0xc3,0x8b,0x82,0x90,0xc3,0x8d,0x8c,0x97,0xc3,0x81,0x86,0x86,0x8d,0xc3,0x8a,0x8e,0x93,0x8f,0x86,0x8e,0x86,0x8d,0x97,0x86,0x87]

private func bottomHidden(you num: UInt8) -> UInt8 {
    return num ^ 227
}

/*: "icon_talk_notic" :*/
fileprivate let kLabelContent:[Character] = ["i","c","o","n","_","t","a"]
fileprivate let user_cellMsg:[Character] = ["l"]
fileprivate let showNameAppearUrl:[Character] = ["k","_","n","o","t","i","c"]
...
@objc public protocol TitleDelegate: NSObjectProtocol {
    /**
     *  长按消息回调
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onLongPressMessage(cell: ToolView)
    func enter(cell: ToolView)
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onRetryMessage(cell: ToolView)
    func thrust(cell: ToolView)
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onSelectMessage(cell: ToolView)
    func dismissAlong(cell: ToolView)
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onSelectMessageAvatar(cell: ToolView)
    func picControl(cell: ToolView)

    /**
     *  点击消息单元中引用消息
     *  @param cell 委托者，消息单元
     */
    //: func onReplyMessage(cell: ToolView)
    func differentiate(cell: ToolView)
}

//: @objcMembers public class ToolView: TCommonTableViewCell {
@objcMembers public class ToolView: LogVideoViewCell {
    /**
     *  信息数据类。
     *  存储了该massageCell中所需的信息。包括发送者 ID，发送者头像、信息发送状态、信息气泡图标等。
     *  messageData 详细信息请参考：Section\Chat\CellData\EqualCompartmentReactiveCompatible.h
     */
    //: public var messageData: EqualCompartmentReactiveCompatible?
    public var messageData: EqualCompartmentReactiveCompatible?

    /// 复用cell标识符
    //: @objc public static let TSystemMessageCell_ReuseId = "TSystemMessageCell_ReuseId"
    public static let TSystemMessageCell_ReuseId = (String(main_viewData) + show_contentData.replacingOccurrences(of: "make", with: "ll") + String(main_valueUrl))

    /**
     *  协议委托
     *  负责实现 TitleDelegate 协议中的功能。
     */
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_matchValue.map{bottomHidden(you: $0)}, encoding: .utf8)!)
    }
    ...
    override public func ladenInfo(with data: DetailCellData) {
        //: super.fill(with: data)
        super.ladenInfo(with: data)
        //: let newData: EqualCompartmentReactiveCompatible = data as! EqualCompartmentReactiveCompatible
        let newData: EqualCompartmentReactiveCompatible = data as! EqualCompartmentReactiveCompatible
    ...
    override public func updateConstraints() {
        ...
        //: let csize = self.messageData?.accumulationOn()
        let csize = self.messageData?.accumulationOn()
    ...
    lazy var avatarView: UIImageView = {
        ...
        let tap1 = UITapGestureRecognizer(target: self, action: #selector(outsideWill(recognizer:)))
    ...
    lazy var nameLabel: UILabel = {
        //: let lb = UILabel()
        let lb = UILabel()
        //: lb.font = UIFont.atomic(fontSize: 13)
        lb.font = UIFont.atomic(fontSize: 13)
        //: lb.textColor = .gray
        lb.textColor = .gray
    ...
    public lazy var container: UIView = {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(tillMessage(recognizer:)))
        ...
        let longPress = UILongPressGestureRecognizer(target: self, action: #selector(underHide(recognizer:)))
    ...
    lazy var retryView: UIImageView = {
        ...
        imagV.image = UIImage.thoughtImageBundleSizeText(name: (String(kLabelContent) + String(user_cellMsg) + String(showNameAppearUrl)))
        ...
        let resendTap = UITapGestureRecognizer(target: self, action: #selector(listRecognizer(recognizer:)))
...
extension ToolView {
    ...
    @objc func outsideWill(recognizer _: UIGestureRecognizer) {
        //: self.delegate?.onSelectMessageAvatar(cell: self)
        self.delegate?.picControl(cell: self)
    }

    //: @objc func onLongPress(recognizer: UIGestureRecognizer) {
    @objc func underHide(recognizer: UIGestureRecognizer) {
    ...
    @objc func listRecognizer(recognizer _: UIGestureRecognizer) {
        //: if self.messageData?.status == .Msg_Status_Fail {
        if self.messageData?.status == .Msg_Status_Fail {
            //: self.delegate?.onRetryMessage(cell: self)
            self.delegate?.thrust(cell: self)
        }
    }

    //: @objc func onSelectMessage(recognizer: UIGestureRecognizer) {
    @objc func tillMessage(recognizer _: UIGestureRecognizer) {
