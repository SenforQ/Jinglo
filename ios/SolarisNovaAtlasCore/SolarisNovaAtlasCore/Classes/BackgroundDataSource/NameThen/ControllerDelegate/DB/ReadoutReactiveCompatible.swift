...
/*: "serialQueue" :*/
fileprivate let constReColorMessage:String = "S"
fileprivate let user_availableSharedFillId:String = "user user edge colorerialQ"

/*: "fromUids" :*/
fileprivate let show_toId:[UInt8] = [0xa6,0xb2,0xaf,0xad,0x95,0xa9,0xa4,0xb3]

private func toALesserExtent(list num: UInt8) -> UInt8 {
    return num ^ 192
}
...
// __DEBUG__
// __CLOSE_PRINT__
//  ReadoutReactiveCompatible.swift
//  AbroadTalking
//
...
public class ReadoutReactiveCompatible: NSObject {
    ...
    private static var _instance: ReadoutReactiveCompatible?
    //: class func shared() -> DBUserInfoManager {
    class func cellOpen() -> ReadoutReactiveCompatible {
        //: guard _instance != nil else {
        guard _instance != nil else {
            //: _instance = DBUserInfoManager()
            _instance = ReadoutReactiveCompatible()
            // 添加退出账号通知
    ...
    @objc private func logout() {
        ...
        ReadoutReactiveCompatible.colorDestroy()
    }
}
...
extension ReadoutReactiveCompatible {
    /// 获取用户消息
    /// - Parameters:
    ///   - userIdsArr: 用户ids
    ///   - completion: 回调
    //: class func cache_getUserInfo(with userIdsArr: [String], completion: @escaping ([RequestHandyJSON]?, Error?) -> Void) {
    class func inner(with userIdsArr: [String], completion: @escaping ([RequestHandyJSON]?, Error?) -> Void) {
        // 数量过多，wcdb的where语句会因过长而报错，所以做分页
        //: var newUserIdsArr = userIdsArr
        var newUserIdsArr = userIdsArr
        //: if DBUserInfoManager.shared().needReqUidsSet != nil {
        if ReadoutReactiveCompatible.cellOpen().needReqUidsSet != nil { // 合并去重
            //: newUserIdsArr = Array(DBUserInfoManager.shared().needReqUidsSet!.union(userIdsArr))
            newUserIdsArr = Array(ReadoutReactiveCompatible.cellOpen().needReqUidsSet!.union(userIdsArr))
            //: DBUserInfoManager.shared().needReqUidsSet = nil
            ReadoutReactiveCompatible.cellOpen().needReqUidsSet = nil
        }
        ...
        repeat {
            ...
            counternalWithoutTo(with: subArr, completion: completion)
            //: location += len
            location += len
    ...
    class func outChat(with userArr: [String], completion: @escaping ([[String: Any]]?, Error?) -> Void) {
        ...
        let serialQueue = DispatchQueue(label: (constReColorMessage.lowercased() + String(user_availableSharedFillId.suffix(6)) + "ueue"))
        ...
        serialQueue.async {
            ...
            for item in finalArray {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) {
                    //: SomebodyRequestTool.req_queryStatus(toUids: requestUidsStr) { _, list in
                    SomebodyRequestTool.toResign(toUids: requestUidsStr) { _, list in
                        //: defer { sema.signal() }
                        defer { sema.signal() }
    ...
    public class func enableUp(message: V2TIMMessage) -> QueryQuantityoMsgTable {
        //: let key = TUISocialChatManager.checkPrimaryKey(withMsgModel: message)
        let key = ModelThen.voiceModel(withMsgModel: message)
        //: var cacheWrap = WCDBVoiceMsgTable.db_getVoiceMsg(with: key)
        var cacheWrap = QueryQuantityoMsgTable.applicationClick(with: key)
        //: if cacheWrap == nil {
        if cacheWrap == nil {
            //: let info = TUISocialChatManager.packageDict(withTXMessage: message)
            let info = ModelThen.runCell(withTXMessage: message)
            //: cacheWrap = WCDBVoiceMsgTable.db_inserVoiceMsgToDB(info)
            cacheWrap = QueryQuantityoMsgTable.videoAdd(info)
        }
        //: return cacheWrap!
        return cacheWrap!
    }
...
extension ReadoutReactiveCompatible {
    ...
    //: @discardableResult
    @discardableResult
    //: class func cache_getCachedUserInfo(targetId: String) -> RequestHandyJSON? {
    class func comparable(targetId: String) -> RequestHandyJSON? {
        //: return WCDBUserInfoTable.db_getUserInfo(with: targetId)
        return GestureInfoTable.commensurate(with: targetId)
    }

    /// 更新数据库【用户模型】，并发送通知
    /// - Parameter userInfo: 用户信息model
    //: class func cache_updateMsgUserInfoAndPostNotif(with userInfo: RequestHandyJSON?) {
    class func postSomeoneInfoAndModifyImageStreetwiseAdd(with userInfo: RequestHandyJSON?) {
        ...
        let model = ReadoutReactiveCompatible.comparable(targetId: userInfo.uid)
        // 自定义字段需要赋值，防止被覆盖
        ...
        GestureInfoTable.database(userInfo)
        //: DispatchQueue.main.async {
        DispatchQueue.main.async {
            //: NotificationCenter.default.post(name: USER_INFO_MSG_LIST_NOTIFICATION, object: [userInfo.uid: userInfo])
            NotificationCenter.default.post(name: constVersionKey, object: [userInfo.uid: userInfo])
        }
    }
    ...
    class func checkTime(with targetId: String, intimate: Int, currTime: Int) {
        //: guard let model = DBUserInfoManager.cache_getCachedUserInfo(targetId: targetId) else { return }
        guard let model = ReadoutReactiveCompatible.comparable(targetId: targetId) else { return }
    ...
    class func viewTime(with targetId: String, time: Int) {
        //: guard let model = DBUserInfoManager.cache_getCachedUserInfo(targetId: targetId) else { return }
        guard let model = ReadoutReactiveCompatible.comparable(targetId: targetId) else { return }
    ...
    class func replacement(with targetUid: String) {
        //: WCDBUserInfoTable.db_deleteUserInfo(with: targetUid)
        GestureInfoTable.valueShow(with: targetUid)
        //: DBUserInfoManager.shared().reqedUidsSet.remove(targetUid)
        ReadoutReactiveCompatible.cellOpen().reqedUidsSet.remove(targetUid)
    }
}
...
extension ReadoutReactiveCompatible {
    ...
    ///   - userIdsArr: 用户ids
    ///   - completion: 回调
    //: private class func inner_getUserInfo(with userIdsArr: [String], completion: @escaping ([RequestHandyJSON]?, Error?) -> Void) {
    private class func counternalWithoutTo(with userIdsArr: [String], completion: @escaping ([RequestHandyJSON]?, Error?) -> Void) {
        //: let userModels = WCDBUserInfoTable.db_getUserInfos(with: userIdsArr)
        let userModels = GestureInfoTable.createBy(with: userIdsArr)
        ...
        for uid in userIdsArr where !userModels.contains(where: { $0.uid == uid }) {
            ...
            if ReadoutReactiveCompatible.cellOpen().reqedUidsSet.contains(uid) == false {
        ...
        if !noDBUidArr.isEmpty {
            ...
            ReadoutReactiveCompatible.straightAndNarrowNow(with: noDBUidArr)
        }
    }
    ...
    private class func straightAndNarrowNow(with userIdsArr: [String]) {
        ...
        ReadoutReactiveCompatible.cellOpen().reqedUidsSet = ReadoutReactiveCompatible.cellOpen().reqedUidsSet.union(userIdsArr)

        // 请求用户消息列表
        //: SomebodyRequestTool.req_refreshMsgUserListInfo(param: ["fromUids": requestUidsStr]) { succeed, dataArr in
        SomebodyRequestTool.atCurrent(param: [String(bytes: show_toId.map{toALesserExtent(list: $0)}, encoding: .utf8)!: requestUidsStr]) { succeed, dataArr in
            //: guard succeed else {
            guard succeed else { // 失败还原数据
                //: DBUserInfoManager.shared().reqedUidsSet.subtract(userIdsArr)
                ReadoutReactiveCompatible.cellOpen().reqedUidsSet.subtract(userIdsArr)
                //: return
                return
            }
            // 更新数据库
            //: DBUserInfoManager.updateMsgUserInfoTable(userInfoArr: dataArr)
            ReadoutReactiveCompatible.liveArr(userInfoArr: dataArr)
            // 发送通知
            //: var retInfoDict: [String: RequestHandyJSON] = [:]
            var retInfoDict: [String: RequestHandyJSON] = [:]
            ...
            if !retInfoDict.isEmpty {
                ...
                DispatchQueue.main.async {
                    ...
                    NotificationCenter.default.post(name: constVersionKey, object: retInfoDict)
                }
            }
        }
    }
    ...
    //: private class func updateMsgUserInfoTable(userInfoArr: [RequestHandyJSON]) {
    private class func liveArr(userInfoArr: [RequestHandyJSON]) {
        //: for aWrap in userInfoArr {
        for aWrap in userInfoArr {
            //: WCDBUserInfoTable.db_updateUserInfo(aWrap)
            GestureInfoTable.database(aWrap)
        }
    }
}
