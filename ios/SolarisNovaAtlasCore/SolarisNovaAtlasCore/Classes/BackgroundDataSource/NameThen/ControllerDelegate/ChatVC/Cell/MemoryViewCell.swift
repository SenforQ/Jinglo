...
fileprivate let notiMainName:[UInt8] = [0x36,0x31,0x36,0x2b,0x77,0x3c,0x30,0x3b,0x3a,0x2d,0x65,0x76,0x7f,0x37,0x3e,0x2c,0x7f,0x31,0x30,0x2b,0x7f,0x3d,0x3a,0x3a,0x31,0x7f,0x36,0x32,0x2f,0x33,0x3a,0x32,0x3a,0x31,0x2b,0x3a,0x3b]

private func dataSound(bottom num: UInt8) -> UInt8 {
    return num ^ 95
}

/*: "all" :*/
fileprivate let mainValueText:String = "amm"
...
class MemoryViewCell: CommonViewCell {
    //: var giftData: TalkingGroupChatGiftMsgCellData?
    var giftData: ComponentThen?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: notiMainName.map{dataSound(bottom: $0)}, encoding: .utf8)!)
    }
...
extension MemoryViewCell {
    ...
    override func ladenInfo(with data: DetailCellData) {
        ...
        self.giftData = data as? ComponentThen
        ...
        if giftData.msgModel.toUser.uid == (mainValueText.replacingOccurrences(of: "m", with: "l")) { // 送礼所有人
            //: self.toUserImgView.image = UIImage.BundleImageNamed(name: giftData.msgModel.toUser.headPic)
            self.toUserImgView.image = UIImage.thoughtImageBundleSizeText(name: giftData.msgModel.toUser.headPic)
            //: } else {
        } else {
            //: self.toUserImgView.setUrlImage(urlStr: giftData.msgModel.toUser.headPic)
            self.toUserImgView.putBackThroughFinish(urlStr: giftData.msgModel.toUser.headPic)
        }
    }
