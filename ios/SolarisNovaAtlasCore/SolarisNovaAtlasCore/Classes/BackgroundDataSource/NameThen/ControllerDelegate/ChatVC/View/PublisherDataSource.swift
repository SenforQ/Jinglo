...
fileprivate let constBottomUrl:[UInt8] = [0xa1,0xa6,0xa1,0xbc,0xe0,0xab,0xa7,0xac,0xad,0xba,0xf2,0xe1,0xe8,0xa0,0xa9,0xbb,0xe8,0xa6,0xa7,0xbc,0xe8,0xaa,0xad,0xad,0xa6,0xe8,0xa1,0xa5,0xb8,0xa4,0xad,0xa5,0xad,0xa6,0xbc,0xad,0xac]

private func viewCurrent(minimize num: UInt8) -> UInt8 {
    return num ^ 200
}

/*: "icon_chats_banner_subtract" :*/
fileprivate let constAddFormat:[Character] = ["i","c","o","n","_","c","h","a","t","s","_"]
fileprivate let main_renderKey:String = "baacceptaccepte"
fileprivate let dataModeUrl:String = "btpathct"

/*: "icon_chats_banner_subtract_pre" :*/
fileprivate let data_toMsg:[Character] = ["i","c","o","n","_","c","h","a","t","s","_","b","a","n","n","e","r","_","s","u"]
fileprivate let kBottomMessage:String = "btaddact"

/*: "bannerList" :*/
fileprivate let noti_rowTitle:[Character] = ["b","a","n","n","e","r","L","i","s","t"]

/*: "JXBannerCellID" :*/
fileprivate let showToTitle:String = "text if searchJXBann"

/*: "#FFFFFF" :*/
fileprivate let data_clickName:[Character] = ["#","F","F","F","F","F","F"]
...
protocol EqualReactiveCompatible: NSObject {
    ...
    func heritableUponView(isMiniView: Bool)
}

//: class TalkingPrivateBannerView: UIView {
class PublisherDataSource: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: constBottomUrl.map{viewCurrent(minimize: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var bannerListData: [MakeSingleBannerModel] = //: return Array<MakeSingleBannerModel>()
        .init()
    //: }()
    ...
    private lazy var closeBtn: UIButton = {
        ...
        btn.setImage(UIImage.thoughtImageBundleSizeText(name: (String(constAddFormat) + main_renderKey.replacingOccurrences(of: "accept", with: "n") + "r_su" + dataModeUrl.replacingOccurrences(of: "path", with: "ra"))), for: .normal)
        //: btn.setImage(UIImage.BundleImageNamed(name: "icon_chats_banner_subtract_pre"), for: .selected)
        btn.setImage(UIImage.thoughtImageBundleSizeText(name: (String(data_toMsg) + kBottomMessage.replacingOccurrences(of: "add", with: "r") + "_pre")), for: .selected)
...
extension PublisherDataSource {
    ...
    func playerOf() {
        ...
        AvailableRequestManager().toCompletion(position: 3) { [weak self] _, result, _ in
            ...
            let array = JSON(result[(String(noti_rowTitle))] ?? []).arrayValue
...
extension PublisherDataSource {
    ...
    @objc private func press() {
        ...
        self.delegate?.heritableUponView(isMiniView: isMiniView)
    }

    //: func resetView(isMiniView: Bool) {
    func gestureTo(isMiniView: Bool) {
        //: if isMiniView {
        if isMiniView {
...
extension PublisherDataSource: JXBannerDelegate, JXBannerDataSource {
    ...
    func jxBanner(_: JXBannerType) -> (JXBannerCellRegister) {
        ...
        return JXBannerCellRegister(type: JXBannerCell.self, reuseIdentifier: (String(showToTitle.suffix(6)) + "erCellID"))
    }
    ...
    func jxBanner(_: JXBannerType, cellForItemAt index: Int, cell: UICollectionViewCell) -> UICollectionViewCell {
        ...
        tempCell.imageView.beforeRender(urlStr: url, placeImg: UIImage.brother())
                  ...
                  builder: JXBannerPageControlBuilder) -> JXBannerPageControlBuilder
    {
        ...
        pageControl.activeColor = UIColor(hex: (String(data_clickName)))!
