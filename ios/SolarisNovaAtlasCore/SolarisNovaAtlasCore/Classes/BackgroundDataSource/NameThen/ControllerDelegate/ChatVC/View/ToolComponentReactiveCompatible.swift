...
fileprivate let appViewTouchTitle:[UInt8] = [0x0,0x7,0x0,0x1d,0x41,0xa,0x6,0xd,0xc,0x1b,0x53,0x40,0x49,0x1,0x8,0x1a,0x49,0x7,0x6,0x1d,0x49,0xb,0xc,0xc,0x7,0x49,0x0,0x4,0x19,0x5,0xc,0x4,0xc,0x7,0x1d,0xc,0xd]

private func sizeUser(count num: UInt8) -> UInt8 {
    return num ^ 105
}

/*: "0/ :*/
fileprivate let mainViewName:[Character] = ["0","/"]

/*: "#AAAAAA" :*/
fileprivate let showInputTitle:String = "#AAAAAAplayer back layer"

/*: "btn_chatsettings_unchoice" :*/
fileprivate let noti_usValue:[Character] = ["b","t","n","_","c","h","a","t","s","e","t","t"]
fileprivate let k_tapUrl:String = "in put viewings_"

/*: "btn_chatsettings_choiced" :*/
fileprivate let show_actionValue:[Character] = ["b","t","n","_","c","h","a","t","s","e","t","t","i"]
fileprivate let data_zoneText:String = "else self data nongs_ch"

/*: "Top" :*/
fileprivate let userManagerLabPath:[Character] = ["T","o","p"]

/*: "Save" :*/
fileprivate let k_intimateName:[Character] = ["S","a","v","e"]

/*: "Cancel" :*/
fileprivate let kAddSucceedStr:String = "Cancelvar less index pop for"

/*: / :*/
fileprivate let show_indicatorPeopleText:String = "record"
...
// __CLOSE_PRINT__
//
//  ToolComponentReactiveCompatible.swift
...
class ToolComponentReactiveCompatible: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: appViewTouchTitle.map{sizeUser(count: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var contentView: UIView = {
        ...
        let height = actualWidth(w: 475) + k_netId
        //: let v = UIView(frame: CGRect(x: 0, y: ScreenHeight-height, width: ScreenWidth, height: height))
        let v = UIView(frame: CGRect(x: 0, y: notiKeyTitle - height, width: kQueryValue, height: height))
        //: v.backgroundColor = .white
        v.backgroundColor = .white
        //: v.Corner(width: v.width, height: v.height, corners: [.topLeft, .topRight], cornerRadii: .init(width: 15, height: 15))
        v.areaRemoveRadiiAt(width: v.width, height: v.height, corners: [.topLeft, .topRight], cornerRadii: .init(width: 15, height: 15))
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var textBgView: UIView = {
        ...
        v.backgroundColor = UIColor.sizeColor()
    ...
    private lazy var limitLab: UILabel = {
        ...
        lab.textColor = UIColor(hex: (String(showInputTitle.prefix(7))))
        //: return lab
        return lab
        //: }()
    }()
    ...
    private lazy var topBtn: StartImageButton = {
        ...
        btn.setImage(UIImage.thoughtImageBundleSizeText(name: (String(noti_usValue) + String(k_tapUrl.suffix(5)) + "unchoice")), for: .normal)
        //: btn.setImage(UIImage.BundleImageNamed(name: "btn_chatsettings_choiced"), for: .selected)
        btn.setImage(UIImage.thoughtImageBundleSizeText(name: (String(show_actionValue) + String(data_zoneText.suffix(6)) + "oiced")), for: .selected)
        ...
        btn.setTitle((String(userManagerLabPath)).localized, for: .normal)
    ...
    private lazy var saveBtn: UIButton = {
        ...
        btn.setTitle((String(k_intimateName)).localized, for: .normal)
    ...
    private lazy var cancelBtn: UIButton = {
        ...
        btn.setTitle((String(kAddSucceedStr.prefix(6))).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(sendMove), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension ToolComponentReactiveCompatible: YYTextViewDelegate {
...
extension ToolComponentReactiveCompatible {
    /// 保存按钮点击事件
    ...
    @objc private func shirtButton() {
        ...
        let isTop = topBtn.isSelected ? 1 : 2
        //: SomebodyRequestTool.req_remark(content: textView.text, isTop: isTop, toUid: targetId) {
        SomebodyRequestTool.alongCompletion(content: textView.text, isTop: isTop, toUid: targetId) {
            //: succeed, _, _ in
            succeed, _, _ in
    ...
    func valueExecute(_ content: String, isTop: Int, toUid: String) {
        //: targetId = toUid
        targetId = toUid
