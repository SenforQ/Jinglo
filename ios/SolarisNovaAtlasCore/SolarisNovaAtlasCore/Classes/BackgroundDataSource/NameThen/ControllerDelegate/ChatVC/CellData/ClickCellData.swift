...
fileprivate let kTitleStr:[UInt8] = [0xd8,0xdf,0xd8,0xc5,0x99,0xd2,0xde,0xd5,0xd4,0xc3,0x8b,0x98,0x91,0xd9,0xd0,0xc2,0x91,0xdf,0xde,0xc5,0x91,0xd3,0xd4,0xd4,0xdf,0x91,0xd8,0xdc,0xc1,0xdd,0xd4,0xdc,0xd4,0xdf,0xc5,0xd4,0xd5]

/*: "[未知消息]" :*/
fileprivate let noti_itemText:String = "[未知消息]"

/*: "extra" :*/
fileprivate let const_addKey:String = "EXTRA"

/*: "</at> " :*/
fileprivate let kModelTextKey:String = "</at> bag of remove price empty"

/*: "#0084FF" :*/
fileprivate let notiReadData:[Character] = ["#","0"]
fileprivate let dataLeadingByMsg:[Character] = ["0","8","4","F","F"]

/*: "#FFEE4B" :*/
fileprivate let dataMakeText:String = "mic"
fileprivate let appModelMessage:[Character] = ["F","F","E","E","4","B"]

/*: "<at>@[\\S\\s]+?</at>" :*/
fileprivate let noti_pageText:String = "<at>@[add popular with"
fileprivate let data_genderValueId:[Character] = ["\\","S","\\","s","]","+","?"]
fileprivate let data_observerMsg:String = "</at>class let message"

/*: "jumpKey" :*/
fileprivate let show_inputMsg:String = "view start view view upjumpK"
fileprivate let constTextUrl:String = "eno"

/*: "user" :*/
fileprivate let dataListAppTitle:[Character] = ["u","s","e","r"]

/*: "uid" :*/
fileprivate let kMakeUrl:[UInt8] = [0xd8,0xc4,0xc9]

private func elseSelf(to num: UInt8) -> UInt8 {
    return num ^ 173
}

/*: "#16D073" :*/
fileprivate let dataStyleKey:String = "#16D073equal play"

/*: "#F95644" :*/
fileprivate let constAddBackMsg:[Character] = ["#","F","9","5","6","4","4"]
...
@objcMembers public class ClickCellData: CompartmentCellData {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kTitleStr.map{$0^177}, encoding: .utf8)!)
    }
    ...
    override public var msgModel: SharedModel {
        ...
        set {
            ...
            if newValue.msgInfo.content.isEmptyString {
                //: contentStr = "[未知消息]"
                contentStr = (noti_itemText.capitalized)
                //: }else {
            } else {
    ...
    public lazy var contentAttrString: NSMutableAttributedString = {
        ...
        let attr = detailForce(msgInfoModel: self.msgModel.msgInfo)
        //: return attr
        return attr
        //: }()
    }()
    ...
    func detailForce(msgInfoModel: UpModel) -> NSMutableAttributedString {
        ...
        guard dic[(const_addKey.lowercased())] is [String: Any] else {
            //: return NSMutableAttributedString.init(string: "")
            return NSMutableAttributedString(string: "")
        }
        ...
        let textColor = (self.direction == .MsgDirectionIncoming) ? UIColor.toEvent() : .white
        //: let atColor = (self.direction == .MsgDirectionIncoming) ? UIColor(hex: "#0084FF")! : UIColor(hex: "#FFEE4B")!
        let atColor = (self.direction == .MsgDirectionIncoming) ? UIColor(hex: (String(notiReadData) + String(dataLeadingByMsg)))! : UIColor(hex: (dataMakeText.replacingOccurrences(of: "mic", with: "#") + String(appModelMessage)))!
        ...
        let regex = try! NSRegularExpression(pattern: (String(noti_pageText.prefix(6)) + String(data_genderValueId) + String(data_observerMsg.prefix(5))), options: [])
        ...
        for (index, value) in rangeArr.enumerated() {
            ...
            if jumpInfo[(String(show_inputMsg.suffix(5)) + constTextUrl.replacingOccurrences(of: "no", with: "y"))].stringValue == (String(dataListAppTitle)) {
                //: uid = jumpInfo["uid"].intValue
                uid = jumpInfo[String(bytes: kMakeUrl.map{elseSelf(to: $0)}, encoding: .utf8)!].intValue
            }
            //: if uid != -1 {
            if uid != -1 {
                ...
                attr.yy_setTextHighlight(range, color: atColor, backgroundColor: .clear, userInfo: [String(bytes: kMakeUrl.map{elseSelf(to: $0)}, encoding: .utf8)!: uid]) { [weak self] _, _, _, _ in
    ...
    func passageIcon() -> NSMutableAttributedString {
        ...
        var color = UIColor(hex: (String(dataStyleKey.prefix(7))))!
        //: if self.translateStatus == .MsgTranslate_fail {
        if self.translateStatus == .MsgTranslate_fail {
            //: color = UIColor(hex: "#F95644")!
            color = UIColor(hex: (String(constAddBackMsg)))!
        }
