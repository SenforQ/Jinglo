...
fileprivate let mainDismissStr:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "move" :*/
fileprivate let k_countFormat:String = "motexte"
...
class TotaloScreenView: UIView {
    ...
    var danumuMsgArray: [ImageTransformable] = []
    //: var danumuItemViewArray: [TalkingGameFloatingScreenItemView] = []
    var danumuItemViewArray: [PhotoCommonItemView] = []

    //: let TapAction: PublishSubject<TalkingGameFloatingScreenModel?> = PublishSubject()
    let TapAction: PublishSubject<ImageTransformable?> = PublishSubject()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: mainDismissStr.reversed(), encoding: .utf8)!)
    }
    ...
    func selectReceive(_ danumuMsgModel: ImageTransformable?) {
    ...
    @objc func benignity(_: UITapGestureRecognizer) {}
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(benignity(_:)))
        //: t.delegate = self
        t.delegate = self
        //: return t
        return t
...
extension TotaloScreenView: UIGestureRecognizerDelegate {
    ...
    func giftFirst(for danumuMsgModel: ImageTransformable?, isFirst: Bool) {
        //: let itemView = TalkingGameFloatingScreenItemView(model: danumuMsgModel)
        let itemView = PhotoCommonItemView(model: danumuMsgModel)
        ...
        var frame = CGRect(x: kQueryValue, y: isFirst ? 7 : 50, width: itemView.widthValue, height: 24)
        //: var endFrame = frame
        var endFrame = frame
        ...
        if let anim = POPBasicAnimation(propertyNamed: kPOPViewFrame) {
            ...
            itemView.pop_add(anim, forKey: (k_countFormat.replacingOccurrences(of: "text", with: "v")))
    ...
    func fireNext(_ isFirst: Bool) {
        ...
        if danumuMsgArray.count > 0 {
            ...
            if let model {
                ...
                giftFirst(for: model, isFirst: isFirst)
...
extension TotaloScreenView {
    // 添加视图
    //: private func setupSubviews() {
    private func appealSubviews() {
