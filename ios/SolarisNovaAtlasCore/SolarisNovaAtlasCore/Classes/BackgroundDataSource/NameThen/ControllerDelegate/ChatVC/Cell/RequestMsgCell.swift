...
fileprivate let main_firstMessage:[UInt8] = [0x8f,0x88,0x8f,0x92,0xce,0x85,0x89,0x82,0x83,0x94,0xdc,0xcf,0xc6,0x8e,0x87,0x95,0xc6,0x88,0x89,0x92,0xc6,0x84,0x83,0x83,0x88,0xc6,0x8f,0x8b,0x96,0x8a,0x83,0x8b,0x83,0x88,0x92,0x83,0x82]

/*: "icon_chat_translate_nor" :*/
fileprivate let dataPlayerPath:String = "mark in nativeicon_cha"
fileprivate let data_labName:String = "try self string toucht_tra"
fileprivate let dataPriceTitle:String = "e_norequal if try path cell"

/*: "icon_chat_translate_pre" :*/
fileprivate let noti_listErrorLastFormat:[Character] = ["i","c","o","n","_","c","h","a","t","_","t","r","a","n","s","l","a"]
fileprivate let appNameId:String = "add lette_pre"

/*: "icon_chat_miss" :*/
fileprivate let user_pathStr:[Character] = ["i","c","o","n","_","c","h","a","t"]
fileprivate let mainSourceFormat:String = "_missif model"

/*: "icon_chat_yes" :*/
fileprivate let dataBubblePath:String = "ifullon"
fileprivate let userGiftName:String = "view frame will to make_chat"
...
class RequestMsgCell: CommonViewCell {
    //: var transBlock: (() -> Void)?
    var transBlock: (() -> Void)?

    //: var textData: ClickCellData?
    var textData: ClickCellData?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: main_firstMessage.map{$0^230}, encoding: .utf8)!)
    }
    ...
    lazy var transBtn: UIButton = {
        ...
        btn.setImage(UIImage.thoughtImageBundleSizeText(name: (String(dataPlayerPath.suffix(8)) + String(data_labName.suffix(5)) + "nslat" + String(dataPriceTitle.prefix(5)))), for: .normal)
        //: btn.setImage(UIImage.BundleImageNamed(name: "icon_chat_translate_pre"), for: .highlighted)
        btn.setImage(UIImage.thoughtImageBundleSizeText(name: (String(noti_listErrorLastFormat) + String(appNameId.suffix(6)))), for: .highlighted)
        //: btn.addTarget(self, action: #selector(transBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(errorDown), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension RequestMsgCell {
    ...
    override func ladenInfo(with data: DetailCellData) {
        ...
        //: let maxWidth = PublisherReactiveCompatible.getScreenWidth() - (15 + (self.cellData?.cellLayout.avatarSize.width ?? 45) + 10) * 2
        let maxWidth = PublisherReactiveCompatible.widthState() - (15 + (self.cellData?.cellLayout.avatarSize.width ?? 45) + 10) * 2
        // 翻译赋值相关
        ...
        if textData.translateStatus == .MsgTranslate_translating {
            ...
            self.transStatusImagV.image = UIImage.thoughtImageBundleSizeText(name: (String(user_pathStr) + String(mainSourceFormat.prefix(5))))
            ...
            self.transStatusImagV.image = UIImage.thoughtImageBundleSizeText(name: (dataBubblePath.replacingOccurrences(of: "full", with: "c") + String(userGiftName.suffix(5)) + "_yes"))
        }
    ...
    func combineLikeTag() {
        ...
        let longPressGr = UILongPressGestureRecognizer(target: self, action: #selector(sizeTap(tap:)))
        //: self.container.addGestureRecognizer(longPressGr)
        self.container.addGestureRecognizer(longPressGr)
    }

    //: @objc func LognCellMessageAction(tap: UILongPressGestureRecognizer) {
    @objc func sizeTap(tap: UILongPressGestureRecognizer) {
        //: if tap.state == .ended {
        if tap.state == .ended {
            //: return
            return
