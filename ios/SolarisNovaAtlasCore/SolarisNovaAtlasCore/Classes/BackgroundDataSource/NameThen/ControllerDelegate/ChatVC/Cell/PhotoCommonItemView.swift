...
/*: "#ffffff" :*/
fileprivate let mainTotalMsg:String = "guard target size event view#ffffff"

/*: "earned" :*/
fileprivate let noti_topMessage:String = "earatd"

/*: "Coins in" :*/
fileprivate let const_colorData:String = "make model video self userCoi"
fileprivate let user_succeedDirectionTitle:[Character] = ["n","s"," ","i","n"]

/*: "Error:  :*/
fileprivate let constSearchedMsg:[Character] = ["E","r"]
fileprivate let dataBackUrl:[Character] = ["r","o","r",":"," "]

/*: "#C946D2" :*/
fileprivate let mainLeadingPath:[Character] = ["#","C","9","4","6","D","2"]

/*: "btn_chat_floatS_userBK_nor" :*/
fileprivate let data_textFormat:String = "else entrybtn_"
fileprivate let constCancelModelTitle:String = "floatSabout conversation color"
fileprivate let userAddFormat:String = "titler"
...
class PhotoCommonItemView: UIView {
    ...
    private let TextColor = (String(mainTotalMsg.suffix(7)))

    //: var clickBlock: (() -> Void)?
    var clickBlock: (() -> Void)?

    //: convenience init(model: TalkingGameFloatingScreenModel?) {
    convenience init(model: ImageTransformable?) {
        //: self.init()
        self.init()
    ...
    private func config() {
        ...
        if danumuMsgModel.bgPic.count > 0 {
            ...
            commitUpUrl(withUrl: danumuMsgModel.bgPic)
        }
        ...
        if danumuMsgModel.content.count > 0 {
            ...
            do {
                ...
                if let range = attrStr.string.range(of: (noti_topMessage.replacingOccurrences(of: "at", with: "ne"))) {
                ...
                if let range = attrStr.string.range(of: (String(const_colorData.suffix(3)) + String(user_succeedDirectionTitle))) {
                    //: attrStr.addAttributes([.foregroundColor: UIColor.white], range: NSRange(range, in: attrStr.string))
                    attrStr.addAttributes([.foregroundColor: UIColor.white], range: NSRange(range, in: attrStr.string))
                }
                //: attrStr.addAttributes([.font: UIFont.pingfangFont(type: .Medium, fontSize: 12)], range: NSRange(location: 0, length: attrStr.length))
                attrStr.addAttributes([.font: UIFont.clickLarge(type: .Medium, fontSize: 12)], range: NSRange(location: 0, length: attrStr.length))
                ...
                printLog(message: (String(constSearchedMsg) + String(dataBackUrl)) + "\(error)")
            }
        }
    }

    //: func updateBackground(withUrl url: String) {
    func commitUpUrl(withUrl url: String) {
        ...
        self.backgroundImageView.beforeRender(urlStr: url) { [weak self] _, image in
            ...
            if let image {
                ...
                if image.size.height > self.HeightOfBackgroundImageView {
                    ...
                    newImage = image.power(with: CGSize(width: targetWidth, height: self.HeightOfBackgroundImageView))
                }
            }
    ...
    var danumuMsgModel: ImageTransformable? {
        //: didSet {
        didSet {
            //: configModel()
            config()
    ...
    lazy var headAnimatedImageView: UIImageView = {
        ...
        h.layer.borderColor = UIColor(hex: (String(mainLeadingPath)))?.cgColor
        //: return h
        return h
        //: }()
    }()
    ...
    lazy var headBackImageView: UIImageView = {
        ...
        h.image = UIImage.thoughtImageBundleSizeText(name: (String(data_textFormat.suffix(4)) + "chat_" + String(constCancelModelTitle.prefix(6)) + "_userBK_" + userAddFormat.replacingOccurrences(of: "title", with: "no")))
        //: return h
        return h
        //: }()
    }()
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(indexTo(_:)))
        //: return t
        return t
        //: }()
    }()
...
extension PhotoCommonItemView {
    /// 点击跳转事件
    //: @objc func tapAction(_ tapGesture: UITapGestureRecognizer) {
    @objc func indexTo(_: UITapGestureRecognizer) {
        //: guard let gameUrl = self.danumuMsgModel?.gameUrl else { return }
        guard let gameUrl = self.danumuMsgModel?.gameUrl else { return }
        //: let httpsUrl = PunctuateThen.share.appConfigMode.urlH5Domain+gameUrl
        let httpsUrl = PunctuateThen.share.appConfigMode.urlH5Domain + gameUrl
...
extension PhotoCommonItemView {
    ...
    private func lucidityNumbereraction() {
