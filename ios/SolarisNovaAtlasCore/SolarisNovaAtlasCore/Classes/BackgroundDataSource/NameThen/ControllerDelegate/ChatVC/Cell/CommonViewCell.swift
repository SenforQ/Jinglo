...
fileprivate let k_keyStatusName:[UInt8] = [0x8a,0x8d,0x8a,0x97,0xcb,0x80,0x8c,0x87,0x86,0x91,0xd9,0xca,0xc3,0x8b,0x82,0x90,0xc3,0x8d,0x8c,0x97,0xc3,0x81,0x86,0x86,0x8d,0xc3,0x8a,0x8e,0x93,0x8f,0x86,0x8e,0x86,0x8d,0x97,0x86,0x87]

/*: "icon_lounge" :*/
fileprivate let kLabPath:String = "color"
fileprivate let dataVideoEffectContent:String = "player instancecon_lo"

/*: "icon_talk_points" :*/
fileprivate let mainBorderMessage:[Character] = ["i","c","o","n","_","t","a","l","k"]
fileprivate let noti_meFormat:String = "head any_points"

/*: "C6BDFF" :*/
fileprivate let constDataFormat:String = "c6bdff"

/*: "#BBBBBB" :*/
fileprivate let notiBagId:String = "height return path#BBBBBB"

/*: "Reply to get points~" :*/
fileprivate let constMakeData:String = "Replyfor view"
fileprivate let show_conversationContent:String = "voice caseet p"

/*: "bth_unread_nor" :*/
fileprivate let k_aspectUrl:String = "bth_unwill server index"
fileprivate let notiErrKey:String = "insidea"
fileprivate let main_withKey:String = "lab make length manager bottomd_nor"

/*: "icon_male_default" :*/
fileprivate let noti_byIndexTitle:[UInt8] = [0x74,0x6c,0x75,0x61,0x66,0x65,0x64,0x5f,0x65,0x6c,0x61,0x6d,0x5f,0x6e,0x6f,0x63,0x69]

/*: "icon_female_default" :*/
fileprivate let showItemFormat:String = "iframen"
fileprivate let notiWillKey:String = "stop model path shareale_de"
fileprivate let const_viewName:String = "colorault"

/*: "   " :*/
fileprivate let notiMakeStr:String = "   "

/*: "+%@ points" :*/
fileprivate let main_countName:String = "+%@ "
fileprivate let user_videoInfoName:String = "poinselecteds"

/*: "bth_read_pre" :*/
fileprivate let app_managerFormat:String = "bth_reapo let gift cancel"
fileprivate let dataListName:String = "info user view i userd_pre"
...
//: class TalkingChatBaseMsgCell: ToolView {
class CommonViewCell: ToolView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_keyStatusName.map{$0^227}, encoding: .utf8)!)
    }
    ...
    private lazy var loungeImgV: UIImageView = {
        ...
        img.image = UIImage.thoughtImageBundleSizeText(name: (kLabPath.replacingOccurrences(of: "color", with: "i") + String(dataVideoEffectContent.suffix(6)) + "unge"))
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var ageLab: UILabel = {
        ...
        label.textColor = .white
        //: label.font = UIFont.atomic(fontSize: 10)
        label.font = UIFont.atomic(fontSize: 10)
        //: return label
        return label
        //: }()
    ...
    public lazy var coinIconImg: UIImageView = {
        ...
        imgV.image = UIImage.thoughtImageBundleSizeText(name: (String(mainBorderMessage) + String(noti_meFormat.suffix(7))))
        //: return imgV
        return imgV
        //: }()
    }()
    ...
    public lazy var coinLabel: UILabel = {
        ...
        label.textColor = UIColor(hex: (constDataFormat.uppercased()))
        //: label.isHidden = true
        label.isHidden = true
        //: label.font = UIFont.pingfangRugularFont(fontSize: 14)
        label.font = UIFont.recoverOf(fontSize: 14)
        //: return label
        return label
        //: }()
    }()
    ...
    public lazy var replyTipLab: UILabel = {
        ...
        label.textColor = UIColor(hex: (String(notiBagId.suffix(7))))
        //: label.isHidden = true
        label.isHidden = true
        //: label.font = UIFont.pingfangRugularFont(fontSize: 14)
        label.font = UIFont.recoverOf(fontSize: 14)
        //: label.text = "Reply to get points~".localized
        label.text = (String(constMakeData.prefix(5)) + " to g" + String(show_conversationContent.suffix(4)) + "oints~").localized
        //: return label
        return label
        //: }()
    }()
    ...
    public lazy var isReadImg: UIImageView = {
        ...
        imgV.image = UIImage.thoughtImageBundleSizeText(name: (String(k_aspectUrl.prefix(6)) + notiErrKey.replacingOccurrences(of: "inside", with: "re") + String(main_withKey.suffix(5))))
        //: return imgV
        return imgV
        //: }()
    }()
...
extension CommonViewCell {
    ...
    override func ladenInfo(with data: DetailCellData) {
        ...
        let userModel = newData.msgModel.user
        //: let loungePlus = (PunctuateThen.share.loginUid == String(userModel.uid)) ? PunctuateThen.share.loginUserMode.loungePlus : userModel.loungePlus
        let loungePlus = (PunctuateThen.share.loginUid == String(userModel.uid)) ? PunctuateThen.share.loginUserMode.loungePlus : userModel.loungePlus

        //: self.nameLabel.text = userModel.nickname
        self.nameLabel.text = userModel.nickname
        //: self.nameLabel.textColor = loungePlus ? .userVipColor() : .appValueColor()
        self.nameLabel.textColor = loungePlus ? .utiliserColor() : .imageColor()
        //: self.nameLabel.font = UIFont.atomic(fontSize: 13)
        self.nameLabel.font = UIFont.atomic(fontSize: 13)
        //: let imgStr = userModel.sex == Int(Gender.male.rawValue) ? "icon_male_default" : "icon_female_default"
        let imgStr = userModel.sex == Int(TopComparable.male.rawValue) ? String(bytes: noti_byIndexTitle.reversed(), encoding: .utf8)! : (showItemFormat.replacingOccurrences(of: "frame", with: "co") + "_fem" + String(notiWillKey.suffix(6)) + const_viewName.replacingOccurrences(of: "color", with: "f"))
        //: sexIconBtn.setBackgroundImage(UIImage.BundleImageNamed(name: imgStr), for: .normal)
        sexIconBtn.setBackgroundImage(UIImage.thoughtImageBundleSizeText(name: imgStr), for: .normal)
        ...
        self.avatarView.putBackThroughFinish(urlStr: userModel.headPic)
        ...
        self.iconBorder.glassesByStr(urlStr: userModel.headPicFrame)

        //: let isShowCoin = newData.msgIncome > 0
        let isShowCoin = newData.msgIncome > 0
        //: self.coinIconImg.isHidden = !(isShowCoin && PunctuateThen.share.appStatus == AppSkinStatus.normal.rawValue)
        self.coinIconImg.isHidden = !(isShowCoin && PunctuateThen.share.appStatus == HiddenHashableRepresentation.normal.rawValue)
        ...
        self.coinLabel.text = (main_countName.capitalized + user_videoInfoName.replacingOccurrences(of: "selected", with: "t")).viewfinder(NSNumber(value: Float(newData.msgIncome)))
        // 女性逻辑, 未回复付费消息提示
        //: if TalkingPrivateChatManager.needShowReplyTipsMessage(newData.direction,
        if MoveReactiveCompatible.addCommunication(newData.direction,
        ...
        if MoveReactiveCompatible.titleData(cellData: newData) {
            //: self.isReadImg.isHidden = false
            self.isReadImg.isHidden = false
            //: var readImgStr = "bth_unread_nor"
            var readImgStr = (String(k_aspectUrl.prefix(6)) + notiErrKey.replacingOccurrences(of: "inside", with: "re") + String(main_withKey.suffix(5)))
            // 消息对端是否已读
            //: if TalkingPrivateChatManager.msgIsRead(cellData: newData) {
            if MoveReactiveCompatible.travel(cellData: newData) {
                //: readImgStr = "bth_read_pre"
                readImgStr = (String(app_managerFormat.prefix(7)) + String(dataListName.suffix(5)))
            }
            //: self.isReadImg.image = UIImage.BundleImageNamed(name: readImgStr)
            self.isReadImg.image = UIImage.thoughtImageBundleSizeText(name: readImgStr)
    ...
    override public func updateConstraints() {
        ...
        if let temCellData = self.cellData {
            //: loungePlus = (PunctuateThen.share.loginUid == String(temCellData.msgModel.user.uid)) ? PunctuateThen.share.loginUserMode.loungePlus : temCellData.msgModel.user.loungePlus
            loungePlus = (PunctuateThen.share.loginUid == String(temCellData.msgModel.user.uid)) ? PunctuateThen.share.loginUserMode.loungePlus : temCellData.msgModel.user.loungePlus
        }
