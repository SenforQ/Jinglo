...
fileprivate let constImageValue:[UInt8] = [0xec,0xeb,0xec,0xf1,0xad,0xe6,0xea,0xe1,0xe0,0xf7,0xbf,0xac,0xa5,0xed,0xe4,0xf6,0xa5,0xeb,0xea,0xf1,0xa5,0xe7,0xe0,0xe0,0xeb,0xa5,0xec,0xe8,0xf5,0xe9,0xe0,0xe8,0xe0,0xeb,0xf1,0xe0,0xe1]

private func errUp(hidden num: UInt8) -> UInt8 {
    return num ^ 133
}

/*: "icon_chat_lock" :*/
fileprivate let notiManagerFormat:String = "coston"
fileprivate let showTopInputName:[Character] = ["_","l","o","c","k"]

/*: "icon_chat_burned" :*/
fileprivate let notiRawContent:String = "icon_icon center in intimate text"
fileprivate let notiSizeShowContent:[Character] = ["b","u","r","n","e","d"]

/*: "icon_chat_burned_shan" :*/
fileprivate let app_effectStr:[Character] = ["i","c"]
fileprivate let showHiddenKey:[Character] = ["o","n","_","c","h","a","t","_","b","u","r","n","e","d","_","s","h","a","n"]

/*: "Already burned" :*/
fileprivate let userModelFormat:[Character] = ["A","l","r","e","a","d","y"," ","b","u","r","n","e"]
fileprivate let mainMakePath:String = "change"
...
class ActionReactiveCompatible: CommonViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: constImageValue.map{errUp(hidden: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var lockImagV: UIImageView = {
        ...
        imag.image = UIImage.thoughtImageBundleSizeText(name: (notiManagerFormat.replacingOccurrences(of: "cost", with: "ic") + "_chat" + String(showTopInputName)))
    ...
    lazy var burnedImagV: StartImageButton = {
        ...
        imgV.setBackgroundImage(UIImage.thoughtImageBundleSizeText(name: (String(notiRawContent.prefix(5)) + "chat_" + String(notiSizeShowContent))), for: .normal)
        //: imgV.setImage(UIImage.BundleImageNamed(name: "icon_chat_burned_shan"), for: .normal)
        imgV.setImage(UIImage.thoughtImageBundleSizeText(name: (String(app_effectStr) + String(showHiddenKey))), for: .normal)
        //: imgV.imageAlignment = .top
        imgV.imageAlignment = .top
        //: imgV.setTitle("Already burned".localized, for: .normal)
        imgV.setTitle((String(userModelFormat) + mainMakePath.replacingOccurrences(of: "change", with: "d")).localized, for: .normal)
        //: imgV.setTitleColor(.white, for: .normal)
        imgV.setTitleColor(.white, for: .normal)
        //: imgV.titleLabel?.font = UIFont.pingfangRugularFont(fontSize: 16)
        imgV.titleLabel?.font = UIFont.recoverOf(fontSize: 16)
        //: return imgV
        return imgV
        //: }()
    }()
...
extension ActionReactiveCompatible {
    ...
    override func ladenInfo(with data: DetailCellData) {
        ...
        let intimatePhoto: PhotoModel? = imageCellData.msgModel.msgInfo.intimatePhoto
        ...
        if intimatePhoto != nil || imageCellData.msgModel.msgInfo.customSend.isBurn == "1" || Int(imageCellData.msgModel.msgInfo.customSend.giftId) ?? 0 > 0 { // 私密照片
            ...
            digitiserMosaic(data: imageCellData, urlStr: imgUrlStr, isMosaic: imgIsMosaic)

            //: } else {
        } else { // 普通图片
            //: showImageView(data: imageCellData, urlStr: nil, isMosaic: false)
            digitiserMosaic(data: imageCellData, urlStr: nil, isMosaic: false)
        }
    }
    ...
    func digitiserMosaic(data: ChatThen, urlStr: String?, isMosaic: Bool) {
        ...
        filePath = (filePath as NSString).appendingPathComponent(constButtonMessage)
        ...
        if data.imageUrl!.count > 0, isExists {
            ...
            DispatchQueue.global().async {
                ...
                DispatchQueue.main.async {
                    ...
                    //: if isMosaic {
                    if isMosaic {
                        //: self.photoView.image = UIImage.pageActive(image, by: 90)
                        self.photoView.image = UIImage.pageActive(image, by: 90)
                        //: } else {
                    } else {
            ...
            allowEqual(urlStr: urlStr ?? "", isMosaic: isMosaic)
            //: } else {
        } else { // 本地图片请求网络
            ...
            if data.imageData != nil {
                ...
                self.photoView.putBackThroughFinish(urlStr: data.imageUrl!)
            }
        }
    }
    ...
    func allowEqual(urlStr: String, isMosaic: Bool) {
        //: self.photoView.setCookieToUrlImage(urlStr: urlStr) {[weak self] image in
        self.photoView.tillCamera(urlStr: urlStr) { [weak self] image in
