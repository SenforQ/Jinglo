...
fileprivate let kMakeMsg:[UInt8] = [0x34,0x39,0x34,0x3f,0xf3,0x2e,0x3a,0x2f,0x30,0x3d,0x5,0xf4,0xeb,0x33,0x2c,0x3e,0xeb,0x39,0x3a,0x3f,0xeb,0x2d,0x30,0x30,0x39,0xeb,0x34,0x38,0x3b,0x37,0x30,0x38,0x30,0x39,0x3f,0x30,0x2f]

fileprivate func appearStop(object num: UInt8) -> UInt8 {
...
/*: "bth_live_follow_pre" :*/
fileprivate let constWillData:[Character] = ["b","t","h","_","l","i"]
fileprivate let dataFirstName:String = "time"
fileprivate let const_topText:[Character] = ["_","f","o","l","l","o","w","_","p","r","e"]

/*: "btn_live_closed_nor" :*/
fileprivate let main_viewMsg:String = "btn_liframe photo lab result gift"
fileprivate let appOnTapListMsg:String = "origin model equal let sizelosed_nor"

/*: "removeAttentionUid" :*/
fileprivate let userPathUrl:String = "REMOV"
fileprivate let appMoreMessage:[Character] = ["e","A","t","t","e","n","t","i","o","n","U","i","d"]

/*: "attentionUid" :*/
fileprivate let user_shadowTitle:String = "akite"
fileprivate let mainMarginKey:[Character] = ["d"]

/*: "source" :*/
fileprivate let show_singleBackPath:[Character] = ["s","o","u","r","c","e"]

/*: "num" :*/
fileprivate let showVoiceTitle:[Character] = ["n","u","m"]

/*: "99+" :*/
fileprivate let showAppUrl:[UInt8] = [0x7d,0x7d,0x6f]

/*: "UID: :*/
fileprivate let userTabElementStr:[Character] = ["U","I","D",":"]
...
// __CLOSE_PRINT__
//
//  SizeBathReactiveCompatible.swift
//  Adjust
//
...
class SizeBathReactiveCompatible: UIView {
    ...
    private var partyModel = VoiceMeasurable()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kMakeMsg.map{appearStop(object: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var iconBgBtn: UIButton = {
        ...
        v.addTarget(self, action: #selector(pushUp), for: .touchUpInside)
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var followBtn: UIButton = {
        ...
        btn.setImage(UIImage.thoughtImageBundleSizeText(name: (String(constWillData) + dataFirstName.replacingOccurrences(of: "time", with: "ve") + String(const_topText))), for: .normal)
        //: btn.addTarget(self, action: #selector(followButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(fallBackToSnap), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var miniBtn: UIButton = {
        ...
        btn.setImage(UIImage.thoughtImageBundleSizeText(name: (String(main_viewMsg.prefix(6)) + "ve_c" + String(appOnTapListMsg.suffix(9)))), for: .normal)
    ...
    private lazy var onlineNumBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(judgeBar), for: .touchUpInside)
    ...
    private lazy var onlineListView: FailViewDelegate = {
        //: let v = TalkingVoiceRoomOnlineListView()
        let v = FailViewDelegate()
...
extension SizeBathReactiveCompatible {
    /// 关注/取消关注
    /// - Parameters:
    ///   - uid: 用户Id
    ///   - isAttention: true: 取关；false: 关注
    //: @objc func req_followRequest(uid: Int, isAttention: Bool, completion: ((_ succeed: Bool) -> Void)? = nil) {
    @objc func voiceDismiss(uid: Int, isAttention: Bool, completion: ((_ succeed: Bool) -> Void)? = nil) {
        ...
        if isAttention {
            ...
            dict[(userPathUrl.lowercased() + String(appMoreMessage))] = uid
            //: } else {
        } else {
            //: dict["attentionUid"] = uid
            dict[(user_shadowTitle.replacingOccurrences(of: "kit", with: "tt") + "ntionUi" + String(mainMarginKey))] = uid
            //: dict["source"] = "2"
            dict[(String(show_singleBackPath))] = "2"
        }
        //: SomebodyRequestTool.req_atationTool(isAttention: !isAttention, params: dict) { succeed, _, _ in
...
extension SizeBathReactiveCompatible {
    ...
    @objc private func underShowCard() {
        //: TalkingVoiceRoomReqTool.req_partyOnlineNum(roomId: partyModel.roomId) { succeed, result, _ in
        ResurveyReqTool.libertyParty(roomId: partyModel.roomId) { succeed, result, _ in
            //: guard succeed else { return }
            guard succeed else { return }
            //: let json = JSON(result ?? [String: Any]())
            let json = JSON(result ?? [String: Any]())
            //: self.onlineNum = json["num"].intValue
            self.onlineNum = json[(String(showVoiceTitle))].intValue
            //: self.refreshOnlineNum()
            self.titleEnableline()
        }
    }
    ...
    private func moveOff() {
        ...
        onlineNunTimer = Timer.scheduledTimer(timeInterval: 60, target: self, selector: #selector(underShowCard), userInfo: nil, repeats: true)
        //: onlineNunTimer?.fire()
        onlineNunTimer?.fire()
    }
...
extension SizeBathReactiveCompatible {
    ...
    func addBy() {
        ...
        partyModel = RangeManager.universalShared().partyModel
        //: nickNameLab.text = partyModel.streamerInfo.nickname
        nickNameLab.text = partyModel.streamerInfo.nickname
        //: uidLab.text = "UID:\(partyModel.streamerInfo.uid)"
        uidLab.text = (String(userTabElementStr)) + "\(partyModel.streamerInfo.uid)"
        ...
        icon.beforeRender(urlStr: partyModel.streamerInfo.headPic, placeImg: UIImage.imaginationImageSex(sex: partyModel.streamerInfo.sex), options: optionsInfo)
        // 是否是主播
        //: let isAnchor = (String(partyModel.streamerInfo.uid) == PunctuateThen.share.loginUid)
        let isAnchor = (String(partyModel.streamerInfo.uid) == PunctuateThen.share.loginUid)
        //: if isAnchor {
        if isAnchor {
            //: refreshFollow(isAttention: true)
            afterAttention(isAttention: true)
            //: } else {
        } else {
            //: refreshFollow(isAttention: partyModel.streamerInfo.isAttention)
            afterAttention(isAttention: partyModel.streamerInfo.isAttention)
        }
        // 定时更新在线人数
        //: startTimer()
        moveOff()
    ...
    @objc private func fallBackToSnap() {
        //: req_followRequest(uid: partyModel.streamerInfo.uid, isAttention: partyModel.streamerInfo.isAttention) { [weak self] succeed in
        voiceDismiss(uid: partyModel.streamerInfo.uid, isAttention: partyModel.streamerInfo.isAttention) { [weak self] succeed in
            //: guard let self = self else { return }
            guard let self = self else { return }
            //: guard succeed else { return }
            guard succeed else { return }
            ...
            self.afterAttention(isAttention: self.partyModel.streamerInfo.isAttention)
        }
    }
    ...
    func afterAttention(isAttention: Bool) {
    ...
    @objc private func judgeBar() {
        //: onlineListView.showView()
        onlineListView.demonstrateGuidance()
    }
}
...
extension SizeBathReactiveCompatible {
    ...
    private func magnitudeeraction() {
        ...
        frame = CGRect(x: 0, y: 0, width: kQueryValue, height: constLiveErrorMessage + actualWidth(w: 43))
