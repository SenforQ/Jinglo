...
fileprivate let app_handleUrl:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "Online Users" :*/
fileprivate let app_shareDateData:[Character] = ["O","n","l","i","n","e"," ","U"]
fileprivate let showImageCenterName:String = "SERS"
...
class FailViewDelegate: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: app_handleUrl.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var dismissBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(dismissAddClick), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var tipsLb: UILabel = {
        ...
        lb.text = (String(app_shareDateData) + showImageCenterName.lowercased()).localized
        //: return lb
        return lb
        //: }()
    }()
...
extension FailViewDelegate {
    /// 请求用户在线列表
    //: private func reqData() {
    private func propertyData() {
        //: TalkingVoiceRoomReqTool.req_partyOnlineList(roomId: TalkingVoiceRoomManager.shared().partyModel.roomId) { succeed, result, _ in
        ResurveyReqTool.credits(roomId: RangeManager.universalShared().partyModel.roomId) { succeed, result, _ in
            ...
            if let datas = [MakeListModel].deserialize(from: array as? Array) {
...
extension FailViewDelegate {
    ...
    @objc private func dismissAddClick() {
        //: dismiss()
        someExecute()
    }
    ...
    func demonstrateGuidance() {
        ...
        contentView.frame = CGRect(x: 0, y: notiKeyTitle, width: kQueryValue, height: contentV_H)
        //: reqData()
        propertyData()
...
extension FailViewDelegate: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell?.messagePage(model: DataSource[indexPath.row], index: indexPath.row)
        //: return cell!
        return cell!
    }
