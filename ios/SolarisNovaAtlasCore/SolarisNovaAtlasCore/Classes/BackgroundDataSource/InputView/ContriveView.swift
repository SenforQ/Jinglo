...
fileprivate let constIndexUrl:String = "设置普通"
fileprivate let data_progressKitFormat:String = "字\u{989c}色时"
fileprivate let dataWithTitle:String = "RGB\u{7a7a}间的\u{989c}色"
fileprivate let app_photoMessage:String = "last"

/*: "设置选中状态的文字颜色时 请使用RGB空间的颜色值" :*/
fileprivate let noti_imageHiddenMsg:[Character] = ["设","置","\u{9009}","中","状","\u{6001}","的","文","字","\u{989c}","色","\u{65f6}"," ","请"]
fileprivate let show_titleName:String = "使用RGB空view size remove"

/*: "init(coder:) has not been implemented" :*/
fileprivate let const_shareFormat:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "设置的下标不合法!!" :*/
fileprivate let k_capacityUrl:String = "\u{8bbe}\u{7f6e}的下"

/*: "Bags" :*/
fileprivate let const_appText:String = "Bagsprofile model finish self user"
...
open class ContriveView: UIView {
    //: open var segmentStyle: SegmentStyle
    open var segmentStyle: MemorySegmentStyle
    /// 点击响应
    ...
    fileprivate lazy var normalColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let normalRgb = self.colorName(self.segmentStyle.normalTitleColor) {
            //: return normalRgb
            return normalRgb
            //: } else {
        } else {
            //: fatalError("设置普通状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((constIndexUrl.capitalized + "状\u{6001}的文" + data_progressKitFormat + " 请使\u{7528}" + dataWithTitle + app_photoMessage.replacingOccurrences(of: "last", with: "值")))
        }
        //: }()
    }()
    ...
    fileprivate lazy var selectedTitleColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let selectedRgb = self.colorName(self.segmentStyle.selectedTitleColor) {
            //: return selectedRgb
            return selectedRgb
            //: } else {
        } else {
            //: fatalError("设置选中状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((String(noti_imageHiddenMsg) + String(show_titleName.prefix(6)) + "间的颜色值"))
        }
    ...
    fileprivate func colorName(_ color: UIColor) -> (r: CGFloat, g: CGFloat, b: CGFloat)? {
    ...
    public init(frame: CGRect, segmentStyle: MemorySegmentStyle, titles: [String]) {
    ...
    @objc func makeClick(_ tapGes: UITapGestureRecognizer) {
        //: guard let currentLabel = tapGes.view as? CustomLabel else { return }
        guard let currentLabel = tapGes.view as? AnglicismActionCustomLabel else { return }
    ...
    public required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: const_shareFormat.reversed(), encoding: .utf8)!)
    }
}
...
//: extension ScrollSegmentView {
extension ContriveView {
    // 根据Titles添加相应的控件
    //: fileprivate func setupTitles() {
    fileprivate func barWith() {
        //: for (index, title) in titles.enumerated() {
        for (index, title) in titles.enumerated() {
            //: let label = CustomLabel(frame: CGRect.zero)
            let label = AnglicismActionCustomLabel(frame: CGRect.zero)
            //: label.tag = index
            label.tag = index
            //: label.text = title
            label.text = title
            ...
            let tapGes = UITapGestureRecognizer(target: self, action: #selector(self.makeClick(_:)))
            ...
            if title == (String(const_appText.prefix(4))).localized {
    ...
    fileprivate func eventInsideBackground() {
        // 设置Label位置
    ...
    fileprivate func withGroupMax() {
        ...
        if let firstLabel = labelsArray[0] as? AnglicismActionCustomLabel {
    ...
    func targetAnimate(_: Bool) {
        // 选中Bags，隐藏红点
        //: if currentIndex == titles.firstIndex(of: "Bags".localized) {
        if currentIndex == titles.firstIndex(of: (String(const_appText.prefix(4))).localized) {
            //: redBagsLab.isHidden = true
            redBagsLab.isHidden = true
            //: TalkingChatGiftManager.share.showBagsRed = false
            NumberryReactiveCompatible.share.showBagsRed = false
        }

        //: guard currentIndex != oldIndex else { return }
        guard currentIndex != oldIndex else { return }
        //: let oldLabel = labelsArray[oldIndex] as! CustomLabel
        let oldLabel = labelsArray[oldIndex] as! AnglicismActionCustomLabel
        //: let currentLabel = labelsArray[currentIndex] as! CustomLabel
        let currentLabel = labelsArray[currentIndex] as! AnglicismActionCustomLabel
        // 让选中标签居中显示
    ...
    public func resume(_ currentIndex: Int) {
...
open class AnglicismActionCustomLabel: UILabel {
    /// 用来记录当前label的缩放比例
