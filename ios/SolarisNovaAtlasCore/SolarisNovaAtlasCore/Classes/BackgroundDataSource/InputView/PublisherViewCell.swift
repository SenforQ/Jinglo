...
fileprivate let dataNoKey:[UInt8] = [0x72,0x75,0x72,0x6f,0x33,0x78,0x74,0x7f,0x7e,0x69,0x21,0x32,0x3b,0x73,0x7a,0x68,0x3b,0x75,0x74,0x6f,0x3b,0x79,0x7e,0x7e,0x75,0x3b,0x72,0x76,0x6b,0x77,0x7e,0x76,0x7e,0x75,0x6f,0x7e,0x7f]

private func hiddenModel(date num: UInt8) -> UInt8 {
    return num ^ 27
}

/*: "icon_coinxxx" :*/
fileprivate let userWriteMainStr:[Character] = ["i","c","o","n","_","c","o","i","n","x","x"]
fileprivate let appBeautyPath:String = "effect"

/*: "icon_gift_num_bg" :*/
fileprivate let main_colorLayerKey:String = "scratchon"
fileprivate let app_throughMsg:[Character] = ["n","u","m","_","b","g"]

/*: "x1" :*/
fileprivate let show_timeGalleryFrameMsg:String = "count"

/*: " x :*/
fileprivate let showTheUrl:String = "the touch x"
...
class PublisherViewCell: UICollectionViewCell {
    //: var model = TalkingRoomGiftModel()
    var model = AvailableTransformable()
    //: let checked = false
    let checked = false
    //: let giftNum = 0
    let giftNum = 0
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: dataNoKey.map{hiddenModel(date: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var nameLB: UILabel = {
        ...
        lb.font = UIFont.clickLarge(type: .Regular, fontSize: 10)
        //: lb.textColor = .white
        lb.textColor = .white
    ...
    lazy var priceBtn: StartImageButton = {
        //: let btn = TalkingButton.init()
        let btn = StartImageButton()
        //: btn.setTitleColor(UIColor.white, for: .normal)
        btn.setTitleColor(UIColor.white, for: .normal)
        //: btn.titleLabel?.font = UIFont.pingfangFont(type: .Regular, fontSize: 10)
        btn.titleLabel?.font = UIFont.clickLarge(type: .Regular, fontSize: 10)
        //: btn.setImage(UIImage.BundleImageNamed(name: "icon_coinxxx"), for: .normal)
        btn.setImage(UIImage.thoughtImageBundleSizeText(name: (String(userWriteMainStr) + appBeautyPath.replacingOccurrences(of: "effect", with: "x"))), for: .normal)
    ...
    lazy var numBgView: UIImageView = {
        ...
        let imag = UIImage.thoughtImageBundleSizeText(name: (main_colorLayerKey.replacingOccurrences(of: "scratch", with: "ic") + "_gift_" + String(app_throughMsg)))
        //: let sizex = imag.size
        let sizex = imag.size
    ...
    lazy var numLb: UILabel = {
        ...
        lb.font = UIFont.clickLarge(type: .Regular, fontSize: 9)
        //: return lb
        return lb
        //: }()
    }()
...
extension PublisherViewCell {
    /// 更改选中状态
    //: func setChecked(checked: Bool) {
    func fill(checked: Bool) {
        ...
        if checked {
            ...
            bgView.layer.borderColor = UIColor.readRemove().cgColor
    ...
    func viewToMagnitude(number: Int) {
    ...
    func sizeSquareManager(currenmodel: AvailableTransformable, giftType: ClickMirrorPath) {
        //: model = currenmodel
        model = currenmodel
        //: giftIcon.setUrlImage(urlStr: model.imgPreview ?? "")
        giftIcon.putBackThroughFinish(urlStr: model.imgPreview ?? "")
        //: priceBtn.setTitle("\(model.price)", for: .normal)
        priceBtn.setTitle("\(model.price)", for: .normal)
        //: selectedImageView.setUrlImage(urlStr: model.cornerMark, placeImg: nil)
        selectedImageView.putBackThroughFinish(urlStr: model.cornerMark, placeImg: nil)
        ...
        let attributes = [NSAttributedString.Key.font: UIFont.clickLarge(type: .Regular, fontSize: 11),
...
extension PublisherViewCell {
    ...
    func sharedBuild() {
        ...
        bgView.backgroundColor = UIColor.momentColor()
