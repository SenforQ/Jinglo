...
/*: "gifts" :*/
fileprivate let appFullName:String = "giequals"

/*: "vipGifts" :*/
fileprivate let app_releaseFormat:String = "VIP"
fileprivate let constSizeId:[Character] = ["G","i","f","t","s"]

/*: "packGifts" :*/
fileprivate let appLabelId:[Character] = ["p","a","c","k","G","i","f","t","s"]

/*: "giftSendItem" :*/
fileprivate let user_transformStr:String = "bar"
fileprivate let noti_angleKitMessage:[Character] = ["f","t","S","e","n","d","I","t","e","m"]
...
// __CLOSE_PRINT__
//
//  NumberryReactiveCompatible.swift
//  AbroadTalking
//
...
class NumberryReactiveCompatible: NSObject {
    /// 是否展示背包红点
    ...
    private var cacheDict = [String: [ClickMirrorPath: [Any]]]()
    // 数据锁
    ...
    private var giftType = RefOutputStream.normal

    //: static let share = TalkingChatGiftManager()
    static let share = NumberryReactiveCompatible()
...
extension NumberryReactiveCompatible {
    /// 发送礼物面板数据
    /// - Parameters:
    ...
    func messageMatch(type: RefOutputStream = .normal, isRefresh: Bool = false, completion: @escaping () -> Void) {
        //: TalkingChatGiftManager.share.giftType = type
        NumberryReactiveCompatible.share.giftType = type
        //: if isRefresh == false {
        if isRefresh == false {
            //: let dataDict = cacheDict[type.rawValue]
            let dataDict = cacheDict[type.rawValue]
            //: let hotArr = dataDict?[GiftDataType.Hot] ?? [Any]()
            let hotArr = dataDict?[ClickMirrorPath.Hot] ?? [Any]()
        ...
        if type == .groupChat {
            ...
            NumberryReactiveCompatible.share.loadChatCompletion(completion: { isSucceed in
            ...
            NumberryReactiveCompatible.share.skiagram(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
            ...
            NumberryReactiveCompatible.share.preempt(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
            ...
            NumberryReactiveCompatible.share.changePhone(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
    ...
    private func changePhone(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = TalkingChatGiftManager.share.giftType
        let type = NumberryReactiveCompatible.share.giftType
        //: SomebodyRequestTool.req_ChatGiftList(type: type.rawValue) { succeed, result, errorModel in
        SomebodyRequestTool.replace(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
                //: var dataDict = [GiftDataType: [Any]]()
                var dataDict = [ClickMirrorPath: [Any]]()
                // hot礼物
                //: if let array: Array = data["gifts"] as? Array<Any> {
                if let array: Array = data[(appFullName.replacingOccurrences(of: "equal", with: "ft"))] as? [Any] {
                    //: var hotArr = [Any]()
                    var hotArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<AvailableTransformable>.deserialize(from: array as? Array) {
                        //: hotArr.append(contentsOf: datas as [Any])
                        hotArr.append(contentsOf: datas as [Any])
                    }
                    //: dataDict[GiftDataType.Hot] = hotArr
                    dataDict[ClickMirrorPath.Hot] = hotArr
                }

                // vip礼物
                //: if let array: Array = data["vipGifts"] as? Array<Any> {
                if let array: Array = data[(app_releaseFormat.lowercased() + String(constSizeId))] as? [Any] {
                    //: var vipArr = [Any]()
                    var vipArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<AvailableTransformable>.deserialize(from: array as? Array) {
                    ...
                    dataDict[ClickMirrorPath.Vip] = vipArr
                }

                // Bags礼物
                //: if let array: Array = data["packGifts"] as? Array<Any> {
                if let array: Array = data[(String(appLabelId))] as? [Any] {
                    //: var bagsArr = [Any]()
                    var bagsArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<AvailableTransformable>.deserialize(from: array as? Array) {
                    ...
                    dataDict[ClickMirrorPath.Bags] = bagsArr
                }
                ...
                let sendItem: Array = data[(user_transformStr.replacingOccurrences(of: "bar", with: "gi") + String(noti_angleKitMessage))] as! [Any]
                ...
                for i in 0 ..< sendItem.count {
                    ...
                    if let configModel = JSONDeserializer<SubMeasurable>.deserializeFrom(dict: sendItem[i] as? [String: Any], designatedPath: nil) {
    ...
    private func loadChatCompletion(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = TalkingChatGiftManager.share.giftType
        let type = NumberryReactiveCompatible.share.giftType
        //: SomebodyRequestTool.req_ChatRoomGiftList(type: type.rawValue) { succeed, result, errorModel in
        SomebodyRequestTool.allCompletion(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    func sexCompletion(params: [String: Any], completion: @escaping (_ data: [Any]?) -> Void) {
        //: SomebodyRequestTool.req_getChatRoomMembers(params: params, completion: { succeed, result, errorModel in
        SomebodyRequestTool.comeInsideVocalisation(params: params, completion: { succeed, result, _ in
            //: if succeed {
            if succeed {
                ...
                for dict in data {
                    ...
                    if let memberModel = JSONDeserializer<SodalistSendMemberModel>.deserializeFrom(dict: dict as? [String: Any], designatedPath: nil) {
    ...
    private func skiagram(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = GiftViewStyle.live
        let type = RefOutputStream.live
        //: SomebodyRequestTool.req_LiveGiftList(type: type.rawValue) { succeed, result, errorModel in
        SomebodyRequestTool.untilShow(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    private func preempt(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = GiftViewStyle.party
        let type = RefOutputStream.party
        //: SomebodyRequestTool.req_partyGiftList(type: type.rawValue) { succeed, result, errorModel in
        SomebodyRequestTool.account(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    func page(perPageCount: Int, dataType: ClickMirrorPath) -> Int {
    ...
    func hetUpViewStatus(dataType: ClickMirrorPath) -> [Any] {
        //: cacheLock.lock()
        cacheLock.lock()
        //: defer {
        defer {
    ...
    func ofItems() -> [Any] {
        ...
        if NumberryReactiveCompatible.share.giftType == .groupChat {
            //: return chatRoomSendItems
            return chatRoomSendItems
            //: } else if TalkingChatGiftManager.share.giftType == .live || TalkingChatGiftManager.share.giftType == .party {
        } else if NumberryReactiveCompatible.share.giftType == .live || NumberryReactiveCompatible.share.giftType == .party {
    ...
    func ground(dataType: ClickMirrorPath, dataArr: [Any]) {
        //: cacheLock.lock()
        cacheLock.lock()
        //: defer {
        defer {
...
extension NumberryReactiveCompatible {
    ...
    private func localNotification() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(titleAdd),
                                               //: name: GIFT_REFRESHPACKAGE_NOTIFICATION,
                                               name: dataTurnContent,
                                               //: object: nil)
                                               object: nil)
    }
...
extension NumberryReactiveCompatible {
    ...
    func suiteBy(showRed: Bool = false) {
        //: TalkingChatGiftManager.share.showBagsRed = showRed
        NumberryReactiveCompatible.share.showBagsRed = showRed
        //: cacheDict.removeAll()
        cacheDict.removeAll()
    }
}
