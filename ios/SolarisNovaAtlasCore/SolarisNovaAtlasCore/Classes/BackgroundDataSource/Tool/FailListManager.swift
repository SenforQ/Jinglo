...
/*: "uid" :*/
fileprivate let noti_labelId:String = "targetd"

/*: "onlineStatus" :*/
fileprivate let data_viewMessage:[Character] = ["o","n","l","i","n","e","S","t","a","t"]
fileprivate let notiLibraryUrl:String = "output"

/*: "isNewUser" :*/
fileprivate let dataTimeId:String = "isNewarray size"

/*: "isTPAuth" :*/
fileprivate let main_removeMessage:[Character] = ["i","s"]
fileprivate let showOverStr:[Character] = ["T","P","A","u","t","h"]

/*: "totalIntimate" :*/
fileprivate let appPathUrl:String = "tdisplaytal"
fileprivate let noti_actionPath:String = "tlabel"

/*: "userStatus" :*/
fileprivate let k_formatViewNameData:[Character] = ["u","s","e","r","S","t","a","t","u"]
fileprivate let mainRegularAddValue:[Character] = ["s"]

/*: "intimate" :*/
fileprivate let appScaleImageMsg:String = "succeed"
fileprivate let notiPlayerLikeData:String = "viewmate"
...
class FailListManager {
    ...
    private var moreMsgModel: TableReactiveCompatible = //: return TableReactiveCompatible.getPlaceholderModel(chatType: .moreMsg)
        .declaration(chatType: .moreMsg)
    //: }()

    // 初始化数据
    //: func req_initData() {
    func loadPush() {
        //: self.officialArr = ModelDataV2Listener.shared.getOfficialList()
        self.officialArr = ModelDataV2Listener.shared.retention()
        //: self.topArr = ModelDataV2Listener.shared.topConvList
        self.topArr = ModelDataV2Listener.shared.topConvList
        //: self.normalArr = ModelDataV2Listener.shared.norConvList
        self.normalArr = ModelDataV2Listener.shared.norConvList
        // 更多数据
        //: req_moreMsgInitData()
        viewFrom()
    ...
    func downExecute(completion: @escaping (_ succeed: Bool) -> Void) {
        //: guard self.isHalf == false else { return }
        guard self.isHalf == false else { return }
        //: ModelDataV2Listener.shared.getChatRoomData { succeed in
        ModelDataV2Listener.shared.circleOfCurvature { succeed in
            //: if succeed {
            if succeed {
                //: self.chatRoomArr = ModelDataV2Listener.shared.chatRoomList
                self.chatRoomArr = ModelDataV2Listener.shared.chatRoomList
                //: completion(true)
                completion(true)
            }
    ...
    func placeFormat(indexPath: IndexPath) -> TableReactiveCompatible? {
    ...
    func infoModel(model: TableReactiveCompatible?) -> TableReactiveCompatible? {
        //: guard let targetId = model?.targetId, targetId.isEmpty == false else { return model }
        guard let targetId = model?.targetId, targetId.isEmpty == false else { return model }
        //: let userInfo: RequestHandyJSON? = DBUserInfoManager.cache_getCachedUserInfo(targetId: targetId)
        let userInfo: RequestHandyJSON? = ReadoutReactiveCompatible.comparable(targetId: targetId)
    ...
    func tickFinish(conversationList: [TableReactiveCompatible], finish: (() -> Void)?) {
        //: var tempArr = [String]()
        var tempArr = [String]()
        ...
        ReadoutReactiveCompatible.inner(with: tempArr) { _, error in
    ...
    func sizeApp(conversationModel: TableReactiveCompatible) {
    ...
    func redItem(completion: @escaping (_ isFinish: Bool) -> Void) {
        //: ModelDataV2Listener.shared.func__getConversationList { arr, leftMore in
        ModelDataV2Listener.shared.dismissClick { _, leftMore in
            //: completion(leftMore)
            completion(leftMore)
        }
    }
    ...
    lazy var officialArr: [TableReactiveCompatible] = //: return Array<TableReactiveCompatible>()
        .init()
    //: }()

    //: lazy var topArr: [TalkingConversationModel] = {
    lazy var topArr: [TableReactiveCompatible] = //: return Array<TableReactiveCompatible>()
        .init()
    //: }()

    //: lazy var normalArr: [TalkingConversationModel] = {
    lazy var normalArr: [TableReactiveCompatible] = //: return Array<TableReactiveCompatible>()
        .init()
    //: }()

    //: lazy var chatRoomArr: [TalkingConversationModel] = {
    lazy var chatRoomArr: [TableReactiveCompatible] = //: return Array<TableReactiveCompatible>()
        .init()
    //: }()

    //: lazy var intimateNorArr: [TalkingConversationModel] = {
    lazy var intimateNorArr: [TableReactiveCompatible] = //: return Array<TableReactiveCompatible>()
        .init()
    //: }()

    //: lazy var intimateTopArr: [TalkingConversationModel] = {
    lazy var intimateTopArr: [TableReactiveCompatible] = //: return Array<TableReactiveCompatible>()
        .init()
    //: }()

    //: lazy var moreMsgArr: [TalkingConversationModel] = {
    lazy var moreMsgArr: [TableReactiveCompatible] = //: return Array<TableReactiveCompatible>()
        .init()
    //: }()
}
...
extension FailListManager {
    /// 初始化数据
    //: func req_moreMsgInitData() {
    func viewFrom() {
        //: self.moreMsgArr = ModelDataV2Listener.shared.moreMsgList
        self.moreMsgArr = ModelDataV2Listener.shared.moreMsgList
    }

    /// 根据索引获取模型（更多消息）
    //: func req_moreMsgConversationModel(indexPath: IndexPath) -> TalkingConversationModel? {
    func orderedSeries(indexPath: IndexPath) -> TableReactiveCompatible? {
    ...
    func timeWith() -> TableReactiveCompatible {
        // 更新更多消息最新数据
        //: req_moreMsgInitData()
        viewFrom()
        ...
        moreMsgModel.unreadCount = eradicate()
...
extension FailListManager {
    ...
    func overFirst(isFirst: Bool, completion: @escaping (_ succeed: Bool) -> Void) {
        ...
        ReadoutReactiveCompatible.outChat(with: data) { resultArr, _ in
            ...
            if resultArr?.count ?? 0 <= 0 { // 无数据
                ...
                resultArr?.forEach { dict in
                    ...
                    let uid = json[(noti_labelId.replacingOccurrences(of: "target", with: "ui"))].stringValue
                    //: let onlineStatus = json["onlineStatus"].intValue
                    let onlineStatus = json[(String(data_viewMessage) + notiLibraryUrl.replacingOccurrences(of: "output", with: "us"))].intValue
                    //: let isNewUser = json["isNewUser"].boolValue
                    let isNewUser = json[(String(dataTimeId.prefix(5)) + "User")].boolValue
                    //: let istpAuth = json["isTPAuth"].boolValue
                    let istpAuth = json[(String(main_removeMessage) + String(showOverStr))].boolValue
                    //: let totalIntimate = json["totalIntimate"].int ?? 0
                    let totalIntimate = json[(appPathUrl.replacingOccurrences(of: "display", with: "o") + "Intima" + noti_actionPath.replacingOccurrences(of: "label", with: "e"))].int ?? 0
                    //: let userStatus = json["userStatus"].int ?? 1
                    let userStatus = json[(String(k_formatViewNameData) + String(mainRegularAddValue))].int ?? 1
                    ...
                    self.actionShow(uid: uid, intimate: totalIntimate)
                    //: })
                }
            }
            //: completion(true)
    ...
    func deviseInfo(userInfo: [String: Any]) -> Bool {
        //: let uid = userInfo["uid"] as! String
        let uid = userInfo[(noti_labelId.replacingOccurrences(of: "target", with: "ui"))] as! String
        //: let onlineStatus = userInfo["onlineStatus"] as! Int
        let onlineStatus = userInfo[(String(data_viewMessage) + notiLibraryUrl.replacingOccurrences(of: "output", with: "us"))] as! Int
        //: let isNewUser = userInfo["isNewUser"] as! Bool
        let isNewUser = userInfo[(String(dataTimeId.prefix(5)) + "User")] as! Bool

        /// 用户状态更新
        //: let userStatus = userInfo["userStatus"] as? Int ?? 1
        let userStatus = userInfo[(String(k_formatViewNameData) + String(mainRegularAddValue))] as? Int ?? 1
        //: var needuserStatus = false
        var needuserStatus = false
        //: if userInfo.keys.contains("userStatus") {
        if userInfo.keys.contains((String(k_formatViewNameData) + String(mainRegularAddValue))) {
            //: needuserStatus = true
            needuserStatus = true
        }
...
extension FailListManager {
    ...
    func sizeChange() {
        ...
        self.intimateTopArr.removeAll()
        //: for model in ModelDataV2Listener.shared.norConvList {
        for model in ModelDataV2Listener.shared.norConvList {
            //: if model.totalIntimate >= PunctuateThen.share.appUserConfigMode.intimateLimit.enterTab {
            if model.totalIntimate >= PunctuateThen.share.appUserConfigMode.intimateLimit.enterTab {
                //: let userModel = self.getUserInfoFromCache(model: model)
                let userModel = self.infoModel(model: model)
                //: self.intimateNorArr.append(userModel!)
                self.intimateNorArr.append(userModel!)
            }
        }
        //: for model in ModelDataV2Listener.shared.topConvList {
        for model in ModelDataV2Listener.shared.topConvList {
            //: if model.totalIntimate >= PunctuateThen.share.appUserConfigMode.intimateLimit.enterTab {
            if model.totalIntimate >= PunctuateThen.share.appUserConfigMode.intimateLimit.enterTab {
                //: let userModel = self.getUserInfoFromCache(model: model)
    ...
    func constraintTop(convModel: TableReactiveCompatible, isTop: Bool) {
        //: if isTop {
        if isTop {
    ...
    func equalConversation(indexPath: IndexPath) -> TableReactiveCompatible? {
        //: switch indexPath.section {
        switch indexPath.section {
    ...
    func actionShow(uid: String, intimate: Int) {
        ...
        ReadoutReactiveCompatible.checkTime(with: uid, intimate: intimate, currTime: Int(Date().timeIntervalSince1970))
    }

    /// 更新消息列表单个用户亲密度
    //: func update_userIntimate(userInfo: [String: Any]) -> Bool {
    func modifyPushNutsAndBoltsInfo(userInfo: [String: Any]) -> Bool {
        //: let uid = userInfo["uid"] as! String
        let uid = userInfo[(noti_labelId.replacingOccurrences(of: "target", with: "ui"))] as! String
        //: let intimate = userInfo["intimate"] as! Int
        let intimate = userInfo[(appScaleImageMsg.replacingOccurrences(of: "succeed", with: "in") + notiPlayerLikeData.replacingOccurrences(of: "view", with: "ti"))] as! Int
        //: var updateSucceed = false
        var updateSucceed = false
        //: if let topModel = self.topArr.filter({ $0.userID == uid }).first {
        if let topModel = self.topArr.filter({ $0.userID == uid }).first {
