...
fileprivate let app_keyMakeName:[UInt8] = [0xe4,0xe3,0xe4,0xf9,0xa5,0xee,0xe2,0xe9,0xe8,0xff,0xb7,0xa4,0xad,0xe5,0xec,0xfe,0xad,0xe3,0xe2,0xf9,0xad,0xef,0xe8,0xe8,0xe3,0xad,0xe4,0xe0,0xfd,0xe1,0xe8,0xe0,0xe8,0xe3,0xf9,0xe8,0xe9]

/*: "SendView" :*/
fileprivate let constNamePath:String = "var toDropM"
fileprivate let userObjectUrl:String = "if copy stillrMenuCe"
fileprivate let appModeEqualData:String = "LL"

/*: "roomId" :*/
fileprivate let showPhotoModelValue:String = "roomIdelse click type guard true"

/*: "type" :*/
fileprivate let noti_postValue:String = "tyconstraint"

/*: "page" :*/
fileprivate let mainPointMessage:[Character] = ["p","a","g","e"]

/*: "icon_chats_pcr" :*/
fileprivate let mainToKey:[Character] = ["i","c","o","n","_","c","h","a","t","s","_","p","c","r"]

/*: "All Numbers" :*/
fileprivate let dataModelMessage:String = "All Ndrunk make"
fileprivate let userPushName:String = "share"
fileprivate let showUnderId:[Character] = ["m","b","e","r","s"]

/*: "all" :*/
fileprivate let app_dataMsg:[Character] = ["a","l","l"]

/*: "Recent Gifts" :*/
fileprivate let showInputTitle:String = "Recentif universal section live"
fileprivate let notiAttributePath:String = " Giftspath video"
...
protocol WeltanschauungViewDelegate {
...
class RefSessionMenuView: UIView {
    //: var pageIndex = 0
    var pageIndex = 0
    //: var roomID = ""
    var roomID = ""
    //: var menuDelegate: DropDownMemberMenuViewDelegate?
    var menuDelegate: WeltanschauungViewDelegate?
    ...
    public var triangleHeight = Config.ColorConfig.triangleHeight
    /**
     *  调整使下拉优先 当向下偏转屏幕距离足够，优先向下偏转
     */
    //: public var adjustPullDown = Config.DropMenuConfig.adjustPullDown
    public var adjustPullDown = Config.ColorConfig.adjustPullDown
    ...
    fileprivate lazy var contentView: UIView = self.buttonBackground()
    //: fileprivate lazy var mTable: UITableView = self.makeTableView()
    fileprivate lazy var mTable: UITableView = self.alongStatus()
    //: fileprivate var anchorRect = CGRect.zero
    fileprivate var anchorRect = CGRect.zero

    //: static let shared = DropDownMemberMenuView(frame: .zero)
    static let shared = RefSessionMenuView(frame: .zero)
    ...
    init(frame: CGRect, anchorView: UIView, roomID: String) {
        ...
        super.init(frame: frame)
        //: let window = PublisherReactiveCompatible.getWindow()
        let window = PublisherReactiveCompatible.exceptAction()
        //: self.roomID = roomID
        self.roomID = roomID
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: app_keyMakeName.map{$0^141}, encoding: .utf8)!)
    }

    //: private func makeContentView() -> UIView {
    private func buttonBackground() -> UIView {
        //: let view  = UIView()
        let view = UIView()
        //: return view
        return view
    ...
    private func alongStatus() -> UITableView {
    ...
    private func undersurface() {
        ...
        mTable.register(SendView.self, forCellReuseIdentifier: (String(constNamePath.suffix(5)) + "embe" + String(userObjectUrl.suffix(7)) + appModeEqualData.lowercased()))
    ...
    func source() {
        ...
        var dict = [String: Any]()
        //: dict["roomId"] = roomID
        dict[(String(showPhotoModelValue.prefix(6)))] = roomID
        //: dict["type"] = "3"
        dict[(noti_postValue.replacingOccurrences(of: "constraint", with: "pe"))] = "3"
        //: dict["page"] = String(pageIndex)
        dict[(String(mainPointMessage))] = String(pageIndex)
        ...
        NumberryReactiveCompatible.share.sexCompletion(params: dict, completion: { array in
            ...
            if self.pageIndex == 0 {
                ...
                let model = SodalistSendMemberModel()
                //: model.headPic = "icon_chats_pcr"
                model.headPic = (String(mainToKey))
                //: model.nickname = "All Numbers".localized
                model.nickname = (String(dataModelMessage.prefix(5)) + userPushName.replacingOccurrences(of: "share", with: "u") + String(showUnderId)).localized
                //: model.uid = "all"
                model.uid = (String(app_dataMsg))
                //: self.menuArray.insert(model, at: 0)
                self.menuArray.insert(model, at: 0)
            }

            //: self.refreshUI()
            self.underLiveCurrent()
...
extension RefSessionMenuView: UITableViewDelegate, UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: (String(constNamePath.suffix(5)) + "embe" + String(userObjectUrl.suffix(7)) + appModeEqualData.lowercased()), for: indexPath) as! SendView
        ...
        if recentmenuArray.count > 0 && indexPath.section == 0 {
            ...
            cellModel = recentmenuArray[indexPath.row] as! SodalistSendMemberModel
            //: }else {
        } else {
            //: cellModel = menuArray[indexPath.row] as! TalkingChatRoomMemberModel
            cellModel = menuArray[indexPath.row] as! SodalistSendMemberModel
        }
        //: cell.setDropMemberMenuCell(model: cellModel)
        cell.snapUser(model: cellModel)
        //: return cell
        return cell
    }
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        menuDelegate?.moveListModel(model: cellModel)
    }

    //: func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
    func tableView(_: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        //: let headeView = UILabel.init(frame: CGRect(x: 0, y: 0, width: ScreenWidth, height: 23))
        let headeView = UILabel(frame: CGRect(x: 0, y: 0, width: kQueryValue, height: 23))
        //: headeView.backgroundColor = UIColor.appBgColor()
        headeView.backgroundColor = UIColor.sizeColor()
        //: let titleLB = UILabel.init()
        let titleLB = UILabel()
        //: titleLB.frame = CGRect(x: 3, y: 5, width: 100, height: 14)
        titleLB.frame = CGRect(x: 3, y: 5, width: 100, height: 14)
        //: titleLB.font = .pingfangFont(type: .Medium, fontSize: 12)
        titleLB.font = .clickLarge(type: .Medium, fontSize: 12)
        //: titleLB.textColor = .appValueDetailColor()
        titleLB.textColor = .hockeyPlayer()
        //: if recentmenuArray.count > 0 && section == 0 {
        if recentmenuArray.count > 0 && section == 0 {
            //: titleLB.text = "Recent Gifts".localized
            titleLB.text = (String(showInputTitle.prefix(6)) + String(notiAttributePath.prefix(6))).localized
            //: }else {
        } else {
            //: titleLB.text = "All Numbers".localized
            titleLB.text = (String(dataModelMessage.prefix(5)) + userPushName.replacingOccurrences(of: "share", with: "u") + String(showUnderId)).localized
        }
