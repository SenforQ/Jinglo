...
fileprivate let notiQuoteText:[UInt8] = [0x76,0x71,0x76,0x6b,0x37,0x7c,0x70,0x7b,0x7a,0x6d,0x25,0x36,0x3f,0x77,0x7e,0x6c,0x3f,0x71,0x70,0x6b,0x3f,0x7d,0x7a,0x7a,0x71,0x3f,0x76,0x72,0x6f,0x73,0x7a,0x72,0x7a,0x71,0x6b,0x7a,0x7b]

private func popularArrayLab(end num: UInt8) -> UInt8 {
    return num ^ 31
}

/*: "SHBDropMenuCell" :*/
fileprivate let const_equalId:String = "should manager botSHBDro"
fileprivate let const_playerData:String = "uCellorigin text case"

/*: "title" :*/
fileprivate let notiInfoKey:String = "taddtle"
...
public class AvailableViewDelegate: UIView {
    //: var menuDelegate: DropDownMenuViewDelegate?
    var menuDelegate: LightViewDelegate?
    ...
    var menuBgColor: UIColor? {
        ...
        //: }
    } // = Config.ColorConfig.menuBgColor
    // 线条颜色
    //: var lineColor = Config.MenuCellConfig.lineColor
    var lineColor = Config.TopConfig.lineColor
    // cell高度
    //: public var menuCellHeight = Config.MenuCellConfig.menuCellHeight
    public var menuCellHeight = Config.TopConfig.menuCellHeight
    /**
     *  table最大高度限制
     *  默认：5 * cellHeight
     */
    //: var menuMaxHeight = Config.MenuCellConfig.menuMaxHeight
    var menuMaxHeight = Config.TopConfig.menuMaxHeight
    /**
     *  小三角高度
     *  45°等腰三角形
     */
    ...
    public var menuStyle = Config.initialMenuStyle ?? StatuteContentMenuStyle.MenuDarkStyle {
        //: didSet {
        didSet {
            //: switch menuStyle {
            switch menuStyle {
    ...
    public class func upArray(anchorView: UIView, titleArray: [String], imageArray: [UIImage]) -> AvailableViewDelegate {
        ...
        let menuView = cardOfArray(anchorView: anchorView, menuArray: menuArray)
        //: return menuView
        return menuView
    }

    //: public class func pullDropDrownMenu(anchorView: UIView, menuArray: [DropMenuModel]) -> DropDownMenuView {
    public class func cardOfArray(anchorView: UIView, menuArray: [SomebodyMenuModel]) -> AvailableViewDelegate {
        ...
        let menuView = AvailableViewDelegate()
...
//        menuView.refreshUI()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: notiQuoteText.map{popularArrayLab(end: $0)}, encoding: .utf8)!)
    }

    //: private func makeContentView() -> UIView {
    private func titleCan() -> UIView {
        //: let view  = UIView()
        let view = UIView()
        //: return view
        return view
    ...
    private func tipTable() -> UITableView {
    ...
    private func toOn() {
        ...
        mTable.register(ListViewCell.self, forCellReuseIdentifier: (String(const_equalId.suffix(6)) + "pMen" + String(const_playerData.prefix(5))))
...
extension AvailableViewDelegate: UITableViewDelegate, UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: (String(const_equalId.suffix(6)) + "pMen" + String(const_playerData.prefix(5))), for: indexPath) as! ListViewCell
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        selectionAction?(indexPath.row, cellModel.title ?? (notiInfoKey.replacingOccurrences(of: "add", with: "i")))
        //: animateRemoveView()
        culminate()

        //: menuDelegate?.didClickSelectedRow(index: indexPath.row, title: cellModel.title ?? "")
        menuDelegate?.postulation(index: indexPath.row, title: cellModel.title ?? "")
    }
}
