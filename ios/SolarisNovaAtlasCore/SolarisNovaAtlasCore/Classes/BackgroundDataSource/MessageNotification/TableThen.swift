...
fileprivate let mainWrapStr:[UInt8] = [0x94,0x99,0x94,0x9f,0x53,0x8e,0x9a,0x8f,0x90,0x9d,0x65,0x54,0x4b,0x93,0x8c,0x9e,0x4b,0x99,0x9a,0x9f,0x4b,0x8d,0x90,0x90,0x99,0x4b,0x94,0x98,0x9b,0x97,0x90,0x98,0x90,0x99,0x9f,0x90,0x8f]

fileprivate func textSlide(after num: UInt8) -> UInt8 {
...
/*: "Reply" :*/
fileprivate let main_titleMsg:[Character] = ["R","e","p","l","y"]

/*: "#F95151" :*/
fileprivate let kFrameMessage:String = "pic equal new hand of#F95151"
...
let constScreenData = (constLiveErrorMessage + 80.0)
...
class TableThen: UIView {
    ...
    var startDismissBlock: ((_ model: TableReactiveCompatible) -> Void)? // 开始移除视图Block
    //: var finishDismissBlock: ((_ view: TalkingMessageNotificationView) -> Void)?
    var finishDismissBlock: ((_ view: TableThen) -> Void)? // 完成移除视图Block
    //: private var currModel = TalkingConversationModel()
    private var currModel = TableReactiveCompatible()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: mainWrapStr.map{textSlide(after: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var nickNameLabel: UILabel = {
        ...
        label.font = .clickLarge(type: .Medium, fontSize: 15)
    ...
    private lazy var replyBtn: UIButton = {
        ...
        btn.titleLabel?.font = .clickLarge(type: .Medium, fontSize: 15)
        //: btn.setTitle("Reply".localized, for: .normal)
        btn.setTitle((String(main_titleMsg)).localized, for: .normal)
        //: btn.setTitleColor(UIColor(hex: "#F95151"), for: .normal)
        btn.setTitleColor(UIColor(hex: (String(kFrameMessage.suffix(7)))), for: .normal)
        //: btn.isEnabled = false
        btn.isEnabled = false
        //: return btn
        return btn
...
extension TableThen {
    ...
    func selected(_ model: TableReactiveCompatible?) {
    ...
    func viewFor(needAlpha: Bool = true) {
        ...
        DispatchQueue.main.async {
            ...
            UIView.animate(withDuration: 0.5) {
                ...
                if needAlpha {
                    ...
                    self.frame.origin.y = -constScreenData
                }
                //: } completion: { _ in
            } completion: { _ in
    ...
    @objc private func circumcise() {
        ...
        if currModel.chatType == .service {
            ...
            //: RangePushManager.share.func__pushToPriveteChatVC(chatID: currModel.targetId)
            RangePushManager.share.liveInsideCompletion(chatID: currModel.targetId)
        }
    }

    /// 滑动手势
    //: @objc private func swipeGestureEvent(gesture: UISwipeGestureRecognizer) {
    @objc private func govern(gesture: UISwipeGestureRecognizer) {
...
extension TableThen {
    ...
    func vocalisation() {
        ...
        if let userInfo = currModel.gj_userInfo {
            ...
            headImgView.beforeRender(urlStr: userInfo.headPic, placeImg: UIImage.imaginationImageSex(sex: userInfo.sex))
            //: if !userInfo.headPicFrame.isEmptyString {
            if !userInfo.headPicFrame.isEmptyString {
                //: iconBorder.isHidden = false
                iconBorder.isHidden = false
                //: iconBorder.setHeadFrameUrlImage(urlStr: userInfo.headPicFrame)
                iconBorder.glassesByStr(urlStr: userInfo.headPicFrame)
                //: } else {
            } else {
    ...
    private func choose() {
        ...
        self.frame = CGRect(x: 0, y: -constScreenData, width: kQueryValue, height: constScreenData)
    ...
    private func scale() {
        ...
        let swipe = UISwipeGestureRecognizer(target: self, action: #selector(govern))
        //: swipe.direction = .up
        swipe.direction = .up
