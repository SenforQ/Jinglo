...
/*: "rYMsgType" :*/
fileprivate let userTitleKey:String = "self app collection hiddenrYMsgT"
fileprivate let mainHandleStr:[Character] = ["y","p","e"]

/*: "GJ:CallCustom" :*/
fileprivate let notiMoreId:[Character] = ["G","J",":","C","a","l","l","C","u","s","t","o","m"]
...
// __CLOSE_PRINT__
//
//  MemoryCustomReactiveCompatible.swift
//  AbroadTalking
//
...
class MemoryCustomReactiveCompatible: NSObject {
    //: private var showView: TalkingMessageNotificationView?
    private var showView: TableThen? // 当前View
    //: private var preView: TalkingMessageNotificationView?
    private var preView: TableThen? // 旧的View
    ...
    static var shared = MemoryCustomReactiveCompatible()
    ...
    @objc func infoTouch() {
        ...
        if self.preView != nil {
            ...
            self.preView?.viewFor()
        }
        //: if self.showView != nil {
        if self.showView != nil {
            //: self.showView?.dismiss()
            self.showView?.viewFor()
        }
    ...
    private lazy var msgArr: [TableReactiveCompatible] = //: return [TableReactiveCompatible]()
        .init()
    //: }()
}
...
extension MemoryCustomReactiveCompatible {
    /// 处理接收到的TX消息，展示消息通知
    /// - Parameters:
    ///   - msg: V2TIMMessage
    ///   - dataDict: 消息字典
    //: func onRecvNewMessage(msg: V2TIMMessage, dataDict: [String: Any]) {
    func failureTool(msg: V2TIMMessage, dataDict: [String: Any]) {
        ...
        guard marginOfSafety() == true || insomniacDisplayable() == true else { return }
        //: guard msg.userID != PublisherReactiveCompatible.getCustomerServiceID() && msg.userID != PublisherReactiveCompatible.getXiaoMiID() else {
        guard msg.userID != PublisherReactiveCompatible.appId(), msg.userID != PublisherReactiveCompatible.exceptXiao() else {
            //: return
            return
        }
        //: let contenType = TUISocialChatManager.messageCellType(dataDict)
        let contenType = ModelThen.startBeauty(dataDict)
        // 提示消息
        ...
        if contenType == .TXMessageContentType_text, dataDict[(String(userTitleKey.suffix(6)) + String(mainHandleStr))] as? String == (String(notiMoreId)) {
            //: return
            return
        }

        // 2. V2TIMMessage => TableReactiveCompatible
        ...
        //: if let userInfo: RequestHandyJSON = DBUserInfoManager.cache_getCachedUserInfo(targetId: model.targetId) {
        if let userInfo: RequestHandyJSON = ReadoutReactiveCompatible.comparable(targetId: model.targetId) {
    ...
    private func translate() {
        ...
        let notifView = TableThen(frame: .zero)
        //: notifView.show(firstModel)
        notifView.selected(firstModel)
    ...
    private func insomniacDisplayable() -> Bool {
        //: guard let currentVC = currentViewController() else { return false }
        guard let currentVC = occurrentWith() else { return false }
        //: if currentVC.isKind(of: TalkingSocialViewController.self) ||
        if currentVC.isKind(of: DetailReactiveCompatible.self) ||
            //: currentVC.isKind(of: SocialPopularViewController.self) ||
            currentVC.isKind(of: ComeOnViewController.self) ||
            //: currentVC.isKind(of: TalkingMomentViewController.self) ||
            currentVC.isKind(of: CountoNavigationDelegate.self) ||
            ...
            currentVC.isKind(of: HiddenDataSource.self)
        {
