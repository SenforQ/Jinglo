...
fileprivate let kActualKey:[UInt8] = [0x1f,0x18,0x1f,0x2,0x5e,0x15,0x19,0x12,0x13,0x4,0x4c,0x5f,0x56,0x1e,0x17,0x5,0x56,0x18,0x19,0x2,0x56,0x14,0x13,0x13,0x18,0x56,0x1f,0x1b,0x6,0x1a,0x13,0x1b,0x13,0x18,0x2,0x13,0x12]

private func meAdd(quick num: UInt8) -> UInt8 {
    return num ^ 118
}

/*: "#30D00B" :*/
fileprivate let mainAtFormat:String = "temp color if result position#30D00"
fileprivate let appMakeViewKey:[Character] = ["B"]

/*: "icon_receivedcalls" :*/
fileprivate let mainNameUrl:String = "iedgen"
fileprivate let noti_arrayMessage:String = "gesture"
fileprivate let noti_closeValue:String = "cdatels"

/*: "Received calls" :*/
fileprivate let main_enterPath:[Character] = ["R","e","c","e","i","v","e","d"," ","c","a","l","l","s"]

/*: "icon_dialedcalls" :*/
fileprivate let kThoughtUrl:[Character] = ["i","c","o","n","_","d","i","a","l"]
fileprivate let show_locationName:String = "edcallmake"

/*: "Dialed calls" :*/
fileprivate let noti_lengthName:[Character] = ["D","i","a","l","e"]
fileprivate let main_labCellValueFormat:[Character] = ["d"," ","c","a","l","l","s"]

/*: "%02i:%02i" :*/
fileprivate let main_infoData:[Character] = ["%","0","2","i",":","%","0","2","i"]
...
// __CLOSE_PRINT__
//
//  PublicReactiveCompatible.swift
//  AbroadTalking
//
...
class PublicReactiveCompatible: UITableViewCell {
    //: var currenModel = TalkingVideoCallRecordModel()
    var currenModel = StartMeasurable()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kActualKey.map{meAdd(quick: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var iconBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(barLine), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    lazy var durationLB: UILabel = {
        ...
        label.font = .clickLarge(type: .Medium, fontSize: 10)
        //: label.textColor = .appValueDetailColor()
        label.textColor = .hockeyPlayer()
    ...
    lazy var onlineView: UIView = {
        ...
        view.backgroundColor = UIColor(hex: (String(mainAtFormat.suffix(6)) + String(appMakeViewKey)))
...
extension PublicReactiveCompatible {
    //: func setVideoCallCell(model: TalkingVideoCallRecordModel) {
    func dataConverterStart(model: StartMeasurable) {
        //: currenModel = model
        currenModel = model
        ...
        if !model.headPicFrame.isEmptyString {
            ...
            iconBorderBtn.glassesByStr(urlStr: model.headPicFrame)
            //: } else {
        } else {
        ...
        if model.callStatus == 0 {
            ...
            rcordImg.image = UIImage.thoughtImageBundleSizeText(name: (mainNameUrl.replacingOccurrences(of: "edge", with: "co") + "_receive" + noti_arrayMessage.replacingOccurrences(of: "gesture", with: "d") + noti_closeValue.replacingOccurrences(of: "date", with: "al")))
            //: rcordLB.text = "Received calls".localized
            rcordLB.text = (String(main_enterPath)).localized
            //: } else {
        } else {
            //: rcordImg.image = UIImage.BundleImageNamed(name: "icon_dialedcalls")
            rcordImg.image = UIImage.thoughtImageBundleSizeText(name: (String(kThoughtUrl) + show_locationName.replacingOccurrences(of: "make", with: "s")))
            //: rcordLB.text = "Dialed calls".localized
            rcordLB.text = (String(noti_lengthName) + String(main_labCellValueFormat)).localized
        }
