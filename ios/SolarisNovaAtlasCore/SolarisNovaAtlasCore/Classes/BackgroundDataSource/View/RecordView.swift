...
fileprivate let user_clickId:[Character] = ["N","e","e","d"," ","a"," ","g","i","f"]
fileprivate let user_presentId:String = "t ttool text component"
fileprivate let app_editKey:String = "o unlab as true manager"

/*: "init(coder:) has not been implemented" :*/
fileprivate let user_scaleName:[UInt8] = [0xed,0xea,0xed,0xf0,0xac,0xe7,0xeb,0xe0,0xe1,0xf6,0xbe,0xad,0xa4,0xec,0xe5,0xf7,0xa4,0xea,0xeb,0xf0,0xa4,0xe6,0xe1,0xe1,0xea,0xa4,0xed,0xe9,0xf4,0xe8,0xe1,0xe9,0xe1,0xea,0xf0,0xe1,0xe0]

private func managingDirector(at num: UInt8) -> UInt8 {
    return num ^ 132
}

/*: "intimatephoto_btn_no" :*/
fileprivate let showEqualMsg:[Character] = ["i","n","t","i","m","a","t","e","p","h","o","t","o","_","b"]
fileprivate let k_sizeStr:[Character] = ["t","n","_","n","o"]

/*: "Burn After Reading" :*/
fileprivate let showRefreshMsg:String = "Burn Afhidden match say as"
fileprivate let notiToName:String = "to messageter "

/*: "Use Photo" :*/
fileprivate let k_inputVoiceBorderFormat:String = "Use in user let catch row"
fileprivate let const_segmentTitle:[Character] = ["o"]

/*: "Use Video" :*/
fileprivate let mainReceiveFormat:String = "to let center temp clickUse Vi"
fileprivate let show_retMessage:String = "counto"

/*: "intimatephoto_btn_chose" :*/
fileprivate let const_appViewMsg:String = "postntpostm"
fileprivate let app_iconStr:String = "up iconhoto_bt"
fileprivate let app_resultSinglePath:[Character] = ["n","_"]
fileprivate let data_centerText:[Character] = ["c","h","o","s","e"]

/*: "%@ (%@x1  %@ Gold coins)" :*/
fileprivate let k_dataTitle:String = "%user ("
fileprivate let data_gestureUrl:String = "reason log if video content@ Go"
fileprivate let showLabKey:String = "view return as selfoins)"

/*: "#7E6AFF" :*/
fileprivate let mainMakeStr:[Character] = ["#","7","E","6","A","F","F"]
...
let userSessionKey = (176 + k_netId)
...
class RecordView: UIView {
    ...
    private let normalGiftText = (String(user_clickId) + String(user_presentId.prefix(3)) + String(app_editKey.prefix(4)) + "lock").localized
    //: private var giftId = "0"  // 解锁礼物Id
    private var giftId = "0" // 解锁礼物Id
    //: private var type = IntimateInfoViewType.intimatePhoto
    private var type = TotaloViewType.intimatePhoto // 私密类型

    //: init(frame: CGRect, type: IntimateInfoViewType) {
    init(frame: CGRect, type: TotaloViewType) {
        //: super.init(frame: frame)
        super.init(frame: frame)
        //: self.frame = CGRect(x: 0, y: ScreenHeight-INTIMATEINFOVIEW_H, width: ScreenWidth, height: INTIMATEINFOVIEW_H)
        self.frame = CGRect(x: 0, y: notiKeyTitle - userSessionKey, width: kQueryValue, height: userSessionKey)
        //: self.type = type
        self.type = type
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: user_scaleName.map{managingDirector(at: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var burnAfterReadingImgV: UIImageView = {
        ...
        let img = UIImageView(image: UIImage.thoughtImageBundleSizeText(name: (String(showEqualMsg) + String(k_sizeStr))))
    ...
    private lazy var burnAfterReadingLab: UILabel = {
        ...
        lab.text = (String(showRefreshMsg.prefix(7)) + String(notiToName.suffix(4)) + "Reading").localized
        //: return lab
        return lab
        //: }()
    }()
    ...
    private lazy var burnAfterReadingBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(speedReading), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var useBtn: UIButton = {
        ...
        btn.setTitle(self.type == .intimatePhoto ? (String(k_inputVoiceBorderFormat.prefix(4)) + "Phot" + String(const_segmentTitle)).localized : (String(mainReceiveFormat.suffix(6)) + show_retMessage.replacingOccurrences(of: "count", with: "de")).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(lifestyleLoad), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension RecordView {
    ...
    @objc private func lifestyleLoad() {
    ...
    @objc private func speedReading() {
        ...
        let imageName = burnAfterReadingBtn.isSelected ? (const_appViewMsg.replacingOccurrences(of: "post", with: "i") + "atep" + String(app_iconStr.suffix(7)) + String(app_resultSinglePath) + String(data_centerText)) : (String(showEqualMsg) + String(k_sizeStr))
        //: burnAfterReadingImgV.image = UIImage.BundleImageNamed(name: imageName)
        burnAfterReadingImgV.image = UIImage.thoughtImageBundleSizeText(name: imageName)
    }
}
...
extension RecordView {
    ...
    @objc private func fixing() {
        ...
        guard needGiftBtn.isSelected == false else { // 还原状态
            ...
            needGiftImgeV.image = UIImage.thoughtImageBundleSizeText(name: (String(showEqualMsg) + String(k_sizeStr)))
        ...
        let type = (self.type == .intimatePhoto ? RefOutputStream.intimatePhoto : RefOutputStream.intimateVideo)
        //: TalkingChatGiftManager.share.func__sendGiftEvent(type: type) {
        NumberryReactiveCompatible.share.messageMatch(type: type) {
    ...
    private func subBy() {
        ...
        giftView.okActionBlock = { [weak self] giftModel in
            ...
            self.needGiftImgeV.image = UIImage.thoughtImageBundleSizeText(name: (const_appViewMsg.replacingOccurrences(of: "post", with: "i") + "atep" + String(app_iconStr.suffix(7)) + String(app_resultSinglePath) + String(data_centerText)))
            ...
            let giftAttrStr = NSMutableAttributedString(string: (k_dataTitle.replacingOccurrences(of: "user", with: "@") + "%@x1  %" + String(data_gestureUrl.suffix(4)) + "ld c" + String(showLabKey.suffix(5))).viewfinder(self.normalGiftText, giftModel.name, giftModel.price))
            ...
            let attributes = [.foregroundColor: UIColor(hex: (String(mainMakeStr)))!, .font: UIFont.clickLarge(type: .Regular, fontSize: 14)] as [NSAttributedString.Key: Any]
...
extension RecordView {
    ...
    private func weltanschauung() {
        ...
        whiteBgV.addSubview(burnAfterReadingBtn)
        //: if PunctuateThen.share.loginUserMode.status != 1 &&
        if PunctuateThen.share.loginUserMode.status != 1,
    ...
    private func tied() {
        ...
        useBtn.snp.makeConstraints { make in
            ...
            make.bottom.equalToSuperview().offset(-20 - k_netId)
